...
[![Status: Alpha](https://img.shields.io/badge/status-alpha-c4785a)](./CHANGELOG.md)
...
> Owlat is currently in **alpha** — it works end-to-end, but expect rough edges and breaking changes between releases.
...
The wizard offers to **explore with sample data**: 15 contacts across 3 topics, email templates, a sent campaign with stats, an automation and a verified `demo.localhost` domain, so an empty instance isn't your first impression. It is opt-in, every row it writes is tagged, and one command takes it back:
...
- **Populated** *(default)* — creates an admin user and seeds realistic demo data (15 contacts across 3 topics, 3 email templates, a sent campaign with stats, an active automation, one verified sending domain). Best for working on existing features. A local `dev` deployment (`OWLAT_DEV_MODE=true`) gets the fuller `/seed/demo` dataset on top — dummy teammate sign-ins and their Postbox mailboxes; any other install gets the same content through the removable sample-data path instead, because those sign-ins carry published password hashes.
