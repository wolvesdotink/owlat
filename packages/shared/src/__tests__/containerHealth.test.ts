...
describe('parseConfiguredVersionFromEnv', () => {
	it('reads OWLAT_VERSION out of a real .env', () => {
		const env = ['# Generated by Owlat setup.', 'OWLAT_VERSION=0.4.13', 'EMAIL_PROVIDER=mta'].join(
			'\n'
		);
		expect(parseConfiguredVersionFromEnv(env)).toBe('0.4.13');
	});

	it('strips surrounding quotes and whitespace', () => {
		expect(parseConfiguredVersionFromEnv('OWLAT_VERSION="0.4.13"')).toBe('0.4.13');
		expect(parseConfiguredVersionFromEnv("OWLAT_VERSION='0.4.13'")).toBe('0.4.13');
		expect(parseConfiguredVersionFromEnv('  OWLAT_VERSION = 0.4.13  ')).toBe('0.4.13');
	});

	it('reads a dev pin', () => {
		expect(parseConfiguredVersionFromEnv('OWLAT_VERSION=dev')).toBe('dev');
	});

	it('returns undefined when the key is absent or empty', () => {
		expect(parseConfiguredVersionFromEnv('FOO=bar')).toBeUndefined();
		expect(parseConfiguredVersionFromEnv('')).toBeUndefined();
		expect(parseConfiguredVersionFromEnv('OWLAT_VERSION=')).toBeUndefined();
		expect(parseConfiguredVersionFromEnv('OWLAT_VERSION=""')).toBeUndefined();
	});

	it('does not match a different key that merely ends in OWLAT_VERSION', () => {
		expect(parseConfiguredVersionFromEnv('PREV_OWLAT_VERSION=0.4.12')).toBeUndefined();
	});

	it('refuses values outside a docker tag charset rather than echoing them back', () => {
		expect(parseConfiguredVersionFromEnv('OWLAT_VERSION=0.4.13; rm -rf /')).toBeUndefined();
		expect(parseConfiguredVersionFromEnv('OWLAT_VERSION=$(whoami)')).toBeUndefined();
		expect(parseConfiguredVersionFromEnv(`OWLAT_VERSION=${'v'.repeat(200)}`)).toBeUndefined();
	});
});
