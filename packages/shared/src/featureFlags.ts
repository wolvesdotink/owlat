...
export const FEATURE_FLAGS: Record<CoreFeatureFlagKey, CoreFeatureFlagDefinition> = {
	...
	ostr: {
		...
		description:
			'Consult the Open Sender Trust Registry for an inbound sender\'s public standing, show its tier beside the sender badge, and file mail from a flagged sender as spam. The tier is a signal, not a verdict: it is recorded on every message either way, and nothing below "flagged" changes where mail lands. The tier chip lives inside the sender-authenticity badge, so it renders only while that feature is on as well; recording the tier and filing flagged mail as spam do not depend on it.',
		...
		//
		// `senderAuthBadges` is NOT listed, deliberately. The chip is rendered by
		// `PostboxAuthBadge`, whose root is gated on that flag, so with badges off
		// the chip half is silent — but the routing half (a `flagged` sender's
		// mail filed as spam) still works, and requiring the badge flag here would
		// switch that off too. The dependency is stated in the description above
		// and in the trust-registry docs instead of being enforced by disabling a
		// half that functions.
		requires: ['postbox'],
	},
