...
- Entries must be safe, exact npm package names installed directly in the root
  `dependencies` or `optionalDependencies`.
- Installed packages are imported only during build-time composition. Their default
  exports are validated with `@owlat/plugin-kit` and ordered by manifest id through
  `@owlat/plugin-host`.
...
- Core source may not import configured plugin packages outside those two generated
  files. `--boundaries-only` enforces that rule without importing plugin code.
