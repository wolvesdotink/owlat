...
describe('hosted contribution ordering', () => {
	it('orders by plugin then contribution identity without mutating input', () => {
		...
		const input = [
			{ pluginId: 'zeta', contributionId: 'first', value: { marker: 1 } },
			{ pluginId: 'alpha', contributionId: 'second', value: { marker: 2 } },
			{ pluginId: 'alpha', contributionId: 'first', value: alphaFirstValue },
		];
	...
	it('uses code-point ordering rather than locale-dependent comparison', () => {
		const ordered = orderHostedContributions([
			{ pluginId: 'alpha', contributionId: 'z', value: 1 },
			{ pluginId: 'alpha', contributionId: 'Z', value: 2 },
		]);
	...
	it('rejects a duplicate identity even when the values differ', () => {
		expect(() =>
			orderHostedContributions([
				{ pluginId: 'alpha', contributionId: 'same', value: 1 },
				{ pluginId: 'alpha', contributionId: 'same', value: 2 },
			])
		).toThrowError(
	...
	it('allows the same local identity in different plugin namespaces', () => {
		expect(
			orderHostedContributions([
				{ pluginId: 'alpha', contributionId: 'shared', value: 1 },
				{ pluginId: 'beta', contributionId: 'shared', value: 2 },
			])
		).toHaveLength(2);
	});
