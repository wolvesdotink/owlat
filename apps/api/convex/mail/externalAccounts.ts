...
/**
 * Outbound transport decision for a mailbox doc — the single source of truth
 * for "where does this mailbox's mail actually ship?". Returns `{kind:'hosted'}`
 * for Owlat-hosted mailboxes (MTA path) or `{kind:'external', smtp…}` (mail-sync
