...
/**
 * Scheduling-focused framing for {@link suggestReplies}. Fixed instruction text
 * — never user-supplied — with the sender's proposed times passed as bounded
 * untrusted DATA, not instructions. Pure + exported so the unit test can assert
 * the framing without a live model.
 */
...
export const suggestReplies = authedAction({
	args: {
		...
		// Optional scheduling-focused framing (the reader's meeting-intent chip).
		focus: v.optional(v.literal('scheduling')),
		// Verbatim proposed-time phrases from the inbound message (untrusted data).
