...
describe('mailbox usedBytes across IMAP COPY', () => {
	...
	it('the 0042 migration recomputes historical counters with per-row QUOTA semantics', async () => {
		const t = convexTest(schema, modules);
		const mailboxId = await seedMailbox(t);
		const inboxId = await seedFolder(t, mailboxId, 'inbox');
		const archiveId = await seedFolder(t, mailboxId, 'archive');
		const messageId = await seedMessage(t, mailboxId, { subject: 'historical copy' });
