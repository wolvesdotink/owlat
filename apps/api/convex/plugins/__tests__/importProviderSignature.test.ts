/**
 * The ORIGIN-ONLY verifier, which gates nothing yet.
 *
 * Its own module (and its own suite) so `lint:convex-orphans` can keep saying
 * so: the replay-bound verifier beside it is wired to a live route, and a
 * module-granular gate cannot tell the two apart when they share a file.
 */
...
import { verifyPluginInboundSignature } from '../importProviderSignature';
...
describe('plugin inbound signature verification', () => {
	...
	it('answers 503 before it looks at the signature at all', async () => {
		// Configuration is the deployment's problem, not the caller's: an operator
		// still wiring the endpoint up must not be told 401 about it.
		const result = await verifyPluginInboundSignature(contract('hmac-sha256', 'hex'), BODY, null);
		expect(result).toMatchObject({ ok: false, status: 503 });
	});
