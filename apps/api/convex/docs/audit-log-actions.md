...
| `deliverability_ramp.cell_evaluation_failed` | `deliverability_ramp` | `{ cell, error }` — one cell threw while the hourly ramp tick was evaluating it; the tick recorded this and continued with the remaining cells. `error` is the thrown message, truncated |
