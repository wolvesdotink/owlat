 ...
 * recipient lands in — which is exactly the move the evidence gate exists to guard. Letting this
 * mutation do it too — on nothing but the hard-stop check, with no promotion evidence and no typed
 * confirmation — would make the gate optional, and an optional gate is not a gate. The upward move
 * lives in `rampPhasePromotion.promoteCellPhase` and nowhere else.
 */
