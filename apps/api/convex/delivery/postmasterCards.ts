...
function complianceCards(signals: PostmasterDomainSignals): PostmasterCard[] {
	const cards: PostmasterCard[] = [];
...
function metricCards(signals: PostmasterDomainSignals): PostmasterCard[] {
	const cards: PostmasterCard[] = [];
	const spam = signals.userReportedSpamRatio;
	if (spam !== null && spam >= POSTMASTER_SPAM_RATE_LIMIT) {
		const copy = CHECK_COPY['SPAM_RATE'];
		if (copy) {
			cards.push({
				id: 'metric:SPAM_RATE',
				severity: copy.severity,
...
export function derivePostmasterCards(signals: PostmasterDomainSignals): PostmasterCard[] {
	const byCheck = new Map<string, PostmasterCard>();
	// Compliance Status first: it is Google's own verdict, so it wins over our
	// threshold on the same underlying check.
	for (const card of [...complianceCards(signals), ...metricCards(signals)]) {
		if (!byCheck.has(card.check)) byCheck.set(card.check, card);
	}
