...
export function registerDeliveryCrons(crons: Crons): void {
	...
	// The same job for the generic relay-identity table, and HOURLY rather
	...
	// THE AIMD RAMP CONTROLLER. Convex owns the decision — it has the
	...
	// returns immediately, writes nothing and raises nothing.
	crons.interval(
	...
	// Re-verify dual-transport alignment: the two arms of a ramp cell must
	...
	//
	// With no seed mailboxes connected it is a permanent no-op.
	crons.interval(
