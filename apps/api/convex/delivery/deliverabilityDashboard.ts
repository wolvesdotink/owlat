	...
	handler: async (ctx): Promise<DeliverabilityDashboard> => {
		...
		for (const cell of allDeliverabilityCells()) {
			...
			const evaluation = evaluator.evaluate({
				...
				// span on the CLOCK rather than on `readWindow.since`, which is a day
				// newer than the controller's own bound — a screen judging the
				// instrument over its own narrower span is how the two come to differ
