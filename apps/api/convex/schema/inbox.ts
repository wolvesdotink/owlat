...
export const inboxTables = {
	...
	// Natural-language handling rules — standing intent the user teaches in
	// prose ("draft a polite decline for recruiters", "flag anything from legal
	// for me"). A cheap LLM compiles the trusted, user-authored `naturalLanguage`
	// into a deterministic `matcher` (AND-ed conditions over the untrusted
	// inbound email) + an `action`. Executed deterministically at classify time
	// (categorize / auto-archive) and at the auto-send gate (a rule can only ever
	// RESTRICT auto-send — never widen it). Inspectable + editable + revocable in
	// settings. A row is only consulted while `status: 'active'` AND `isEnabled`,
	// so a still-compiling or failed compile is inert and never touches the
	// pipeline (fail-soft).
