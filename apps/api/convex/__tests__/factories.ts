...
/**
 * A curated `campaignSenders` row. Enabled + default by default, so a suite that
 * seeds one for its campaign's `fromEmail` satisfies the send-time curated-sender
 * gate (2026-07-10 plan, decision 8) without turning on the custom-senders toggle.
 * Pass `email` to match the campaign under test.
 */
export function createTestCampaignSender(overrides: Record<string, unknown> = {}) {
	const now = Date.now();
...
export function createTestWebhook(overrides: Record<string, unknown> = {}) {
	...
	return {
		...
		events: ['contact.created', 'email.sent'] as (
			| 'email.sent'
			| 'email.delivered'
			| 'email.opened'
			| 'email.clicked'
