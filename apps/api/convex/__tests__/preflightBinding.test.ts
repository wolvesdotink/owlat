...
describe('pre-flight capacity gate — segment audiences', () => {
	...
	it('allows the send when the audience scan exhausts its read budget', { timeout: 120_000 }, async () => {
		const t = convexTest(schema, modules);
		await seedWarmingState(t);
...
describe('pre-flight capacity gate — audiences past the read budget', () => {
	...
	it('allows a small audience hidden behind a read-budget-exhausting noise floor', { timeout: 120_000 }, async () => {
		const t = convexTest(schema, modules);
		await seedWarmingState(t);
		const campaignId = await seedSegmentCampaign(t, {
			matching: 40,
