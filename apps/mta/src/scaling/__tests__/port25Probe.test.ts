...
describe('classifyPort25', () => {
	...
	it('reports refusals as unknown, never as clean and never as a block', () => {
		expect(
			classifyPort25([target('gmail.com', 'refused'), target('yahoo.com', 'refused')])
		).toEqual({ status: 'unknown', reason: 'inconclusive' });
		// A refusal is an ANSWER: the packet reached the network, so silence is not
		// consistent and this is not a provider block.
		expect(
			classifyPort25([
				target('gmail.com', 'timeout'),
				target('outlook.com', 'timeout'),
				target('yahoo.com', 'refused'),
			])
		).toEqual({ status: 'unknown', reason: 'inconclusive' });
	});

	it('still calls a block when one target could not even resolve its MX', () => {
		// A DNS hiccup on one operator says nothing about egress, so it must not
		// downgrade consistent silence across the two that DID answer.
		expect(
			classifyPort25([
				target('gmail.com', 'timeout'),
				target('outlook.com', 'timeout'),
				target('yahoo.com', 'resolution_error'),
			])
		).toEqual({ status: 'blocked', reason: 'all_targets_timed_out' });
	});

	it('will not call a block on one timeout beside one unusable target', () => {
		expect(
			classifyPort25([target('gmail.com', 'timeout'), target('yahoo.com', 'resolution_error')])
		).toEqual({ status: 'unknown', reason: 'insufficient_targets' });
	});
...
describe('probePort25Egress', () => {
	...
	it('keeps the answers of operators that replied when another one hangs', async () => {
		const result = await probePort25Egress('203.0.113.10', {
			now: () => 1_000,
			reachability: reachability({ 'yahoo.com': 'hang' }),
			domains: DOMAINS,
			deadlineMs: 25,
		});
		// Gmail and Outlook connected: egress is demonstrably open, whatever Yahoo
		// is doing. Discarding the settled results would report `unknown` here.
		expect(result.status).toBe('open');
		expect(result.reason).toBe('connected');
		expect(result.targets.map((entry) => entry.domain)).toEqual(['gmail.com', 'outlook.com']);
	});

	it('reports a deadline in the reason when the answers that did arrive are inconclusive', async () => {
		const result = await probePort25Egress('203.0.113.10', {
			now: () => 1_000,
			reachability: reachability({ 'gmail.com': 'refused', 'outlook.com': 'hang' }),
			domains: ['gmail.com', 'outlook.com'],
			deadlineMs: 25,
		});
		expect(result.status).toBe('unknown');
		expect(result.reason).toBe('probe_deadline_exceeded');
		expect(result.targets.map((entry) => entry.domain)).toEqual(['gmail.com']);
	});
