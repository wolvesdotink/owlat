...
/**
 * Guards for the marketing message catalogs (apps/marketing/i18n/locales).
 * Modelled on apps/web/app/__tests__/localeCatalogs.test.ts — the two apps ship
 * the same kind of catalog, so they get the same guarantees.
 *
 * `en` is the source of truth and the `fallbackLocale`, so a translation that
 * drifts fails quietly at runtime — a visitor on /de/ just gets an English line
 * in the middle of a German page. These checks turn every way that drift
 * happens into a test failure instead:
 ...
 *    or — for the `<I18nT>` headlines — silently drops a slot;
 ...
 *    (the mock inbox address has to be written as `mia{'@'}northwind.example`).
 */
...
describe('marketing message catalogs', () => {
	...
	// `{placeholder}` is a visible defect on the page a stranger lands on first.
