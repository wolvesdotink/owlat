...
export default defineEventHandler(async (event): Promise<{ ok: boolean; message?: string; redirectTo?: string }> => {
	...
	const hasRealProvider = chosenProvider === 'mta' || chosenProvider === 'resend' || chosenProvider === 'ses';
	...
	const profiles = getActiveProfiles(body.flags, { deliveryProvider: body.env?.['EMAIL_PROVIDER'] });
	...
	try {
		const seedRes = await fetch(`${convexSiteUrl}/seed/admin`, {
			...
			body: JSON.stringify({ email: body.admin.email, name: body.admin.name, passwordHash, flags: resolved }),
		});
	...
	if (seedStatus !== 201 && seedStatus !== 409) {
		return { ok: false, message: `Admin account creation failed: ${seedError ?? `status ${seedStatus}`}.` };
	}
