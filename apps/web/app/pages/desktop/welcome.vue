...
const { workspaces, activeId, addWorkspace, switchTo, removeWorkspace } = useDesktopWorkspaces();
...
<template>
	...
	>
		...
		<div v-if="!isDesktop" class="w-full max-w-md rounded-2xl border border-border-default bg-bg-surface p-8 text-sm text-text-secondary">
		...
		<div v-else-if="view === 'welcome'" class="w-full max-w-md text-center">
			...
			>
				<span class="flex size-10 shrink-0 items-center justify-center rounded-lg bg-brand text-white">
					<Icon name="lucide:server" class="size-5" />
				</span>
				<span class="min-w-0 flex-1">
					<span class="block text-sm font-semibold">Set up a new server</span>
					<span class="mt-0.5 block text-xs text-text-secondary">Install Owlat on a fresh Linux server over SSH.</span>
				</span>
