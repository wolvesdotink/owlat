...
const themeOptions: { value: ThemeOption; label: string; icon: string; description: string }[] =
	[
		{ value: 'light', label: 'Light', icon: 'lucide:sun', description: 'Light background with dark text' },
		{ value: 'dark', label: 'Dark', icon: 'lucide:moon', description: 'Dark background with light text' },
		{ value: 'system', label: 'System', icon: 'lucide:monitor', description: 'Match your device settings' },
	];
...
const handleSave = async () => {
	...
	if (form.archiveEnabled !== archiveFlag) {
		if ((await setFeatureFlag({ flag: 'campaigns.archive', value: form.archiveEnabled })) === undefined) {
			isSaving.value = false;
			return;
		}
	}
...
const { data: isPlatformAdmin } = useConvexQuery(api.platformAdmin.platformAdmin.isPlatformAdmin, () => ({}));
...
const settingsSections = computed(() => {
	...
	if (isPlatformAdmin.value === true) {
		sections.push({
			...
			description: 'Review held content, manage organization sending status, and curate platform admins',
...
<template>
	<div class="p-6 lg:p-8">
		...
		<UiQueryBoundary :loading="isLoading && !organizationSettings" :error="organizationSettingsError">
			<template #loading>
				...
				<form class="p-6" @submit.prevent="handleSave">
					<div class="grid gap-6 max-w-2xl">
						...
						<p class="-mt-4 text-xs text-text-tertiary">
							Fallback timezone for send-time-optimized campaigns when a recipient's own
							timezone is unknown.
						</p>
						...
						<div class="flex items-center justify-between py-2">
							<div>
								<p class="text-sm font-medium text-text-primary">Enable campaign archives by default</p>
			...
			<UiCard padding="none" overflow="hidden">
				...
				<div class="p-6">
					<div class="max-w-2xl">
						...
						<p class="text-xs text-text-tertiary mb-4">
							Select your preferred color scheme. Choose "System" to automatically match your device
							settings.
						</p>
