...
/**
 * Warm off-white ink used as the readable foreground on every saturated avatar
 * fill (and, by extension, on the workspace-rail tiles that reuse this palette).
...
export const AVATAR_COLOR_STYLES: Record<AvatarColorToken, { background: string; color: string }> =
	{
		clay: { background: '#8a4630', color: AVATAR_INK },
		rust: { background: '#93412c', color: AVATAR_INK },
		gold: { background: '#7a5a1e', color: AVATAR_INK },
		moss: { background: '#4e6337', color: AVATAR_INK },
		sage: { background: '#3f6b4f', color: AVATAR_INK },
		teal: { background: '#2f6b66', color: AVATAR_INK },
		slate: { background: '#3f6480', color: AVATAR_INK },
		indigo: { background: '#4d5a8c', color: AVATAR_INK },
		plum: { background: '#7a4a6d', color: AVATAR_INK },
		mauve: { background: '#8c4a52', color: AVATAR_INK },
	};
...
function identityInitials(nameOrEmail: string): string {
	...
	const source =
		trimmed.includes('@') && !trimmed.includes(' ') ? (trimmed.split('@')[0] ?? trimmed) : trimmed;
