	...
	<div class="rounded-xl border border-border-default bg-bg-elevated p-6">
		<div class="flex items-baseline justify-between gap-4 flex-wrap mb-4">
			<h3 class="font-semibold text-text-primary">
				{{ t('components.system.llmSpendCard.title') }}
			</h3>
			<p class="text-2xl font-medium tracking-[-0.02em] text-text-primary">
				{{ formatUsd(llmSpend?.totalCostUsd ?? 0) }}
			</p>
		</div>
			...
			>
				...
				<span class="text-text-primary font-medium">
					...
					<span class="text-text-tertiary font-normal">{{
						t('components.system.llmSpendCard.calls', { count: f.calls })
					}}</span>
				</span>
			</div>
		</div>
		<p v-else class="text-text-tertiary text-sm">
			{{ t('components.system.llmSpendCard.noUsage') }}
		</p>
		...
		>
			<p class="text-xs font-medium text-text-tertiary uppercase tracking-wider mb-2">
				{{ t('components.system.llmSpendCard.byProvider') }}
			</p>
			<div class="space-y-2">
				<div
				...
				>
					...
					<span class="text-text-primary font-medium">
						{{ formatUsd(p.costUsd) }}
						<span class="text-text-tertiary font-normal">{{
							t('components.system.llmSpendCard.calls', { count: p.calls })
						}}</span>
					</span>
				</div>
			</div>
		...
		<div v-if="spendBudget?.configured" class="mt-4 pt-4 border-t border-border-default space-y-2">
			<div class="flex items-baseline justify-between gap-2 flex-wrap">
				<h4 class="text-xs font-medium text-text-tertiary uppercase tracking-wider">
					{{ t('components.system.llmSpendCard.spendBudget') }}
				</h4>
				<span
				...
				>
					{{
						spendBudget.state === 'exceeded'
							? t('components.system.llmSpendCard.ceilingReached')
							: t('components.system.llmSpendCard.approachingCeiling')
					}}
				</span>
			</div>
			<div v-if="spendBudget.daily.configured" class="flex items-center justify-between text-sm">
				<span class="text-text-secondary">{{
					t('components.system.llmSpendCard.dailyRemaining')
				}}</span>
				<span class="text-text-primary font-medium">
					{{ formatUsd(spendBudget.daily.remainingUsd) }}
					<span class="text-text-tertiary font-normal">{{
						t('components.system.llmSpendCard.ofLimit', {
							amount: formatUsd(spendBudget.daily.limitUsd),
						})
					}}</span>
				</span>
			</div>
			<div v-if="spendBudget.monthly.configured" class="flex items-center justify-between text-sm">
				<span class="text-text-secondary">{{
					t('components.system.llmSpendCard.monthlyRemaining')
				}}</span>
				<span class="text-text-primary font-medium">
					{{ formatUsd(spendBudget.monthly.remainingUsd) }}
					<span class="text-text-tertiary font-normal">{{
						t('components.system.llmSpendCard.ofLimit', {
							amount: formatUsd(spendBudget.monthly.limitUsd),
						})
					}}</span>
				</span>
			</div>
