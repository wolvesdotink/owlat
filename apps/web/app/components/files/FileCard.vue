...
const mimeIcon = computed(() => {
	...
	if (mime.includes('spreadsheet') || mime.includes('csv') || mime.includes('excel')) return 'lucide:table';
	...
	if (mime.includes('zip') || mime.includes('tar') || mime.includes('compressed')) return 'lucide:file-archive';
...
const sourceLabel = computed(() => {
	switch (props.sourceType) {
		...
		case 'agent_generated': return 'AI';
		default: return props.sourceType;
	}
});
...
const sourceVariant = computed(() => {
	switch (props.sourceType) {
		...
		case 'email_attachment': return 'info';
		case 'agent_generated': return 'brand';
...
<template>
	<div
		class="group bg-bg-elevated border border-border-subtle rounded-lg overflow-hidden cursor-pointer transition-all duration-150 hover:border-border-default hover:shadow-sm"
		@click="navigate"
	>
		<!-- Icon area -->
		<div class="flex items-center justify-center py-8 bg-bg-surface">
			<Icon :name="mimeIcon" class="w-10 h-10 text-text-tertiary group-hover:text-text-secondary transition-colors" />
		</div>
			...
			<div class="flex items-center justify-between">
				<span
					...
					:class="{
						...
						'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300': sourceType === 'email_attachment',
						'bg-brand-subtle text-brand': sourceType === 'agent_generated',
					}"
				>
					<Icon
						:name="sourceType === 'upload' ? 'lucide:upload' : sourceType === 'email_attachment' ? 'lucide:mail' : 'lucide:sparkles'"
						class="w-3 h-3"
					/>
					{{ sourceLabel }}
				</span>
