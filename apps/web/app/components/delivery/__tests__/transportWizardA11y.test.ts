...
import { buttonByText, mountWizard, openWizard, type WizardWrapper } from './wizardHarness';
...
describe('transport wizard — accessibility', () => {
	...
	it('labels every credential input', async () => {
		const wrapper = mountWizard();
		await openWizard(wrapper);
		for (const input of wrapper.findAll('input[type="password"], input[type="text"]')) {
