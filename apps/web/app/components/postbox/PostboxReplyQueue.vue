...
// Draft-on-arrival review slots the user has dismissed this session (keyed by
// thread). The slot is advisory; dismissing hides just the draft, not the row.
const dismissedSlots = ref<Set<string>>(new Set());
function dismissSlot(threadId: string) {
	dismissedSlots.value = new Set(dismissedSlots.value).add(threadId);
}

/**
 * "Review & send" on a draft-on-arrival slot: open the composer prefilled with
 * the pre-generated draft over the quoted original. HUMAN REVIEW ONLY — it opens
 * the composer, it never sends.
 */
async function reviewSlot(row: QueueRow, draft: string) {
	await openReplyComposer(row, draft);
}
			...
			<li v-for="(row, i) in needsYouRows" :key="row._id" class="group relative">
				...
				>
					...
					<div class="flex-1 min-w-0">
						...
						<!-- Draft-on-arrival review slot (postbox.aiDraft): a reply
							pre-generated by the shared draft service. Human review only. -->
						<PostboxReviewSlot
							v-if="row.kind !== 'followup' && row.draftSlot && !dismissedSlots.has(row.threadId)"
							:draft-slot="row.draftSlot"
							@review="(draft) => reviewSlot(row, draft)"
							@dismiss="dismissSlot(row.threadId)"
						/>
					</div>
				</div>
				<!-- Hover / focus actions -->
