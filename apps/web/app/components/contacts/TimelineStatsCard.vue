...
const { stats, statsLoading, channelIcon, channelLabel, channelColor } =
	...
	<div class="card">
		<div class="flex items-center gap-3 mb-4">
			...
			<h2 class="text-lg font-medium text-text-primary">
				{{ t('components.contacts.timelineStatsCard.title') }}
			</h2>
		</div>
		...
		<template v-else-if="stats">
			...
			<div class="grid grid-cols-2 gap-3 mb-4">
				<div class="p-3 rounded-lg bg-bg-surface">
					...
					<p class="text-xs text-text-tertiary">
						{{ t('components.contacts.timelineStatsCard.messages') }}
					</p>
				</div>
				<div class="p-3 rounded-lg bg-bg-surface">
					<p class="text-lg font-semibold text-text-primary">{{ stats.totalThreads ?? 0 }}</p>
					<p class="text-xs text-text-tertiary">
						{{ t('components.contacts.timelineStatsCard.threads') }}
					</p>
				</div>
			</div>
			...
			<div v-if="stats.channelCounts" class="space-y-2 mb-4">
				<p class="text-xs font-medium text-text-tertiary uppercase tracking-wider">
					{{ t('components.contacts.timelineStatsCard.byChannel') }}
				</p>
				<div
				...
				>
					<div class="flex items-center gap-2">
						<Icon
							:name="channelIcon(String(channel))"
							class="w-4 h-4"
							:class="channelColor(String(channel))"
						/>
						<span class="text-sm text-text-secondary">{{ channelName(String(channel)) }}</span>
					</div>
					<span class="text-sm font-medium text-text-primary">{{
						t('components.contacts.timelineStatsCard.channelCount', {
							received: count.inbound,
							sent: count.outbound,
						})
					}}</span>
				</div>
			...
			<div class="space-y-2 pt-3 border-t border-border-subtle">
				<div v-if="stats.firstInteraction" class="flex items-center justify-between">
					<span class="text-xs text-text-tertiary">{{
						t('components.contacts.timelineStatsCard.firstContact')
					}}</span>
				...
				<div v-if="stats.lastInteraction" class="flex items-center justify-between">
					<span class="text-xs text-text-tertiary">{{
						t('components.contacts.timelineStatsCard.lastContact')
					}}</span>
		...
		<div v-else class="text-center py-4">
			<p class="text-text-tertiary text-sm">
				{{ t('components.contacts.timelineStatsCard.empty') }}
			</p>
		</div>
	</div>
</template>
