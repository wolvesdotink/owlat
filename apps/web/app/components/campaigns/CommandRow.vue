...
const metaLine = computed(() => {
	...
	if (row.reason === 'ab_decision') {
		...
		if (a != null && b != null) {
			...
			if (diff >= 0.1)
				return t(`${meta}.variantLeads`, { variant: leader, points: diff.toFixed(1) });
	...
	// The status chip already names these states; the meta line only adds
	// detail it does not carry.
	if (c.status === 'sending' || c.status === 'cancelled') return '';
	if (c.status === 'sent') {
	...
	return t(`${meta}.updated`, { updated: formatCompactRelativeTime(c.updatedAt) });
});

/** Unsent campaigns have no rates yet, so the metric columns stay blank. */
const hasMetrics = computed(() => props.row.openRate != null || props.row.clickRate != null);
</script>
	...
	<UiContextMenu :items="contextItems">
		<template #default="{ onContextmenu, onKeydown }">
			<li
				...
				tabindex="0"
				role="link"
				:aria-label="t('components.campaigns.commandRow.openCampaign', { name: row.campaign.name })"
				@click="emit('open')"
			...
			>
				<div class="min-w-0 flex-1">
					<!-- Phones: the name wraps and the badge flows inline after it;
					     wider screens keep one truncated line. -->
					<div class="min-w-0 sm:flex sm:items-center sm:gap-2">
						<span
							:class="[
								'min-w-0 sm:truncate text-text-primary',
						...
						<span
							...
							class="inline-flex items-center gap-1 ml-1.5 sm:ml-0 px-1.5 py-0.5 rounded text-xs font-medium text-text-secondary bg-bg-elevated shrink-0 align-middle"
					...
					<div class="flex items-center gap-2 mt-1 min-w-0">
						...
						<span v-if="metaLine" class="text-xs text-text-tertiary truncate">{{ metaLine }}</span>
					</div>
				</div>

				<!-- Sparkline (A/B variant open-rate trend; hidden otherwise). The wrapper
			 owns the breakpoint: the svg's own inline-block would override `hidden`. -->
				<div v-if="row.spark.length >= 2" class="hidden md:block shrink-0">
					<UiSparkline
						:data="row.spark"
						:ariaLabel="
				...
				<!-- Kept in layout (invisible) when there are no rates, so actions stay aligned -->
				<div :class="['hidden sm:flex items-center gap-6 shrink-0', hasMetrics ? '' : 'invisible']">
