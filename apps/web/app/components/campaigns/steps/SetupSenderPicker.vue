...
const domainVerificationWarning = computed(() => {
	...
	if (!status.exists) {
		return t('components.campaigns.steps.setupSenderPicker.domainNotRegistered', {
			domain: status.domain,
		});
	}
	if (!status.verified) {
		return t('components.campaigns.steps.setupSenderPicker.domainNotVerified', {
			domain: status.domain,
		});
	}
	if (status.stale) {
		return t('components.campaigns.steps.setupSenderPicker.domainStale', {
			lastChecked: status.lastVerifiedAt
				? new Intl.DateTimeFormat(locale.value).format(new Date(status.lastVerifiedAt))
				: t('components.campaigns.steps.setupSenderPicker.domainNeverChecked'),
		});
	}
	return null;
});
...
function validate(): string | null {
	...
	if (problem === 'none-selected') {
		senderError.value = t('components.campaigns.steps.setupSenderPicker.errors.noneSelected');
		return senderError.value;
	}
	if (problem === 'missing-name') {
		senderError.value = t('components.campaigns.steps.setupSenderPicker.errors.missingName');
		senderErrorField.value = 'name';
		return senderError.value;
	}
	if (problem === 'invalid-email') {
		senderError.value = t('components.campaigns.steps.setupSenderPicker.errors.invalidEmail');
	...
	// `detail` is the shared sender-alignment helper's message KEY, so it goes
	// through t() here exactly as the chip renders it.
	const auth = selectedSenderAuth.value;
	if (auth?.blocked) {
		senderError.value = auth.detail
			? t(auth.detail)
			: t('components.campaigns.steps.setupSenderPicker.errors.senderBlocked');
		return senderError.value;
	}
	return null;
}
...
<template>
	<div>
		<label for="senderPicker" class="label flex items-center gap-2">
			...
			{{ t('components.campaigns.steps.setupSenderPicker.sendFrom') }}
			<span class="text-error">*</span>
		</label>

		<!-- Loading -->
		<p v-if="senderPickerLoading && !senderPicker" class="mt-1.5 text-sm text-text-tertiary">
			{{ t('components.campaigns.steps.setupSenderPicker.loading') }}
		</p>
		...
		<template v-else>
			...
			<p v-if="!isCustomSelected && !senderError" class="mt-1.5 text-sm text-text-tertiary">
				{{ t('components.campaigns.steps.setupSenderPicker.curatedHint') }}
			</p>
			...
			<div v-if="isCustomSelected" class="mt-4 space-y-4">
				<div>
					<label for="fromName" class="label flex items-center gap-2">
						<Icon name="lucide:user" class="w-4 h-4 text-text-tertiary" />
						{{ t('components.campaigns.steps.setupSenderPicker.fromNameLabel') }}
						<span class="text-error">*</span>
					</label>
					<input
						id="fromName"
					...
					<p class="mt-1.5 text-sm text-text-tertiary">
						{{ t('components.campaigns.steps.setupSenderPicker.fromNameHint') }}
					</p>
				</div>
				...
				<div>
					<label for="fromEmail" class="label flex items-center gap-2">
						...
						{{ t('components.campaigns.steps.setupSenderPicker.fromEmailLabel') }}
						<span class="text-error">*</span>
					</label>
					<input
						id="fromEmail"
					...
					>
						...
						{{
							t('components.campaigns.steps.setupSenderPicker.domainVerified', {
								domain: domainVerificationStatus.domain,
							})
						}}
					</p>
					<p v-else class="mt-1.5 text-sm text-text-tertiary">
						{{ t('components.campaigns.steps.setupSenderPicker.fromEmailHint') }}
					</p>
				</div>
			</div>
		</template>
