 ...
 * every row has a copy button. The copy affordance swaps icon in place, so
 * the feedback causes no layout shift.
 *
...
<template>
	<div class="grid grid-cols-[auto_auto_1fr_auto] items-center gap-x-3 gap-y-1.5 font-mono text-xs">
		<template v-for="r in records" :key="keyOf(r)">
			...
			<span class="select-none rounded bg-bg-surface px-1.5 py-0.5 text-[10px] font-semibold uppercase text-text-secondary">
				{{ r.type }}
			</span>
			<span class="select-all whitespace-nowrap" :class="r.placeholder ? 'italic text-amber-300' : 'text-text-secondary'">{{ r.value }}</span>
			<button
				v-if="!r.placeholder"
				type="button"
				...
				:title="isCopied(keyOf(r)) ? 'Copied' : `Copy ${r.name}`"
				@click="copy(r.name, keyOf(r))"
			>
				<Icon :name="isCopied(keyOf(r)) ? 'lucide:check' : 'lucide:copy'" class="size-3.5" :class="isCopied(keyOf(r)) ? 'text-emerald-400' : ''" />
			</button>
			<span
				v-else
				class="flex size-6 items-center justify-center justify-self-end text-text-tertiary"
				:title="`Set the server's public IP to copy ${r.name}`"
			>
				<Icon name="lucide:copy" class="size-3.5 opacity-30" />
			</span>
			<p v-if="r.note" class="col-span-4 -mt-0.5 font-sans text-[11px] leading-snug text-text-tertiary">{{ r.note }}</p>
		</template>
	</div>
</template>
