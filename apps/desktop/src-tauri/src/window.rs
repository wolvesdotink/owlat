...
///   resize events (registered below), and visibility toggles (Electron's
///   macOS 26 note: toggling the buttons' hidden state re-layouts the titlebar
///   — see `apply_traffic_lights`). All calls are idempotent no-ops when the
///   frames are already right.
#[cfg(target_os = "macos")]
pub fn setup_traffic_lights(window: &WebviewWindow) {
    ...
    // Safety net for AppKit re-layouts the container observer can't see
    // (e.g. the zoom button recreated by the window-tiling hover machinery).
...
/// Safety-net re-layout, called from tauri window events (Resized/Focused,
/// registered in `setup_traffic_lights`). Catches AppKit re-layouts that
/// change no container frame — most
...
#[cfg(target_os = "macos")]
fn relayout_traffic_lights(window: &WebviewWindow) {
