...
Everything executable lives at a `module.exportPath` your package exports. Codegen verifies each path without running it; the host imports the module only when a send, a delivery or an identity check actually needs it.
...
1. resolved the route by plugin id (`POST /webhooks/plugin/<pluginId>`), answering 404 for an unknown one before a byte of the body is read;
2. applied a per-plugin rate limit and a body-size bound;
3. recomputed your declared HMAC over `<timestamp>.<rawBody>` in constant time and compared it against the header you named;
4. refused a timestamp further from now than your declared tolerance;
5. refused a delivery whose signature digest it has already accepted;
6. rechecked the operator's capability grant under its own audited operation.

What you declare, and what it costs to get wrong:
...
- **`replay` provisions are required too.** An HMAC over the body alone proves origin and nothing else — the same captured bytes verify forever. Binding a timestamp into the signed string and bounding its age is what makes a delivery happen once.
- **`toleranceSeconds` is bounded.** At most 900, and the host clamps it again at request time. Pick the smallest window your provider's retries tolerate.
- **`secretEnvVar` must be `PLUGIN_`-prefixed**, and it belongs in the plugin's `flag.requiredEnvVars`, not on the transport: without it the route can verify nothing and answers every delivery 503, so the plugin must not be enableable without it.
- **One webhook per plugin.** The route surface is keyed by plugin id, so a second declaration could never be addressed. A malformed second one counts too.
- **`storeRawPayload` defaults to false.** A third party's payload may carry recipient content this deployment never asked to keep, so retention is your explicit decision rather than the pipeline's default.

Inside the module: return the empty array for a console verification ping and for event kinds Owlat does not consume — a 400 would make your provider redeliver them forever — and **throw** on a body you cannot read. Import no Node builtins: the module is loaded by the HTTP router.
...
`owlat plugins create <id> --template send-provider` emits:
