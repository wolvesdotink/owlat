...
| 4 | Webhook adapter | `webhooks/adapters/<kind>.ts` implementing `InboundAdapter` + an `http.ts` route, declared back on the entry as `providerFeedback` (path, signing key, and the setup panel its ceremony gets — the delivery config page renders from that, not from the kind). **The `setupPanel` is not drawn yet for a new kind**: each panel still reports numbers from a per-kind backend read (`getMandrillFeedbackStatus`'s key presence, `getLastSesEventAt`'s timestamp), so `apps/web/app/utils/providerFeedbackPanel.ts` holds an answering set per panel and a kind outside it gets no panel rather than another provider's figures under its name. Generalising those reads — and with them the set — is the webhook-registry work | iff `hasProviderFeedback` |
