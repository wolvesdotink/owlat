...
**Supported kinds:** `mta`, `ses`, `resend`, `smtp`, `mandrill` — in the catalog's own declaration order, which `SEND_TRANSPORT_KINDS`, `DELIVERY_PROVIDER_KINDS` and the tables derived from them follow. The relay picker in the setup wizard is the one deliberate exception: `TRANSPORT_PICKER_COPY` (`useRelayCredentialDraft.ts`) leads with the four incumbents in the order the shipped screens have always shown them (own MTA, SES, SMTP relay, Resend) and appends everything else in catalog order, because re-sorting a shipped form is a user-visible change and adding a provider must not be one.
...
| `optionalEnvVars` | Variables it reads but does not need — refinements a deployment may never set | TLS floor, webhook secret | — | webhook secret | port/TLS | webhook key, subaccount, IP pool |
