...
| 7 | UI wording overrides | `apps/web/app/utils/transportState.ts` (`TRANSPORT_LABEL_OVERRIDE`, `TRANSPORT_DESCRIPTION`), `apps/web/app/utils/transportDnsGuidance.ts` (`GUIDANCE`), `useRelayCredentialDraft.ts` (`TRANSPORT_PICKER_COPY`) | **optional** — every one of these is a `Partial` override over a catalog-derived fallback, so a kind with no row renders with the entry's own label, a capability-derived DNS paragraph and a neutral picker icon | **No equivalent step at this tier**, and no way to take one: `TRANSPORT_LABEL_OVERRIDE`, `TRANSPORT_DESCRIPTION` and `GUIDANCE` are `Partial<Record<DeliveryProviderKind, …>>` — keyed by *core* kind — and each is consulted only for a kind the union contains, so a namespaced plugin kind can hold no row in them (nor in `TRANSPORT_PICKER_COPY`, whose order is filtered through `SEND_TRANSPORT_KINDS` — the core union again). What renders instead is the composed entry's own `label`, which reaches the transport card through the plugin-aware `providerLabel` on `delivery/status.ts`. The DNS card is the honest exception, and the one thing this tier is still owed: `transportDnsGuidance` resolves its entry through `coreSendProviderCatalogEntry`, core-only by construction, so for a namespaced kind it returns `null` and the card renders **nothing** — not the capability-derived paragraph — until a composed-catalog view lands in `apps/web`. That is the same missing view `apps/web/app/composables/__tests__/pluginTransportCredentialGap.test.ts` pins for the credentials form; closing it closes both |
...
**Step 7 is deliberately NOT enforced.** The three web tables used to be exhaustive `Record<DeliveryProviderKind, …>` maps, so a sixth kind was a compile error in `apps/web` — a file the provider's own bundle has no business touching, which is the opposite of "adding a provider is additive". They are `Partial` overrides now: a **core** kind with no row is rendered from its catalog entry (`transportKindLabel` falls back to `entry.label`, `transportDnsGuidance` to the `tier` / `domainVerification` paragraph, the picker to the entry's label and a neutral icon). Nothing breaks, and nothing is blank — so write a row when this surface should word your provider differently, not because the build made you. A namespaced plugin kind reaches those same fallbacks only where this app can resolve its entry, which today is the label and not the DNS card; step 7's plugin cell says exactly where that stops.
