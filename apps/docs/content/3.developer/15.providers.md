...
::callout{type="warning" title="`acceptanceSemantics: 'accepted'` has two prerequisites outside the catalog"}
It is **not yet a general capability** — only the own MTA declares it, and it is the one field where the wrong value is not cosmetic. Two sites still spell the custody arm as that one kind, and both must be generalized in the same change as a second kind declaring it: `delivery/sendLifecycle.bindMtaProviderIdentity` (it patches `providerType: 'mta'` onto the Send unconditionally) and `delivery/lastMileRouting.ts` (its `mtaReconciliation` pin defers every non-`mta` reconciliation attempt, so the replay would be deferred at 60s intervals until the delivery deadline killed the send). Declaring it also *requires* `messageIdSource: 'idempotency-key'` — the pairing is a compile-time union in `catalog.ts`, because a replay is only safe when it carries the id we minted. When in doubt, `unknown-on-timeout` / `provider` is the fail-closed pair, and it is also what an absent declaration means.
::
...
- `CoreSendProviderCatalogEntry` in `catalog.ts` is a union, not a bag of independent fields: it breaks the build on a core entry that omits `domainVerification` / `acceptanceSemantics` / `messageIdSource`, or that pairs `acceptanceSemantics: 'accepted'` with an id the provider mints — the pairing that would answer an ambiguous send with an idempotency key the provider never saw (step 3).
