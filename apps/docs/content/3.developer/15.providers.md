...
It is **not yet a general capability** — only the own MTA declares it, and it is the one field where the wrong value is not cosmetic. Two sites still spell the custody arm as that one kind and must be generalized in the same change as a second kind declaring it. **Read the PREREQUISITES note on `AcceptanceSemantics` in `apps/api/convex/lib/sendProviders/catalog.ts` before declaring it**: that note names both sites and what breaks if one is missed, and it is deliberately the only copy, so this page cannot go stale against it. Declaring it also *requires* `messageIdSource: 'idempotency-key'` (and vice versa) — the pairing is a compile-time union in `catalog.ts`, because a replay is only safe when it carries the id we minted. When in doubt, `unknown-on-timeout` / `provider` is the fail-closed pair, and it is also what an absent declaration means.
::
