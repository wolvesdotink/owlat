...
A bundled plugin transport gets step 4 through its own contribution rather than through this registry: a `sendTransports` entry may declare a `webhook` (a parse-only module export plus the signature contract the host verifies it with), and every plugin transport's feedback arrives on one route, `POST /webhooks/plugin/<pluginId>`, behind the hosted-contribution authorization seam. The tier difference is who verifies — the host does, always, and a webhook whose contract carries no verifier or no replay provisions fails manifest validation. See [Plugin Contributions → Feedback webhook](/developer/plugin-contributions#feedback-webhook).
