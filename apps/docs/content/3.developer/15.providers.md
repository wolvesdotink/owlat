...
Every catalog entry answers eleven questions about the provider, and the rest of the system asks the catalog rather than naming a kind:
...
| `optionalEnvVars` | Variables it reads but does not need — refinements a deployment may never set | TLS floor | — | webhook secret | port/TLS | webhook key, subaccount, IP pool |
