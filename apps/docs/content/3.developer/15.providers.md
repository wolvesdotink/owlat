...
- `CoreSendProviderCatalogEntry` in `catalog.ts` is a union, not a bag of independent fields: it breaks the build on a core entry that omits `domainVerification` / `acceptanceSemantics` / `messageIdSource`, or that splits the custody pair in either direction — `acceptanceSemantics: 'accepted'` with an id the provider mints (which would answer an ambiguous send with an idempotency key the provider never saw), or `messageIdSource: 'idempotency-key'` without the custody that makes pre-binding that id worth its prerequisites (step 3).
