...
| 4 | Webhook adapter | `webhooks/adapters/<kind>.ts` implementing `InboundAdapter` + an `http.ts` route, declared back on the entry as `providerFeedback` (path, signing key, and the setup panel its ceremony gets — the delivery config page renders from that, not from the kind) | iff `hasProviderFeedback` |
...
| 6 | UI label + docs | `apps/web/app/utils/transportState.ts` (`TRANSPORT_LABEL`, `TRANSPORT_DESCRIPTION`), `DomainDnsGuidance.vue`, `useRelayCredentialDraft.ts` (`RELAY_PROVIDER_OPTIONS`), this page | always |
...
- `CoreSendProviderCatalogEntry` — the core entry type, in `packages/shared/src/sendProviderCatalogTypes.ts` and re-exported from both catalog modules — is a union, not a bag of independent fields: it breaks the build on a core entry that omits `domainVerification` / `acceptanceSemantics` / `messageIdSource` / `deduplicatesOnIdempotencyKey`, or that splits the custody pair in either direction — `acceptanceSemantics: 'accepted'` with an id the provider mints (which would answer an ambiguous send with an idempotency key the provider never saw), or `messageIdSource: 'idempotency-key'` without the custody that makes pre-binding that id worth its prerequisites (step 1).
- The web maps are typed `Record<DeliveryProviderKind, …>`, so a new kind is a missing-key error rather than a silently blank label (step 6).
