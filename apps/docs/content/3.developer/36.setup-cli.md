...
Removal is exact rather than a table wipe: every row a loader inserts carries `seedTag: 'demo'`, removal deletes only tagged rows, and it re-checks the tag inside the delete transaction. A sample contact you edited is still tagged and still goes; a contact you created is untagged and stays. Because the tag has no index, removal scans page by page from an action rather than in one mutation, so it stays inside the per-transaction read limit on an instance that has since accumulated real data.
