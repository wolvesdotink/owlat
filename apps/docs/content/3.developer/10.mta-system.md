...
2. The send producers put that value on the durable send **envelope** at enqueue time — campaign audience resolution projects it from the contact it has already loaded, and `delivery/enqueue.ts` does a single indexed point read for automation / agent-reply sends. The dispatch action never reads a contact per recipient.
3. `delivery/governedDispatch.ts` stamps the envelope value onto `MtaExtras.engagementScore`, and the MTA adapter forwards it in the `POST /send` body.

**Absence is normal and is not an error.** A contact the scorer has not reached yet, a send with no contact record at all (transactional receipts, agent replies to an unknown address), and any envelope queued before the field existed all arrive with the field **omitted**. `mapToPriority` maps a missing score to `PRIORITY_BANDS.DEFAULT` (the LOW band), so such mail is ordered exactly as it was before any scoring existed — neither promoted nor pushed behind cold contacts.
