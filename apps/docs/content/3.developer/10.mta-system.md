...
Submission job identities are deterministic across an uncertain DATA retry. The identity hashes the authenticated principal, normalized envelope sender, ordered de-duplicated envelope recipients, and exact DATA bytes, then derives one job ID per recipient. A durable four-day intake receipt plus the explicit GroupMQ job ID lets a retry reconcile a committed enqueue and complete only missing recipients after partial fan-out. Consequently, an intentional byte-for-byte resend with the same principal and envelope during that window is treated as the same submission; change the message (normally its `Message-ID` header) to create a distinct send.
