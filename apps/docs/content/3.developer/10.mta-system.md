...
When the cap for the day is reached the attempt **defers to the next cap window** — the next UTC day boundary, where the daily budget actually resets — rather than to a fixed short retry (`intelligence/warmingCapWindow.ts`). The verdict cannot change before then, so a shorter retry would re-queue the whole deferred backlog every few minutes for the rest of the day to reach the same answer. Intraday **pacing** is different and keeps its own retry instant: a paced attempt genuinely does get capacity back within the day, on a curve.
...
2. **Two** of the send producers put that value on the durable send **envelope** at enqueue time, and the value is normalised there (in the enqueue mutation, and at the contact read itself on the non-campaign path) so nothing degenerate can enter a durable envelope:
   ...
   - automation and agent-reply sends — `delivery/enqueue.ts:enqueueNonCampaignSend` does a single indexed point read of the contact inside the existing enqueue transaction.
