...
A transient failure is retried up to three times with exponential backoff inside a bounded budget before the sweep concludes `unknown`, so a slow resolver does not cost a false `unknown`, and a dead one cannot stall the sweep.
