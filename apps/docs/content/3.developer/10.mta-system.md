...
| `FBL_DEDUP_PROTOCOL` | `legacy-shadow` when absent; fresh setup writes `owned-v2` | Complaint deduplication migration mode |
...
old worker merely claimed the complaint or finished it. An upgrade therefore
defaults to `legacy-shadow` when `FBL_DEDUP_PROTOCOL` is missing. This preserves
mixed-version `SET NX` exclusion and shadows successful processing into the
versioned owned state, but it deliberately retains the legacy retry limitation.
Do not describe a legacy `1` as a durable completion record.

Use this cutover sequence for an existing installation:

1. Deploy the new binary to every MTA replica in `legacy-shadow` mode. Do not
   enable `owned-v2` while any old binary or shadow-mode replica can accept FBL
   SMTP traffic.
2. After the final old binary is gone, remain in shadow mode for at least seven
   full days (the legacy key TTL). This drains every ambiguous key written by an
   old worker while new successful complaints acquire versioned terminal state.
3. Quiesce bounce/FBL SMTP intake and drain in-flight handlers. Set
   `FBL_DEDUP_PROTOCOL=owned-v2` on every replica in one coordinated restart,
   then resume intake. Mixed shadow/v2 intake during this switch is unsupported.
4. Do not roll an owned-v2 fleet back to binaries that know only the legacy key.

Owned-v2 never consults an ambiguous legacy `1`. It ACKs only a versioned
`completed` state; a failed owner releases versioned state to `retryable`, which
every active owned-v2 replica can reserve again. Fresh setup writes `owned-v2`
because it has no legacy workers or keys to drain.
