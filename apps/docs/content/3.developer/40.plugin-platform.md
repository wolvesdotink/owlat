...
| **2 — Connected app** | An external HTTPS service with plugin-bound API keys and webhooks, plus a signed synchronous hook protocol (implemented; **no pipeline call site yet** — see [Connected Apps](/developer/plugin-connected-apps)) | Register in the dashboard; instant, no rebuild | Untrusted network peer. Every response is strictly validated, scrubbed, clamped, and bounded |
