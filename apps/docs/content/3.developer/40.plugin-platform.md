...
| **2 — Connected app** | An external HTTPS service with plugin-bound API keys and webhooks, plus a specified signed synchronous hook protocol whose runtime adapter is not shipped — see [Connected Apps](/developer/plugin-connected-apps) | Register in the dashboard; instant, no rebuild | Untrusted network peer. Every response is strictly validated, scrubbed, clamped, and bounded |
