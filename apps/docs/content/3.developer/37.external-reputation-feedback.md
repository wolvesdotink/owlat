...
usable for slowing the ramp and can never speed it up. Every verdict derived
from an unattributed window says so in its reason and names the remedy, and a
spam-trap hit that is the *only* breaching evidence in such a window holds
rather than retreats: evidence that may be a neighbour's moves the ramp in
neither direction. Declaring your pool is
