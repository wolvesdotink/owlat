...
A `navItems` entry's `section` must be one of the **core sidebar section keys** — `inbox`, `postbox`, `chat`, `assistant`, `send`, `audience`, `delivery`, `knowledge`, `settings`. A plugin cannot create a section, and the host drops an entry naming an unknown one fail-closed, so a wrong key silently never renders.
