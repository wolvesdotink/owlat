...
description: "Operator guide to Sealed Mail's transport hardening: publishing MTA-STS, the OUTBOUND_TLS_MODE posture, enabling DANE, reading the TLS-RPT dashboard, and the secrets that protect mail at rest."
---
...
## Enabling DANE (`DANE_ENABLED`)

DANE (RFC 7672) authenticates a receiver's TLS certificate against a DNSSEC-signed TLSA record, closing the trust gap that plain WebPKI leaves. It's **off by default** (locked decision D6) and gated behind two env vars on the MTA:

| Env var | Purpose |
| --- | --- |
| `DANE_ENABLED` | `true` to attempt DANE at send time. Default `false`. |
| `DANE_RESOLVER_URL` | A validating DoH (DNS-over-HTTPS, RFC 8484) resolver endpoint. **Required** when `DANE_ENABLED=true`. |

DANE's entire security rests on the resolver's DNSSEC **AD (Authenticated Data) bit**, so the channel to the resolver must be one an on-path attacker can't forge over. The MTA enforces this at boot:

- `DANE_ENABLED=true` with no `DANE_RESOLVER_URL` → boot fails (DANE needs a validating resolver to be safe; booting without one would silently disable it).
