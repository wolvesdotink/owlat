...
description: "Operator guide to Sealed Mail's transport hardening: publishing MTA-STS, the OUTBOUND_TLS_MODE posture, the DANE_MODE posture (off/report/enforce), reading the TLS-RPT dashboard, and the secrets that protect mail at rest."
---
...
Everything here is optional and safe-by-default: an unconfigured deployment behaves exactly as it did before Sealed Mail shipped. You opt into stricter postures deliberately, one step at a time.
...
The default is `report`. Report-only honours locked decision **D6** ("DANE never bounces mail by default") because it has zero enforcement/delivery impact, while still giving you DANE visibility in the [TLS-RPT dashboard](#reading-the-tls-rpt-dashboard) out of the box. Promote a domain to `enforce` only once report-only shows its TLSA records validating cleanly — exactly the `testing` → `enforce` progression you use for MTA-STS.

DANE needs a **validating DoH (DNS-over-HTTPS, RFC 8484) resolver**, configured with `DANE_RESOLVER_URL`:

| Env var | Purpose |
| --- | --- |
| `DANE_MODE` | `off`, `report` (default), or `enforce`. An unrecognised value fails the MTA boot fast rather than silently degrading to a different posture. |
...
`report` and `enforce` both require `DANE_RESOLVER_URL`. **When it is unset, DANE is inert in every mode** (no TLSA lookups), so a fresh install — which has no resolver configured — behaves exactly as it did before, even though the default mode is `report`. Set `DANE_RESOLVER_URL` to turn report-only observability on.
::
...
- An overall TLS success rate, plus a per-partner breakdown.
...
Ingestion is idempotent by each report's own `report-id`, so a re-delivered report never double-counts. Watch this dashboard while a domain is on MTA-STS `testing` before you promote it to `enforce`.
