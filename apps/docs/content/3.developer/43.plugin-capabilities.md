...
Browser-bound plugin text is a different, narrower control: nav and settings labels are clamped to 64 code points with control and bidi-format characters stripped when the entry is derived. That is spoofing defense — Vue's HTML escaping is what defends against markup — and the injection-scrub policy is *not* applied on that path.

Hosted mail projections are truncated by Unicode code points: `from` 512, `to` 2 048, `subject` 1 024, each decrypted body 65 536.
