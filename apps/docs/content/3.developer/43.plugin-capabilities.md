...
External HTTPS services reached through scoped API keys, webhooks, and signed synchronous hooks. They never execute code inside Convex or Nuxt. The API-key and webhook halves are live; the hook protocol is implemented but **no pipeline stage calls it yet**. See [Connected Apps](/developer/plugin-connected-apps).
