...
Plugin-produced text is untrusted at every boundary. The host clamps it to a code-point maximum and runs an injection scrub before it can reach a prompt, a stored field, or the browser. There is deliberately no permissive default policy: each host adapter names the policy for its runtime, and a non-string value, a scrubber that throws, or a scrubber returning a non-string is rejected outright (`untrusted_output_rejected`).
...
5. **Plugin-produced text is untrusted** — clamped and injection-scrubbed at every boundary.
