...
Tenant-owned campaign, automation, agent-reply, transactional, and test sends resolve a fresh last-mile decision immediately before dispatch. Governed MTA intake requires the resulting lease and binds it to the message, organization, recipient, sender, message type, provider/pool, selected IP, and breaker generations. A changed or expired decision is scheduled through the same bounded workpool with the same idempotency key. System/auth mail and Postbox use separate master-key-only fixed-scope MTA endpoints; authenticated raw SMTP submission is an owned-MTA path and does not participate in the tenant relay fallback.
