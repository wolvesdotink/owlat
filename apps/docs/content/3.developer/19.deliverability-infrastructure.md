...
- **No reference transport means nothing to align.** The verdict is `single_arm`: the gate opens, every check reads as a pass, and no error, warning or "setup incomplete" state appears anywhere. The gate answers `hasReferenceArm` from the identity tables rather than from the stored row, so a deployment with zero third-party accounts is never blocked by a sweep that has not run yet.
