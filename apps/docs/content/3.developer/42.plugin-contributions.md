...
Both are data only: a labelled link to an internal dashboard path. The host clamps and injection-scrubs the label at render time, gates the entry behind the plugin flag, and orders it deterministically **after** every core entry.
