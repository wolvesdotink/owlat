...
Two capabilities have no contribution bucket because the host mediates them directly: `llm:invoke` (metered generation) and `worker:enqueue` (Tier-3 jobs).
...
The six built-in steps map to three host-owned placements:
...
Both are data only: a labelled link to an internal dashboard path. The label is clamped to 64 code points with control and bidi-format characters stripped when the entry is derived — that is spoofing defense, so a plugin cannot draw a label that visually impersonates a core one; HTML escaping in Vue is the XSS defense. The entry is gated behind the plugin flag and ordered deterministically **after** every core entry.
