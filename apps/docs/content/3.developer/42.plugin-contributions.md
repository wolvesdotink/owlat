...
sendTransports: [{
  ...
  requiredEnvVars?, optionalEnvVars?,                 // PLUGIN_-prefixed, no "__"
...
| --- | --- | --- |
| `supportsCustomReturnPath` | `yes`, `no` | `no`. `yes` asks the router to resolve an envelope sender and offer it to `buildDispatchExtras` as `returnPathHost` — forward it, or the measurement plane grades this arm's bounce data as comparable with our own while the bounces still land at your provider. |
