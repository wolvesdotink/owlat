...
`dkimSelectors` and `spfMechanisms` are carried on the STATE rather than declared in the manifest because both provider shapes are real — one shared account-wide selector, or per-domain tokens that only exist after registration. They are what the dual-transport alignment pre-flight resolves live, so an empty list reads as "we cannot describe this domain's signing identity": a HOLD on the ramp, never an opened gate. At most 8 of each (`PLUGIN_DOMAIN_IDENTITY_MAX_DNS_FACTS`), each at most 255 characters (`PLUGIN_DOMAIN_IDENTITY_MAX_DNS_FACT_LENGTH`), and anything over is dropped rather than refused. The `error` on a failed outcome or an invalid record is provider free text kept for an operator log line only, truncated at 500 characters (`PLUGIN_DOMAIN_IDENTITY_MAX_ERROR_LENGTH`) and never rendered as guidance.
