...
Four capabilities have no contribution bucket: the host mediates `llm:invoke`, `plugin-storage:read` and `plugin-storage:write`, while `worker:enqueue` is reserved for a Tier-3 adapter that is not shipped. The storage quotas and LLM budget live on the [capability reference](/developer/plugin-capabilities); the dormant worker protocol is documented under [Sandboxed Jobs](/developer/plugin-sandboxed-jobs).
