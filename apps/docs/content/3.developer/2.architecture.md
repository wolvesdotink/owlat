...
| Adjust autonomy thresholds | 7 days | Tighten/loosen autonomy thresholds by rejection rate |
...
/dashboard/transactional/*      # Transactional templates         (flag: transactional)
...
`NOTIFICATION_PROVIDER` and `VECTOR_STORE` are declared env keys (`lib/env.ts`) with no implemented factory/registry yet. Analytics is PostHog, configured via `POSTHOG_API_KEY` / `POSTHOG_HOST` — there is no provider-factory abstraction and no `ANALYTICS_PROVIDER` switch.
