...
| Email send | `lib/sendProviders/index.ts` (registry `SEND_PROVIDERS`; routing/fallback in `lib/sendProviders/routing.ts`) | `EMAIL_PROVIDER` | `mta`, `ses`, `resend`, `smtp`, `mandrill` (the catalog's kinds) — no implicit default; unset resolves to unconfigured |
