...
1. **Adapters** — one object-literal module per core kind the catalog declares, plus the generated module of any bundled plugin transport, each doing a single-attempt send and classifying its own errors.
...
The second `extras` argument is typed per provider via `ExtrasFor<K>`. MTA uses `MtaExtras` (`messageId`, `ipPool`, `engagementScore`, `dkimDomain`, governed organization/message type/routing lease, or the explicit system intake path) and Resend uses `ResendExtras` (`idempotencyKey`, forwarded as the `Idempotency-Key` header); SES takes no extras.
...
::callout{type="info"}
The `EMAIL_PROVIDER` env var is only the instance-wide fallback, and it names no kind of its own: it is consulted by `resolveRoute()` when an org has no `providerRoutes` config, no providers are enabled, or the chosen strategy returns nothing — see Routing below. It is no longer a hard switch on provider selection, and when it is unset too the route resolves to `null` (fail-closed) rather than to the MTA.
::

### Routing Strategies

`resolveRoute(routeConfig, healthStatuses)` in `lib/sendProviders/routing.ts` reads an org's `providerRoutes` row, filters to enabled + recognized providers, looks up a strategy via `strategyFor()`, and returns a `ResolvedRoute` (`providerType`, optional `ipPool`, and a `source` of `org_config | env_fallback | deliverability_fallback`). If no `providerRoutes` config resolves a provider, it falls through to the `EMAIL_PROVIDER` env var (`source: env_fallback`); if the env var is unset or unrecognized, `resolveRoute` returns `null` (fail-closed) — there is no implicit `mta` default.
...
| `adaptive_mix`      | Deterministic per-recipient split between the own MTA and the reference transport at the ramp controller's per-cell share. Controller-owned — written by the controller, never offered in the operator's strategy picker. | `strategies/adaptive_mix/`      |

Each strategy is a pure `select(entries, ipPool, healthStatuses, mix)` function. The fourth argument is the per-recipient mix context; only `adaptive_mix` reads it, and the shipped three ignore it. Adding a strategy is a one-folder change in `lib/sendProviders/strategies/`.
...
::callout{type="tip"}
Adding a send provider is a one-folder change: declare the entry in `packages/shared/src/sendProviderCatalog.ts` (the kind literal lives there — `SendProviderKind` in the backend is a re-export), create `lib/sendProviders/<kind>/index.ts`, and register it in `SEND_PROVIDERS`. A compile-time check on the registry catches a missing method or a declared kind with no adapter.
::
