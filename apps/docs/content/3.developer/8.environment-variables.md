...
| `DANE_ENABLED` | Enable DANE (RFC 7672) at send time for the built-in MTA: authenticate each recipient MX's certificate against its DNSSEC-validated TLSA RRset (`_25._tcp.<mx>`). DANE supersedes MTA-STS; a non-matching certificate defers the message (never falls back to cleartext). Off by default — byte-identical to legacy behaviour when off (`apps/mta/src/smtp/daneVerify.ts`). | `false` |
| `DANE_RESOLVER_URL` | DoH (DNS-over-HTTPS, RFC 8484 JSON) resolver URL for DANE TLSA lookups. **Required when `DANE_ENABLED=true`** (the MTA refuses to boot otherwise). Must be a **validating** resolver: the DNSSEC AD bit is trusted and an answer without it is treated as "no TLSA". Running a local validating resolver (e.g. Unbound on `127.0.0.1`) is the recommended production configuration. | — |
