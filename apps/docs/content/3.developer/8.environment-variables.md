...
| Variable | Description |
| --- | --- |
| `TWILIO_AUTH_TOKEN` | Validates `X-Twilio-Signature` on inbound SMS/voice webhooks (`webhooks/adapters/twilio.ts`). Fallback only: the SMS channel's stored **Auth Token** wins when set. The endpoint refuses requests if neither is configured. |
| `META_APP_SECRET` | Validates the `X-Hub-Signature-256` HMAC on inbound WhatsApp/Messenger webhooks (`webhooks/adapters/meta.ts`). Fallback only: the WhatsApp channel's stored **App Secret** wins when set. |
| `META_VERIFY_TOKEN` | Must match `hub.verify_token` during Meta's webhook subscription challenge (`webhooks/adapters/meta.ts`). Fallback only: the WhatsApp channel's stored **Verify Token** wins when set. |
| `GENERIC_WEBHOOK_SECRET` | Compared against the `x-webhook-secret` header on the generic inbound channel adapter (`webhooks/adapters/generic.ts`). Fallback only: the generic channel's stored **Secret Key** wins when set. |
...
| `TWILIO_AUTH_TOKEN` | Convex | If SMS channel, and not set in Settings → Channels | — | Validates inbound Twilio webhook signatures |
| `META_APP_SECRET` | Convex | If WhatsApp channel, and not set in Settings → Channels | — | Validates inbound Meta webhook HMAC |
| `META_VERIFY_TOKEN` | Convex | If WhatsApp channel, and not set in Settings → Channels | — | Meta webhook subscription challenge token |
| `GENERIC_WEBHOOK_SECRET` | Convex | If generic channel, and not set in Settings → Channels | — | Shared secret for the generic inbound channel adapter |
