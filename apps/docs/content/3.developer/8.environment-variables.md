...
| `DANE_RESOLVER_URL` | DoH (DNS-over-HTTPS, RFC 8484 JSON) resolver URL for DANE TLSA lookups. **Required when `DANE_ENABLED=true`** — the MTA refuses to boot without a valid URL. Must be a **validating** resolver: the DNSSEC AD bit is trusted and an answer without it is treated as "no TLSA". Running a local validating resolver (e.g. Unbound on `127.0.0.1`) is the recommended production configuration. | unset |
