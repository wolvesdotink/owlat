...
```sh
# NOTIFICATION_PROVIDER=noop   # reserved — declared but not yet consumed (see below)
```

::callout{type="warning" title="NOTIFICATION_PROVIDER is reserved"}
`NOTIFICATION_PROVIDER` is declared in the typed env allowlist (`lib/env.ts`) but is **not currently read by any provider implementation** — there is no consumer in `apps/api/convex`, so the value has no effect yet.
::

### Optional: Vector store (`ai.knowledge` flag)

```sh
# VECTOR_STORE=convex          # reserved — declared but not yet read by code (Convex is the only backend)
```
...
| Variable | Description | Default |
| --- | --- | --- |
| `NOTIFICATION_PROVIDER` | Reserved. Declared in `lib/env.ts` but not yet consumed by any provider implementation. | `noop` |

::callout{type="warning" title="NOTIFICATION_PROVIDER is reserved"}
`NOTIFICATION_PROVIDER` is declared in the typed env allowlist (`lib/env.ts`) but is **not currently read by any code** in `apps/api/convex`. Setting it has no effect until a provider implementation consumes it.
::
...
| Variable | Description | Default |
| --- | --- | --- |
| `VECTOR_STORE` | Reserved. Declared in `lib/env.ts` but not yet read by any code — Convex is the only vector backend. | `convex` |
...
| `NOTIFICATION_PROVIDER` | Convex | No | `noop` | Reserved — declared but not yet consumed by any code |
| `VECTOR_STORE` | Convex | No | `convex` | Reserved — declared but not yet read by code |
