...
### Optional: Slack approvals reference app (Tier-2 connected app)

The bundled Slack approvals connected app (PP-26) holds every autonomous auto-send until a quorum of Slack users approves it. It is **restrict-only**: an approval only releases its own hold (returning the send to the path it would have taken anyway — every core safety gate still runs), and it can never force an approval, bypass a gate, or send. The app is **inert** unless BOTH secrets below are set; a failed Slack notification, a Slack outage, or any error holds the send (never sends). See `convex/slack/`.

| Variable | Description | Default |
| --- | --- | --- |
| `SLACK_APPROVALS_SIGNING_SECRET` | Slack app signing secret. Authenticates the interaction callback at `POST /webhooks/slack/approvals` with Slack's v0 HMAC-SHA256 scheme plus a 5-minute freshness window (replay defense). Unset ⇒ the app is inert and the callback returns `503` (`slack/signature.ts`, `slack/approvalsHttp.ts`). | — |
| `SLACK_APPROVALS_WEBHOOK_URL` | Slack incoming-webhook URL the approval request is posted to. Must be an `https://hooks.slack.com` URL (SSRF allowlist). Unset ⇒ the app is inert (`slack/notify.ts`, `slack/config.ts`). | — |
| `SLACK_APPROVALS_QUORUM` | Distinct Slack approvers required to release a hold. Clamped to `[1, 25]`; invalid/unset ⇒ `1`. A larger value never weakens the gate (`slack/config.ts`). | `1` |
| `SLACK_APPROVALS_TTL_MINUTES` | Minutes an approval request stays open before it expires unapproved (the send stays held). Clamped to `[1, 10080]`; invalid/unset ⇒ `1440` (24h) (`slack/config.ts`). | `1440` |
