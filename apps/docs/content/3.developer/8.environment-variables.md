...
| `DANE_MODE` | DANE (RFC 7672) posture at send time — `off`, `report` (default), or `enforce`. `off` is byte-identical to legacy behaviour (no TLSA lookups). `report` looks up each recipient MX's DNSSEC-validated TLSA RRset (`_25._tcp.<mx>`), evaluates the certificate against it and **emits the TLS-RPT result** (`success`/`validation-failure` under the `tlsa` policy), but never requires TLS or bounces — delivery proceeds on the normal opportunistic/MTA-STS decision (observability only, honours D6). `enforce` additionally authenticates the certificate against the RRset (supersedes MTA-STS; a non-match defers the message, never cleartext). Needs `DANE_RESOLVER_URL` to run: without a resolver DANE is inert in every mode, so a fresh install behaves exactly as before. Read from the MTA's own process env (`apps/mta/src/config.ts`), **not** the Convex dashboard (`apps/mta/src/smtp/daneVerify.ts`). An unrecognised value fails the boot. | `report` |
