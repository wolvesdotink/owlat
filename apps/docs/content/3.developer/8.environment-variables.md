...
| `DANE_ENABLED` | Enable DANE (RFC 7672) at send time: authenticate each recipient MX's certificate against its DNSSEC-validated TLSA RRset (`_25._tcp.<mx>`). DANE supersedes MTA-STS; a non-matching certificate defers the message (never falls back to cleartext). Off by default — byte-identical to legacy behaviour when off. Read from the MTA's own process env (`apps/mta/src/config.ts`), **not** the Convex dashboard (`apps/mta/src/smtp/daneVerify.ts`). | `false` |
| `DANE_RESOLVER_URL` | DoH (DNS-over-HTTPS, RFC 8484 JSON) resolver URL for DANE TLSA lookups. **Required when `DANE_ENABLED=true`** — the MTA refuses to boot without a valid URL, and requires `https://` (plain `http://` is accepted only for a loopback resolver, since the trusted DNSSEC AD bit must travel over a channel an on-path attacker cannot forge). Must be a **validating** resolver: the AD bit is trusted and an answer without it is treated as "no TLSA"; a lookup that cannot be completed (SERVFAIL/timeout) defers delivery rather than downgrading. Running a local validating resolver (e.g. Unbound on `127.0.0.1`) is the recommended production configuration. | unset |
