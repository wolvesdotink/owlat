...
apps/desktop/
  src-tauri/                 # Rust backend
    src/
      ...
      tray.rs                # System tray (icon, menu, click-to-show)
...
**System tray** — persistent tray icon (`src-tauri/src/tray.rs`) with a menu and click-to-show. Notifications and tray badges are driven by Convex reactive queries the webview already subscribes to.

**Native notifications** — when a new item enters the review queue or a colleague sends a chat message, the OS notification system triggers via the Tauri notification plugin (`src/notifications.ts`).
...
The ask-anything surface is wired end-to-end. The backend is `quickQuery.ts` → `ask`, an `authedMutation` gated on the `knowledge:read` permission (not the `ai.knowledge` flag): it runs full-text search over `knowledgeEntries` (the `search_knowledge` index, top 5 hits) and synthesizes a plain answer by concatenating the matching entry snippets, each with its source citation. There is **no** vector/semantic retrieval, no semantic file store, and no LLM call — the answer is assembled directly from the matched entries. This is a separate read surface from the inbound-mail classifier — it does not need a "query" classification category.
::
