...
**Quick queries.** Organization members can ask the system free-form questions and get back the matching knowledge. The backend (`quickQuery.ask`) runs a full-text keyword search over Knowledge Graph entries only (the `search_knowledge` index) and returns the matching entries' content as the answer, with the entry titles as source references. It does *not* search the file store, does *not* do vector/semantic retrieval, and does *not* use the LLM to synthesize an answer or produce bracketed citations. The semantic, cross-source, LLM-synthesized answer with citations is planned. The ask-anything box (`QuickQueryPanel`) lives on the knowledge dashboard today.
...
**Internal knowledge queries.** Team members ask the system questions: "What was our refund policy decision last quarter?" or "How did we handle the last outage?" `quickQuery.ask` does a keyword search over Knowledge Graph entries only and returns the matching entries' content with their titles as sources — shipped today via the ask-anything box (see Quick queries above). It does *not* retrieve across the file store and does *not* LLM-synthesize an answer; the cross-source, LLM-synthesized version is planned.
