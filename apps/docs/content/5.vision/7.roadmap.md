...
| --- | --- | --- |
| Files | **Auto-capture of agent-generated output**, and **auto-linking captured attachments to the sender thread**. | Inbound email attachments are auto-captured into the library on delivery and linked to the **sender contact** when one already exists (unknown senders stay org-general), carrying the source Message-ID as provenance. Agent output is not auto-captured, and captured attachments are not yet linked to a thread. |
...
| Daily agent digest | On-thread TL;DRs already ship in [Postbox](/guide/postbox#ai-thread-summary) (cached advisory summaries on long threads); the remaining piece is a daily email summarizing auto-sends, the review backlog, and category trends. | M | P2 |
...
| Saved-reply library for the agent | A personal `/`-triggered snippet library already ships in [Postbox](/guide/postbox#text-snippets-canned-responses); the remaining piece is a shared, human-authored reply library the team-inbox agent can also draw on for routine answers. | M | P2 |
