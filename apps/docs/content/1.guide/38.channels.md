...
*Inbound webhook verification* secrets are read from Convex environment variables, **not** from these form fields — the environment variables are what make inbound requests succeed. See [Inbound channel webhooks](#inbound-channel-webhooks).

The form fields are what *outbound* dispatch sends with: they are encrypted at rest (`channels.outbound.encryptAndPersistConfig`) and decrypted by `channels.outbound.dispatchOutbound` when the AI agent (or the manual composer) replies on the channel. Two of them are stored-only and reach no provider today — WhatsApp's **Business Account ID** (the Meta send call is keyed on the Phone Number ID) and the generic webhook's **Secret Key** (nothing reads it; see below).
::
...
| **Secret Key** | stored encrypted, but currently read by nothing — see the warning below |

**Outbound generic POSTs are unsigned.** Owlat's POST to your endpoint carries `Content-Type: application/json` and nothing else — no signature and no shared-secret header. Do **not** configure your receiving endpoint to require one, or every AI-agent reply and manual composer send will be rejected and recorded as a failed message. Until per-channel secrets land, authenticate outbound deliveries with an unguessable path segment or query token on the **Endpoint URL**.

The **Secret Key** field is kept for a future per-channel *inbound* secret; no code path reads it today. For inbound generic messages, set the `GENERIC_WEBHOOK_SECRET` environment variable on the Convex backend — that variable, not the **Secret Key** field, is what Owlat compares against the incoming request's secret. With it unset, every inbound generic webhook is rejected.
