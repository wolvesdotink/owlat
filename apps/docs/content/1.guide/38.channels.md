...
The form previously offered a **Secret Key** box. It has been removed: nothing ever read it on the outbound path, and inbound verification does not use it either, so it only stored a shared secret that could not be honoured. For inbound generic messages, set the `GENERIC_WEBHOOK_SECRET` environment variable on the Convex backend — that variable is what Owlat compares against the incoming request's secret. With it unset, every inbound generic webhook is rejected.
