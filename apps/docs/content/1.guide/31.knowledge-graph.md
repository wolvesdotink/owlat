...
The extractor makes a single structured LLM call per message, embeds each resulting entry, and stores it. It does **not** itself deduplicate near-identical entries (a separate daily job handles that — see [Confidence decay and expiry](#confidence-decay-and-expiry)) or check for contradictions. It does hand the freshly-stored batch to a deterministic structural linker that auto-creates generic `relates_to` edges (as noted under [Relations](#relations-between-entries-typed-edges)); the LLM-inferred semantic edges remain on the roadmap. Semantic ("vector") search over knowledge **is** live: the agent's contact-scoped knowledge retrieval runs hybrid vector + full-text search fused with reciprocal-rank fusion, returning true nearest-neighbour matches. Full-text search and browse-by-type remain the live retrieval paths in the dashboard UI.
::
