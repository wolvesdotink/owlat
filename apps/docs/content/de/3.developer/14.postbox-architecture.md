...
inbound SMTP ──► apps/mta ──► mailboxResolver (apps/mta/src/inbound)
                                  ...
                                  ▼
                         mail/webhookHttp.ts ──► mail/delivery.ts
                                              │
...
Der MTA routet Zustellungen an persönliche Postfächer auf die dedizierte Route `POST /webhooks/mta-mailbox` (`apps/api/convex/http.ts`, bedient von `handleMailWebhook` aus `mail/webhookHttp.ts`); die Routing-Entscheidung fällt in `apps/mta/src/webhooks/convexNotifier.ts` (`inbound.mailbox.received` → `/webhooks/mta-mailbox`, alles andere → `/webhooks/mta`).
...
- Jeder Ingest trägt ein **`origin`**: `'sync'` für die vorwärts laufende IDLE-/Poll-Schleife, `'backfill'` für einen Import der Historie (`apps/mail-sync/src/connection.ts` setzt es an beiden Aufrufstellen). **Nur die Vorwärts-Synchronisierung** erreicht die KI-Pipeline: Eine als `'sync'` markierte Zustellung in den Posteingang stellt die Antwort-Warteschlangen- und die Smart-Inbox-Kategorieklassifizierung ein (`enqueueNeedsReplyCheck` / `enqueueCategoryCheck`), genau wie der gehostete Webhook-Pfad; ein `'backfill'` — und ebenso ein fehlendes `origin`, wie es ein älterer Worker sendet — stellt nichts ein, sodass der Import eines mehrjährigen Postfachs nie Hintergrund-LLM-Arbeit auffächert. `ingestExternalRaw` liest außerdem die Anti-Loop-Header (`Precedence:`) aus der Rohnachricht und reicht sie weiter, da sie nicht auf der Nachrichtenzeile gespeichert werden.
- Ein Worker-Neustart nach einer Ausfallzeit holt `lastSeenUid+1:*` — unter Umständen Hunderte Nachrichten, alle zu Recht als `'sync'` markiert und alle klassifiziert. Das ist derselbe Schwall, den ein gehostetes Postfach nach einer Störung verarbeitet, auf der günstigen Klassifizierer-Stufe.
...
Die Module für Verbinden/Testen/Credentials sind `external/accounts.ts` (Queries + interne Mutationen), `external/accountsActions.ts` (`connect` / `testConnection` / Credential-Handhabung), `external/accountShared.ts` (die Zeilenform samt `connectFieldsValidator`, über die beide Verbindungswege schreiben) und `external/delivery.ts` (Ingest).

### OAuth für externe Konten (Google)

`externalMailAccounts.authMethod` ist `'password'` (App-Passwort) oder `'oauth2'` (`oauthProvider: 'google'`). OAuth ist **ergänzend**, und der Passwortweg ist der, den es immer gibt: Das Verbindungsformular zeigt für Gmail die Felder aus E-Mail-Adresse und App-Passwort, gleich ob ein Client konfiguriert ist, und der Google-Block ist eine Ergänzung darunter. `GOOGLE_OAUTH_CLIENT_ID` / `GOOGLE_OAUTH_CLIENT_SECRET` zu setzen ist Betreiberarbeit pro Installation, die Oberfläche hängt also nie davon ab.

Der Ablauf ist ein Authorization-Code-Grant mit PKCE und benötigt **keine HTTP-Route** — das Weiterleitungsziel `${SITE_URL}/oauth/google/callback` ist eine Seite in `apps/web` unter dem Dashboard-Layout. Dadurch liegt die Sitzung während des gesamten Handshakes vor, und die bestehenden sitzungsgebundenen Mutationen `_connectInternal` / `_connectSharedInternal` / `_connectSeedInternal` sowie die Rotationsmutationen werden unverändert wiederverwendet:

1. `api.mail.external.googleOAuth.isConfigured` sagt dem Verbindungsformular, ob der zusätzliche Google-Block gerendert wird (ein aus der Env-Präsenz abgeleiteter Boolescher Wert — nie die Zugangsdaten selbst). Bei `false` entfällt der Block einfach; am übrigen Formular ändert sich nichts. Vorangestellt statt nachgestellt wird er in genau einem Fall — `mode="update"` auf einem Konto mit `authMethod: 'oauth2'`, wo „Erneut mit Google verbinden“ die gesuchte Aktion ist — und auch dort folgt das Passwortformular darunter, sodass sich ein solches Konto weiterhin auf ein App-Passwort umstellen lässt.
2. `api.mail.external.googleOAuthActions.start` nimmt die **Intent** (`connect` / `update` / `connectShared` / `updateShared` / `connectSeed`) und ein seiteneigenes `returnTo` entgegen, erzeugt eine `state`-Nonce plus einen PKCE-Verifier, legt beides in `externalMailOAuthStates` ab (15 Minuten TTL, eine aktive Zeile pro Benutzerin bzw. Benutzer, bei Verwendung gelöscht) und liefert die Autorisierungs-URL. Scopes: `https://mail.google.com/ openid email`, mit `access_type=offline` + `prompt=consent`, damit Google ein Refresh-Token zurückgibt.
3. Die Callback-Seite übergibt `code` + `state` an `api.mail.external.googleOAuthActions.complete`. Die Action verbraucht die State-Zeile (einmalig; die Zeile einer anderen Person oder eine abgelaufene gilt als nicht vorhanden), tauscht den Code unter `https://oauth2.googleapis.com/token` ein, liest die angemeldete Adresse aus dem `id_token` (`aud` / `iss` / `email_verified` werden geprüft; die Signatur nicht, da das Token als Antwortkörper unserer eigenen Server-zu-Server-TLS-Anfrage kam) und führt die hinterlegte Intent aus. Dass die Intent bereits bei `start` festgelegt wird, verhindert, dass ein manipulierter Callback aus „mein Postfach neu verbinden“ ein „Team-Postfach verbinden“ macht; eine erneute Autorisierung muss zusätzlich dieselbe Adresse anmelden.
4. Das Refresh-Token liegt **im bestehenden verschlüsselten Umschlag** (`{ oauthRefreshToken }` statt `{ imapPassword, smtpPassword }`), sodass es weiterhin genau eine Verschlüsselungs- und eine Entschlüsselungsstelle gibt und kein Access-Token je auf die Platte gelangt. `getCredentialsForWorker` erzeugt pro Abruf ein kurzlebiges Access-Token (im warmen Node-Isolat zwischengespeichert, solange >60 s Restlaufzeit; der Cache-Schlüssel enthält Konto **und** Umschlag, sodass nach einem erneuten Verbinden nie das Token der vorherigen Freigabe ausgeliefert wird) und liefert `{ kind: 'credentials', credentials }` mit `imapAccessToken` + `smtpAccessToken` und leeren Passwortfeldern. Andernfalls antwortet die Action mit `{ kind: 'unavailable', reason }`: `auth_revoked` (ein `invalid_grant` — das Konto wird auf `auth_error` mit „Google authorization was revoked. Reconnect with Google.“ gesetzt), `refresh_failed` (eine vorübergehende Ablehnung durch Google) oder `missing` (keine Zeile oder ein Umschlag, der sich nicht entschlüsseln lässt).
5. Der mail-sync-Worker wählt das Verfahren in `src/auth.ts`: Ein nicht leeres Access-Token bedeutet `AUTHENTICATE XOAUTH2` (ImapFlow) bzw. SASL XOAUTH2 (`@owlat/smtp-client`), sonst das Passwort. `isAuthError` behandelt auch die XOAUTH2-Fehlermeldungen als endgültig. `auth_revoked` ist ebenso endgültig und wird getrennt von den wiederholbaren Gründen behandelt: Die Verbindungsschleife stoppt und schreibt **keinen** Status, sodass die Meldung „Reconnect with Google“ des Backends erhalten bleibt, während `missing` / `refresh_failed` den gewohnten Fehler-und-Backoff-Pfad nehmen. Die `/send`-Route meldet den widerrufenen Fall mit genau diesen Worten, und der Seed-Probe-Durchlauf überspringt das Konto (eine Probe, nach der nicht gesucht werden konnte, wird nie als fehlend erfasst).

`testConnection` bleibt passwortbasiert — eine OAuth-Verbindung bestätigt sich durch die erste Statusrückmeldung des Workers.
