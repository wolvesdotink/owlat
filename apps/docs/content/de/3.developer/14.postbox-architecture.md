...
`externalMailAccounts.authMethod` ist `'password'` (App-Passwort) oder `'oauth2'` (`oauthProvider: 'google'`). OAuth ist **ergänzend**: App-Passwörter bleiben bei jedem Anbieter unterstützt, auch bei Gmail, und eine Installation ohne `GOOGLE_OAUTH_CLIENT_ID` / `GOOGLE_OAUTH_CLIENT_SECRET` verhält sich exakt wie bisher.
...
1. `api.mail.external.googleOAuth.isConfigured` sagt dem Verbindungsformular, ob der Google-Zweig angeboten wird (ein aus der Env-Präsenz abgeleiteter Boolescher Wert — nie die Zugangsdaten selbst).
