...
Wenn Sie Redis mit einem `REDIS_PASSWORD` abgesichert haben (siehe [Redis-Authentifizierung](/developer/self-hosting-production#redis-authentication)), stellen Sie jedem `redis-cli`-Befehl `-a "$REDIS_PASSWORD" --no-auth-warning` voran, z. B. `docker compose exec redis redis-cli -a "$REDIS_PASSWORD" --no-auth-warning info memory`.
::
...
4. **Redis absichern** — `REDIS_PASSWORD` ergänzen, wie unter [Produktions-Deployment](/developer/self-hosting-production#redis-authentication) beschrieben.
