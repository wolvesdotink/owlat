...
Die beiden stets vorhandenen Abstraktionen liegen unter `apps/api/convex/lib/` (die Dateien der Entscheidungsebene stehen weiter unten bei ihr):
...
## Entscheidungs-Provider (die dritte KI-Ebene)

**Gespeicherte Konfiguration:** die Admin-Seite „KI-Provider“. **Env-Fallback:** `DECISION_PROVIDER`, `TYPESAFE_API_KEY`, `DECISION_MODEL`, `DECISION_BASE_URL`.

**Unterstützte Werte:** `llm` (Standard), `typesafe`

Die LANGUAGE-Ebene schreibt, die EMBEDDING-Ebene sucht. Die **Entscheidungs**-Ebene tut beides nicht: Sie beantwortet typisierte Fragen — ja/nein, eine dieser Kategorien wählen, etwas auf einer geordneten Skala einordnen — und liefert eine Wahrscheinlichkeitsverteilung über die mitgesendeten Optionen statt eines Satzes mit einem selbst berichteten Confidence-Feld darin. Sie ist eine eigene Ebene, weil diese Wahrscheinlichkeiten die `LanguageModel`-Schnittstelle, die die beiden anderen sprechen, nicht überleben (siehe ADR-0060).

```
lib/
├── decisionProvider.ts         ← löst die Ebene auf: gespeicherte Konfiguration → Env → Standard
├── decisionProviders/
│   ├── types.ts                ← DecisionProviderAdapter<K>, Kinds, Endpunkt-Provenienz (rein)
│   ├── index.ts                ← Registry DECISION_PROVIDERS + decisionProviderFor(kind)
│   ├── typesafe.ts             ← TypeSafe (Jev) — POST <origin>/v1/systemone
│   ├── llm.ts                  ← sprachmodellgestützter Adapter — was eine Installation ohne Schlüssel nutzt
│   └── wire.ts                 ← Codec für Anfrage und Antwort
└── decision/
    ├── questions.ts            ← noul() / choice() / score() und die Antworttypen (rein)
    ├── catalog.ts              ← das eine Verzeichnis der Fragensätze, die das Produkt stellt
    ├── contract.ts             ← was runDecision entgegennimmt und zurückgibt, und seine Ports
    ├── dispatch.ts             ← runDecision() — Deadline, Retry, ein Fallback-Sprung, Abrechnung
    ├── ports.ts                ← die ctx-gebundenen Ports: Sicherung, Verbrauchsschreiber
    └── pricing.ts              ← Listenpreis + Kostenzulassung für die Ebene
```

Zwei Adapter werden ausgeliefert. `llm` übersetzt einen Fragensatz auf das Sprachmodell, das das Deployment ohnehin hat, und kennzeichnet sein Ergebnis mit `calibrated: false`; darauf löst jede Installation ohne Entscheidungsschlüssel auf, und es entspricht dem Verhalten, das das Produkt vor dieser Ebene hatte. `typesafe` spricht mit dem eigenen Endpunkt des Anbieters und liefert kalibrierte Wahrscheinlichkeiten. Aufrufstellen importieren `runDecision` und ihren eigenen Fragensatz — nie einen Adapter —, weshalb der Wechsel zwischen beiden eine Auswahlliste ist.

Jeder Fragensatz, den das Produkt stellt, ist in `catalog.ts` registriert, und
eine Testsuite führt das gesamte Verzeichnis durch **beide** Adapter: Sie prüft,
dass beide mit denselben Schlüsseln, denselben Wertebereichen und demselben
Wahrscheinlichkeits-Schlüsselraum antworten — nur `calibrated` darf sich
unterscheiden. Erst das macht aus der Auswahlliste eine echte Wahl.

Die Modellversion ist gepinnt, und der Pin wirkt auf die **Antwort**: Die von
Ihnen konfigurierte ID (Auswahlliste oder `DECISION_MODEL`) wird auch gesendet,
und alles, was der Anbieter außer der exakt gepinnten Version
zurückmeldet, gilt als unkalibriert — damit ruhen alle nachgelagerten
Schwellenwerte, bis die Kalibrierung erneut gelaufen ist. Die Schaltfläche
**Testen** auf der Einstellungsseite stellt eine echte Anfrage: eine einzige
Ja/Nein-Frage zu einem neun Wörter langen Zustand. Ein widerrufener Schlüssel,
eine weiterleitende Basis-URL und eine abgekündigte Modellversion fallen so an
der Schaltfläche auf statt später auf dem Eingangspfad.

Nach einem Upgrade ist hier nichts eingeschaltet. Das Feature-Flag `ai.decisionPlane` ist standardmäßig **aus**, setzt `ai` voraus und gehört bewusst **nicht** zum Feature-Paket `ai`: Ein Klick auf ein Paket darf nicht damit beginnen, Nachrichteninhalte an einen Anbieter zu senden, den niemand ausgewählt hat. Ohne hinterlegten Schlüssel löst die Ebene auf `llm` auf.

::callout{type="warning" title="Datenfluss: was Ihr Deployment verlässt — und wie Sie es abschalten"}
**Was das Deployment verlässt.** Nur, wenn das Flag an *und* ein `typesafe`-Schlüssel hinterlegt ist. Jeder Aufruf sendet den **Zustand** einer Entscheidung — Nachrichtentext, auf sichtbaren Text reduziert, ohne zitierte Verläufe und Signaturen, ohne Anhänge, auf ein Zeichenbudget gekürzt — sowie den Text der Fragen selbst. Was dieser sichtbare Text dann noch enthält, geht mit, Namen und E-Mail-Adressen eingeschlossen. Anhänge, Bilder und gespeicherte Zugangsdaten nie. Zurück kommen Zahlen.

**An welchen Host.** `POST https://api.typesafe.ai/v1/systemone` oder an den Origin, den Sie in `DECISION_BASE_URL` setzen, wenn Sie dem Anbieter einen eigenen Proxy vorschalten. Ausschließlich HTTPS und über den SSRF-Guard (`lib/ssrfGuard.ts`), der Weiterleitungen ablehnt — eine 3xx-Antwort dieses Hosts wird zum Fehler, statt einem Sprung zu folgen, den Sie nicht konfiguriert haben.

**Mit wessen Schlüssel.** Mit Ihrem. Owlat liefert keinen Schlüssel mit, vermittelt keinen und leitet nichts weiter. Sie eröffnen ein Konto beim Anbieter und hinterlegen den Schlüssel entweder auf der Seite „KI-Provider“ (versiegelt mit dem Instanz-Datenschlüssel, entschlüsselt nur innerhalb einer Node-Action, nie von einer Query zurückgegeben) oder setzen `TYPESAFE_API_KEY` im Deployment.

**Wie Sie es abschalten.** Drei Schalter, vom gröbsten an: Das Flag `ai.decisionPlane` ausschalten — ein Klick, kein erneutes Deployment, jede Entscheidung geht zurück an Ihr Sprachmodell; oder auf der Seite „KI-Provider“ das Sprachmodell wieder als Entscheidungs-Provider wählen; oder den Schlüssel löschen. Eine Installation, die nie einen hinterlegt hat, ist bereits in diesem Zustand.

**Welche Postfächer betroffen sind.** Derzeit keines: Die Ebene wird ohne Aufrufstelle ausgeliefert, sodass die einzige Anfrage, die ein Deployment verlässt, die des Test-Buttons in den Einstellungen ist. Wenn die Aufrufstellen migrieren, wird ein von Entscheidungen Dritter ausgenommenes Postfach ausgeschlossen, bevor der Zustand überhaupt zusammengestellt wird — der Ausschluss ist eine Eigenschaft des Postfachs und nichts, worüber jede Aufrufstelle neu entscheidet. Solange dieser Schalter je Postfach noch nicht auf dem Bildschirm ist, ist das obige Flag der Ausschluss, und es gilt für das gesamte Deployment.

**Wer verantwortlich ist.** Sie. In einem selbst gehosteten Deployment sind **Sie** die verantwortliche Stelle und der Entscheidungsanbieter ist **Ihr** Auftragsverarbeiter — nicht unserer. Wir sind an diesem Verhältnis nicht beteiligt, können nichts in Ihrem Namen unterzeichnen, und es gibt in diesem Repository keine Unterauftragsverarbeiter-Liste, auf der Ihr Deployment stünde. Lassen Sie sich vor dem Einschalten Region, Aufbewahrungsdauer und die Haltung zum Modelltraining schriftlich geben und halten Sie das gegen das, was Sie Ihren eigenen Nutzerinnen und Nutzern zugesagt haben.
::
...
Jede Zelle oben ist der in `packages/shared/src/sendProviderCatalog.ts` deklarierte Wert — die Tabelle gibt den Katalog wieder, sie definiert ihn nie. `apps/docs/__tests__/pageInvariants.test.ts` liest beides und schlägt fehl, sobald diese Seite eine vom Katalog deklarierte Art nicht mehr nennt, und `apps/docs/__tests__/docsVocabulary.test.ts` schlägt bei jedem Namen fehl, den der Code nicht mehr deklariert — eine Zelle, die dem Code widerspricht, ist damit ein Testfehler und nichts, was eine lesende Person bemerken müsste.
...
- **Governed Dispatch** (`delivery/governedDispatch.ts`) — die Identitätsbindung vor dem Dispatch, die Message-ID-Substitution, das Verdikt `isCustodyHandoff` und das Replay einer mehrdeutigen Annahme lesen alle `acceptanceSemantics` und `messageIdSource`. Diese Datei vergleicht überhaupt keine Provider-Art mehr mit einem Literal; ein Test sichert das ab.
...
| 6 | Doku | diese Seite — ein Abschnitt hier und eine Zeile, die Sie in der [Umgebungsvariablen-Referenz](/developer/environment-variables#provider-selection) schreiben; es gibt keinen Generator, beide sind also handgeschrieben und beide sind durch `apps/docs/__tests__/pageInvariants.test.ts` an den Eintrag **gepinnt**, das fehlschlägt, wenn eine Art auf dieser Seite fehlt oder die Referenz eine Convex-Variable dokumentiert, die das Backend nie liest | immer | Die `README.md` des Pakets und das in `plugins.config.ts` gelistete Paket — was es zugleich *gebundelt* statt bloß geschrieben macht |
