...
`apps/api/convex/seedAdminHttp.ts`); jeder weitere Nutzer tritt dieser Singleton-Org
über den Einladungsablauf als Editor/Admin/Owner bei.

In Produktion kann eine öffentliche Registrierung nicht das erste Konto anlegen:
Dafür ist der authentifizierte Seed-Endpunkt zuständig. Nur mit
`OWLAT_DEV_MODE=true` bleibt die Erstregistrierung für Entwicklungstests erlaubt.

Neue Konten erhalten immer eine Verifizierungs-E-Mail. Eine Organisationseinladung
kann erst mit bestätigter E-Mail-Adresse angenommen werden; die Einladungs-ID
allein genügt nicht, da Organisationsmitglieder Einladungen auflisten können.
`REQUIRE_EMAIL_VERIFICATION` steuert unabhängig davon die Verifizierung beim Login.
Bestehende Installationen dürfen die Einstellung unverändert lassen, damit bisher
unverifizierte Mitglieder sich weiterhin anmelden können. Für neue Einladungen
ist ein funktionierender System-Mailversand erforderlich.
