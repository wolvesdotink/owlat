...
### Drei Ebenen, eine Adapter-Regel

Die Abstraktion umfasst inzwischen drei KI-Ebenen. Jede wird in einem eigenen Modul aufgelöst, und jede folgt derselben Regel: eine Registry aus Ein-Datei-Adaptern hinter einer Mapped-Type-Zuweisung (eine fehlende Methode ist ein Compile-Fehler), wobei Aufrufstellen den Einstiegspunkt importieren statt eines Adapters.

| Ebene | Aufgelöst in | Aufgabe | Konfiguriert über |
| --- | --- | --- | --- |
| **Sprache (LANGUAGE)** | `lib/llmProvider.ts` | alles, was schreibt — Entwürfe, Antworten, Zusammenfassungen, Übersetzungen, der Assistent | die `LLM_*`-Variablen oben oder die Admin-Seite für KI-Provider |
| **Embedding** | `lib/llmProvider.ts` (`getEmbeddingModel()`) | Vektoren für die Suche, unabhängig von der Sprach-Ebene aufgelöst, damit die Suche auch unter einem Sprachanbieter ohne Embeddings-API weiter funktioniert | `LLM_EMBEDDING_MODEL` oder ein lokaler Embedder über `LOCAL_EMBEDDING_BASE_URL` |
| **Entscheidung (DECISION)** | `lib/decisionProvider.ts` | beantwortet typisierte Fragen — ja/nein, eine dieser Kategorien, ein Punkt auf einer geordneten Skala — mit einer Wahrscheinlichkeitsverteilung über die mitgesendeten Optionen | `DECISION_PROVIDER`, `TYPESAFE_API_KEY` oder die Admin-Seite für KI-Provider |

Die Entscheidungs-Ebene ist eine eigene Ebene und kein vierter LLM-Provider, weil das, was sie zurückgibt, die `LanguageModel`-Schnittstelle des AI SDK nicht überlebt: Eine als Completion verpackte Verteilung kommt an der Aufrufstelle als geparstes Objekt an, dessen Wahrscheinlichkeiten an der Grenze weggeworfen wurden — also genau das, wofür es die Ebene gibt.

Sie ist **opt-in und standardmäßig aus**: Das Feature-Flag `ai.decisionPlane` setzt `ai` voraus und gehört bewusst nicht zum Feature-Paket `ai`. Eine Installation ohne konfigurierten Entscheidungs-Schlüssel löst auf den `llm`-Adapter auf, der dieselben Fragen über das ohnehin konfigurierte Sprachmodell beantwortet und seine Antworten als nicht kalibriert kennzeichnet. Ein bestehendes Deployment verhält sich damit exakt wie zuvor.

Ihr Contract — die Fragen-DSL, die Regeln des Dispatch zu Deadline, Retry, dem einen Fallback-Sprung und der Abrechnung sowie die Begründung, warum keine Entscheidungsantwort automatisches Senden ausweiten darf — steht in ADR-0060 (`docs/adr/0060-decision-plane.md`). Konfiguration und Datenfluss aus Betreibersicht stehen auf der Seite [Provider](/developer/providers#entscheidungs-provider-die-dritte-ki-ebene). Beides wird hier nicht wiederholt.
