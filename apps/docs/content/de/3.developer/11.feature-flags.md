...
Das optionale Flag `ostr` (standardmäßig aus, benötigt `postbox`) lässt die Registerstufe `flagged` eingehende Nachrichten nach Spam verschieben und zeigt die Stufe im Leser an. Es benötigt eine Aggregator-URL und einen Verifikationsschlüssel; Veröffentlichungen im Beobachtermodus werden separat vom Betreiber aktiviert. Siehe [Vertrauensregister](/developer/trust-registry).
