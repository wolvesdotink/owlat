...
- **Komponierte Floors**: Statt diesen Aufruf am Anfang jedes Handlers zu wiederholen, kann ein gegatetes Modul die Prüfung in seine Builder backen — `featureGated(authedQuery, '<flag>')` für ein einzelnes Flag, `featureGatedAny(authedMutation, ['<a>', '<b>'])`, wenn bereits eine von mehreren Fähigkeiten die Oberfläche rechtfertigt. `chat/_helpers.ts` (`chatQuery` / `chatMutation`) und `mail/_helpers.ts` (`postboxQuery` / `postboxMutation` auf `['postbox','mail.external']`, passend zum `requiresAnyFeature` der Route) tun genau das. Der Floor läuft nach dem Auth-Floor des Builders und vor dem Handler; die Authorisierung pro Datensatz bleibt im Handler. Nur Queries und Mutationen lassen sich so gaten — die Prüfung liest `ctx.db`, eine gegatete Action behält daher die Prüfung im Handler gegen eine Query, die sie aufruft.
- **Weiche Gates für weiche Reads**: Eine `publicQuery`, deren Vertrag „gib die leere Form zurück, wirf nie“ lautet, sollte keinen werfenden Floor bekommen, denn sie wird oft von einer Oberfläche ohne Feature-Meta abonniert. Entscheide das Flag dort, wo der Read den Zugriff ohnehin entscheidet — `mail/permissions.ts` meldet aus dem Mailbox-Gate das Ergebnis `feature_off`, sodass jeder mailbox-bezogene Read auf sein bestehendes leeres Resultat zurückfällt.
...
6. **Gaten Sie den Server.** Rufen Sie am Anfang jeder öffentlichen Convex-Funktion hinter dem Flag (nach der Auth-Prüfung) `await assertFeatureEnabled(ctx, '<key>')` aus `apps/api/convex/lib/featureFlags.ts` auf — die Funktion liest die gespeicherten Flags, löst Abhängigkeiten auf und wirft selbstständig einen `forbidden`-Fehler. Für ein ganzes Modul komponieren Sie die Prüfung mit `featureGated` / `featureGatedAny` in einen Builder, statt sie zu wiederholen. Ein Gate in der Oberfläche allein ist keine Durchsetzung: Jede öffentliche Convex-Funktion ist für jeden authentifizierten Aufrufer erreichbar, der die Deployment-URL kennt.
