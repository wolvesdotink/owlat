...
- `isBlocked` (Session) — eine Punktabfrage. Andere Convex-Funktionen lesen die Liste ohne Zugriffsprüfung über `findBlockedByEmail` (`blockedEmails/lookup.ts`); die letzte Prüfung des Workers vor dem Versand einer Marketing-Mail (`delivery/marketingDispatchGate.ts`) ist eine davon. `isBlockedInternal` bleibt nur für ein Release erhalten, für Worker des vorigen Releases, die während eines Upgrades noch laufen.
