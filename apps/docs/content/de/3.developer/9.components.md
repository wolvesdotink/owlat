...
Umschalter aus Schiene und Griff mit `role="switch"`-Semantik — das An/Aus-Bedienelement, das in Einstellungen, Präferenzen und Konfigurationskarten verwendet wird. Verwenden Sie ihn für jede An/Aus-Einstellung und lassen Sie den Schalter den Zustand zeigen: Geben Sie ihm einen festen Namen (die Einstellung, nicht „An“/„Aus“) über `label` oder ein `<label for>`. Checkboxen bleiben für Mehrfachauswahl-Listen, Segment-Steuerelemente für Einstellungen mit mehr als zwei Zuständen und ein maßgeschneidertes Bedienelement für dreiwertige Feature-Pakete (an/teilweise/aus).
