...
| `CLAMAV_IMAGE` | `clamav/clamav` | ClamAV-Image-Repository. Das Alpine-Standardimage gibt es nur für amd64; der Setup-Assistent schreibt auf arm64-Hosts `clamav/clamav-debian`. Bei einer bestehenden amd64-Installation nicht umstellen: Das Debian-Image läuft mit einer anderen uid und kann das vom Alpine-Image angelegte Signatur-Volume nicht lesen. |
| `CLAMAV_VERSION` | `stable` | Tag auf `CLAMAV_IMAGE` |
| `TINYPROXY_VERSION` | `1.7` | `kalaksi/tinyproxy` (nur mit `--profile inbox-codetasks` verwendet) |
