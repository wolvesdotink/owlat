...
Die Aufbewahrung unterscheidet sich bewusst nach Grund. `hard_bounce` und `complaint` laufen nie ab — ein Befund über ein Postfach veraltet nicht. Ein über `POST /suppression` ergänzter `manual`-Eintrag läuft nach sieben Tagen ab; so erhält eine eskalierte Soft-Bounce-Adresse eine zweite Chance, sobald sich das Postfach erholen konnte. Über `POST /suppression/bulk` ergänzte Einträge laufen unabhängig vom Grund nie ab, denn über diesen Endpunkt überträgt eine Betreiberin eine bestehende Suppression-Liste auf diesen MTA. Ablaufende Einträge werden nach Fälligkeit in `mta:suppressed-expiring` indiziert und von einem stündlichen Sweep auf dem Leader entfernt; ein dauerhafter Eintrag steht nie in diesem Index und kann deshalb nicht entfernt werden.
