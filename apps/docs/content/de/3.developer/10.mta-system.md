...
| `GET` | `/identity` | Master | Gespeicherter FCrDNS-/Quelladressen-Status je Versand-IP |
| `POST` | `/identity/recheck` | Master | Reverse DNS sofort neu beobachten und die frischen Urteile zurückgeben (das rufen `owlat doctor` und der Installer nach einer PTR-Änderung auf, statt auf den stündlichen Durchlauf zu warten) |
...
Adaptive Drosselungsprofile lassen sich zur Laufzeit ohne erneutes Deployment anpassen. Profile werden beim Start aus `DESTINATION_PROVIDER_PROFILES` in `src/config/ispProfiles.ts` eingespielt (über `HSETNX`, sodass Laufzeit-Overrides Neustarts überleben) und über `src/routes/ispProfiles.ts` verwaltet.
...
| Methode | Pfad | Auth | Beschreibung |
| --- | --- | --- | --- |
| `GET` | `/queue/stats` | Master | Tiefe nach Zustand (waiting, active, delayed, completed, failed), Gruppenanzahl und das Delay-Set-Urteil, das auch `/health` meldet |
| `GET` | `/queue/pending` | Master | Kopf der Queue (`?limit=`, max. 200; `?domain=` für eine Empfängerdomain) |
| `GET` | `/queue/jobs/:jobId` | Master | Aufgelöster Zustand, Versuchsanzahl und Payload-Metadaten eines Jobs |
| `DELETE` | `/queue/jobs/:jobId` | Master | Einen eingereihten Job entfernen. Verweigert (`409`), wenn ein Worker ihn gerade zustellt |

`waiting` zählt jeden Job, der in einer Gruppe liegt, und ein verzögerter Job
bleibt während seiner Wartezeit in seiner Gruppe — in einer gesunden Queue ist
`delayed` also eine Teilmenge von `waiting` und kein Geschwisterwert; die beiden
addieren sich nicht. Diese Enthaltensein-Beziehung gilt allerdings nur im
gesunden Zustand: ein Eintrag im Delay-Set, dessen Gruppeneintrag fehlt, zählt
in `delayed` und nicht in `waiting`, `delayed` kann `waiting` also übersteigen.
Lesen Sie diese Umkehrung als Diagnose und nicht als Fehler — sie bedeutet, dass
das Delay-Set Einträge hält, die nichts mehr abarbeiten kann; `delayedQueue.status`
benennt es.

`completed` und `failed` sind Größen des Aufbewahrungsfensters, keine
Gesamtsummen über die Laufzeit: die Queue behält eine begrenzte Anzahl erledigter
Jobs (`keepCompleted` / `keepFailed`) und verwirft den Rest. Auch
`/delivery-logs` ist begrenzt (`DELIVERY_LOG_MAX_LEN`, `DELIVERY_LOG_TTL_HOURS`).
Die dauerhafte Aufzeichnung dessen, was versendet wurde, liegt in Convex und wird
von den Zustell-Webhooks geschrieben.

Nachrichteninhalte werden nie zurückgegeben: beide Job-Endpunkte melden nur
identifizierende Metadaten und Größenangaben. Die Größen unter `bytes` sind
durchweg Byte-Zahlen und jedes Feld ist nach dem benannt, was es zählt
(`htmlBytes`, `textBytes`, `ampBytes`, `sealedMimeBytes`, `attachmentCount`,
`attachmentBytes`).

`/queue/pending` liefert den Kopf der Queue statt einer Seite — es meldet
`waiting` neben `sampled` (was der Scan gesehen hat), damit eine Teilansicht nie
mit einer leeren verwechselt wird. Mit `?domain=` sind beide Zahlen auf diese
Empfängerdomain bezogen und beide exakt: der Filter wird über die Gruppen
`{ipPool}:{recipientDomain}` aufgelöst, die die Domain kodieren, und nicht durch
Filtern einer Stichprobe — ein großer Rückstau für eine Domain kann also nie als
„nichts eingereiht“ zurückkommen.

Es gibt kein Bulk-Flush. GroupMQ gruppiert Jobs nach
`{ipPool}:{recipientDomain}`, die Queue hat also keine Organisationsdimension,
entlang derer sich leeren ließe, und auch kein Bulk-Remove entlang der Dimension,
die ein Flush bräuchte. Die einzige Umsetzung müsste jede eingereihte Payload
lesen — unbegrenzte Arbeit genau auf dem Rückstau, für den man zu einem solchen
Aufruf greift, und sie würde zustellbare Mail mit einer einzigen unbestätigten
Anfrage vernichten. Entfernen Sie Jobs einzeln.

Um die gesamte Queue nach einer Fehlkonfiguration zu verwerfen, löschen Sie
ausschließlich die Schlüssel der Queue:

```bash
docker compose exec -T redis redis-cli -a "$REDIS_PASSWORD" --no-auth-warning \
  --scan --pattern 'groupmq:owlat-mta:*' | \
  xargs -r docker compose exec -T redis redis-cli -a "$REDIS_PASSWORD" --no-auth-warning DEL
```

::callout{type="warning" title="Niemals FLUSHALL auf diesem Redis"}
Das Redis des MTA ist zugleich der Geheimnisspeicher der Instanz: dort liegen die
**privaten** DKIM-Schlüssel (`mta:dkim:{domain}`), die es sonst nirgends gibt —
Convex erhält immer nur den öffentlichen Teil — dazu die API-Zugangsdaten pro
Organisation und der Warmup-Zustand. `FLUSHALL` vernichtet sie, und jede spätere
Nachricht wird dann mit einem frischen Schlüssel signiert, dessen öffentlicher
Teil nicht zum veröffentlichten `._domainkey`-Eintrag passt. Siehe
[Redis-Wartung](/developer/self-hosting-maintenance#redis-wartung).
::
...
- **`src/queue/handler.ts`** ist die einzige an GroupMQ gekoppelte Datei. Die Pipeline wirft niemals; der Handler reiht ein `defer`-Ergebnis selbst als Nachfolge-Job mit der berechneten Verzögerung ein (mit ±15 % Jitter, um eine Thundering Herd zu vermeiden), sodass eine Zurückstellung nie einen Zustellversuch verbraucht, und übersetzt ein `drop` in ein protokolliertes Endergebnis.
...
Die Aufbewahrung unterscheidet sich bewusst nach Grund. `hard_bounce` und `complaint` laufen nie ab — ein Befund über ein Postfach veraltet nicht. Ein über `POST /suppression` ergänzter `manual`-Eintrag läuft nach sieben Tagen ab.

Dieser Ablauf ist keine zweite Chance für das Postfach. Die Adresse wird nicht erneut geprüft, und der Blocklisten-Eintrag, der die Spiegelung ausgelöst hat (siehe unten), verfällt nicht von selbst — jeder von Owlat abgesicherte Versandpfad lehnt sie also weiterhin ab. Begrenzt wird durch die sieben Tage nur die KOPIE in dieser Absicherung: Die Spiegelung verläuft nur in eine Richtung und nur beim Anlegen, eine in der App später entsperrte Adresse bliebe hier sonst für immer unzustellbar. Sichtbar wird der Unterschied allein auf den Pfaden ohne Convex-seitige Prüfung — Systemmail einschließlich Double-Opt-in-Bestätigungen, Postbox-Versand, Testvorschauen für Betreiberinnen — und dort läuft eine `manual`-Sperre nach einer Woche tatsächlich ab.

`POST /suppression/bulk` schreibt jeden Eintrag unabhängig vom Grund dauerhaft. Owlat ruft diesen Endpunkt nirgends auf: Der Migrations-Import reicht jede übernommene Adresse durch dieselbe Einzelspiegelung, die auf `/suppression` schreibt — ein importierter Eintrag, der auf `manual` abgebildet wird, läuft hier also nach sieben Tagen ab wie jeder andere. Dauerhaft ist der Import in der Blockliste von Owlat, und die ist der maßgebliche Datensatz; die Dauerhaftigkeit von Bulk ist eine Eigenschaft des Endpunkts und keine Zusage über Importe.

Ablaufende Einträge werden nach Fälligkeit in `mta:suppressed-expiring` indiziert und von einem stündlichen Sweep auf dem Leader entfernt; ein dauerhafter Eintrag steht nie in diesem Index, der Sweep kann ihn also nicht erreichen. `GET /suppression/stats` zeigt die aktuellen Anzahlen je Grund.

Diese Liste ist die letzte Absicherung vor dem Versand, nicht der maßgebliche Datensatz: Die Blockliste von Owlat ist die Quelle der Wahrheit, und jeder dort ergänzte Bounce, jede Beschwerde und jede manuelle Sperre wird hierher gespiegelt (eine `unengaged`-Sunset-Sperre nicht — sie ist eine Marketing-Hygiene-Entscheidung und darf transaktionale Mail nicht blockieren). Die Spiegelung verläuft nur in eine Richtung und nur beim Anlegen — wer eine Adresse in der App aus der Blockliste entfernt, entfernt sie hier **nicht**. Löschen Sie die Kopie zusätzlich mit `DELETE /suppression/:email`, sonst bleibt die Adresse über diesen MTA unzustellbar.
...
**Ein Wiederholungsintervall, das ein fremder Server nennt, wird bei einer Stunde gedeckelt.** `extractGreylistDelay` liest `try again in N seconds/minutes` aus dem Antworttext — die einzige Verzögerung im MTA, deren Wert von einem Fremden stammt; alles andere (die Provider-Signaturen, das Aufwärm-Fenster der Obergrenze, die Breaker-Abkühlung) gehört uns und ist bereits begrenzt. Ohne Deckel konnte `try again in 999999 minutes` eine Wiederholung rund 694 Tage in die Zukunft legen: Der Job hält seine Nutzlast die ganze Wartezeit über in Redis, und wenn er aufwacht, ist seine Defer-Übergabequittung (TTL = das viertägige maximale Nachrichtenalter) verfallen, sodass die Nachricht im Dead-Letter landet statt zu bouncen. Eine Stunde ist das längste Intervall, das der MTA sonst irgendwo anfordert, und weit länger als jedes reale Greylist-Fenster. Wenn der Deckel greift, wird das gesagt — das Zustellprotokoll-Ereignis der Zurückstellung trägt eine Anmerkung, die nennt, worum der Empfänger gebeten hat. Unabhängig vom Parsen darf keine Zurückstellung eine Wiederholung hinter den Ablauf der Nachricht selbst legen: Eine Wartezeit, die die Nachricht überdauern würde, wird auf die Frist zurückgezogen, wo sie einen ordentlichen Ablauf-Bounce erhält.
...
Gibt eine Dispatch-Phase ein `defer`-Ergebnis zurück, reiht `src/queue/handler.ts` selbst einen Nachfolge-Job mit der gewünschten Verzögerung (plus ±15 % Jitter) ein und schließt den aktuellen ab, sodass eine Zurückstellung nie einen Zustellversuch verbraucht. Die Verzögerung wird durch die verbleibende Lebensdauer der Nachricht begrenzt: Eine Wartezeit, die das viertägige maximale Nachrichtenalter überdauern würde, wird auf die Frist zurückgezogen, denn ein Nachfolger, der nach dem Ablauf seiner Defer-Übergabequittung aufwacht, kann nur noch im Dead-Letter landen.

### Integrität der Verzögerungsmenge

GroupMQs sortierte Menge `:delayed` ist die einzige Struktur in der Wiederholungsleiter ohne Eigentümer außerhalb des Promoters — ein Mitglied gelangt durch eine verzögerte Einreihung hinein und wird vom Promoter entfernt, oder von gar nichts. Ein Mitglied, dessen Job-Hash fehlt, sammelt sich daher unsichtbar an: Es ist kein Job, auf den jemand wartet, kein Versand, über den jemand berichtet, und keine Queue-Tiefe, die irgendetwas ausliest. Unbenannt wächst es, bis Redis der Speicher ausgeht.

`GET /health` meldet einen `queue`-Block mit der Größe der Verzögerungsmenge, wie viele Mitglieder mehr als zehn Minuten überfällig sind und wie viele aus einer Stichprobe fester Größe dieser überfälligen Mitglieder keine Nutzlast mehr haben:

| `status` | Bedeutung |
| --- | --- |
| `ok` | Nichts ist überfällig. |
| `behind` | Überfällige Mitglieder, alle mit intakter Nutzlast — der Promoter hinkt hinterher, was sich allein durch Zeit löst. |
| `orphaned` | Mindestens ein überfälliges Mitglied hat keine Nutzlast. Nichts kann es zustellen und nichts wird es zurückfordern. |
| `unknown` | Die Sonde konnte Redis nicht lesen. |

Bei einer gesunden Queue sind das zwei `O(log N)`-Lesezugriffe auf die sortierte Menge; die Stichprobe wird nur genommen, wenn etwas überfällig ist, und wächst nie mit dem Rückstand, den sie beschreibt. `orphaned` wird vom MTA als Warnung protokolliert und von `owlat doctor` benannt. Es fließt bewusst **nicht** in das übergeordnete `degraded`-Urteil des Health-Endpunkts ein: Ein Leck in der Wiederholungsleiter ist kein Grund, einem Betreiber zu sagen, sein Zustellpfad sei ausgefallen.
...
| TLS-Handshake-Fenster | 30 s (nur beim Implicit-TLS-Listener) |
...
| `SUBMISSION_IMPLICIT_TLS_ENABLED` | `false` | Zusätzlich auf einem Implicit-TLS-Port lauschen, der ab dem ersten Byte verschlüsselt ist (RFC 8314) |
| `SUBMISSION_IMPLICIT_TLS_PORT` | `465` | Port für die Implicit-TLS-Submission |

**Handshake-Fenster.** Beim Implicit-TLS-Listener findet der TLS-Handshake statt, *bevor* eine SMTP-Sitzung existiert; der Kommando-Leerlauf-Timer, der eine hängende 587-Verbindung begrenzt, ist also noch nicht scharf. Eine Gegenstelle, die den TCP-Handshake abschließt und danach nichts sendet, wird stattdessen durch ein **30-Sekunden**-Fenster für den TLS-Handshake begrenzt; danach wird die Verbindung protokolliert und verworfen. Das Fenster ist ein fester Richtwert und keine Umgebungsvariable: Es ist weit genug, damit eine verlustbehaftete Mobilfunkstrecke ihr ClientHello mehrfach erneut senden kann (eine TCP-RTO-Staffel kostet bei drei Verlusten rund 7 s und bei vier rund 15 s), und deutlich enger als die 120 Sekunden von Node, mit denen eine nicht authentifizierte Gegenstelle zwei Minuten lang einen Verbindungsplatz belegen kann. Der 587-Listener bleibt davon unberührt — dessen Schweigen vor STARTTLS begrenzt der Kommando-Leerlauf-Timer.
...
Der Bounce-Server spiegelt die Dispatch-Architektur: Eine typisierte Phasen-Pipeline (`bounce/pipeline.ts` + `bounce/phases/`) klassifiziert jede Nachricht in einen `BounceAttempt`, und der reine Reducer in `bounce/outcome.ts` gibt eine typisierte Liste von `BounceEffect`s aus (Convex-Webhooks, Circuit-Breaker-Ergebnisse, Kontingenterhöhungen). Anhangs-Bytes sind nie darunter. Auf dem Weg für geroutete eingehende Mail (`inbound.received`) trägt die Payload **nur Metadaten** — Dateiname, Content-Type und Größe — und die Bytes stehen nachgelagert überhaupt nicht zur Verfügung. Nur der Weg zum persönlichen Postfach (`inbound.mailbox.received`) behält sie: Er überträgt die vollständige Nachricht als rohes RFC822, das Convex speichert und aus dem der Postbox-Reader MIME-Teile extrahiert.
...
| `SUBMISSION_IMPLICIT_TLS_ENABLED` | `false` | Den Implicit-TLS-Submission-Listener aktivieren (Port 465); sein TLS-Handshake-Fenster beträgt fest 30 s |
| `SUBMISSION_IMPLICIT_TLS_PORT` | `465` | Port für die Implicit-TLS-Submission |
