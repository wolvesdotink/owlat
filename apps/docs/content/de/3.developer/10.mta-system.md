...
| `GET` | `/queue/pending` | Master | Begrenzte Stichprobe der eingereihten Jobs (`?limit=`, max. 200; `?domain=`) |
...
bleibt während seiner Wartezeit in seiner Gruppe — `delayed` ist also eine
Teilmenge von `waiting` und kein Geschwisterwert; die beiden addieren sich
nicht. Nachrichteninhalte werden nie zurückgegeben: beide Job-Endpunkte melden
nur identifizierende Metadaten und Größenangaben.

`/queue/pending` liefert eine begrenzte Stichprobe statt einer Seite — es meldet
`waiting` (die gesamte Queue) neben `sampled` (was der Scan gesehen hat), damit
eine Teilansicht nie mit einer leeren verwechselt wird.
...
entlang derer sich leeren ließe; die einzige Umsetzung müsste jede eingereihte
Payload lesen — unbegrenzte Arbeit genau auf dem Rückstau, für den man zu einem
solchen Aufruf greift, und sie würde zustellbare Mail mit einer einzigen
unbestätigten Anfrage vernichten. Entfernen Sie Jobs einzeln, oder siehe
[Wartung & Updates](/developer/self-hosting-maintenance) zum vollständigen
Leeren der Queue.
