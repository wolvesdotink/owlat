...
| `soft_bounce` | Nach wiederholten temporären Zustellfehlern ergänzt |
...
Die Aufbewahrung unterscheidet sich bewusst nach Grund. `hard_bounce`, `complaint` und `manual` laufen nie ab; eine ausdrückliche Sperre bleibt bestehen, bis sie ausdrücklich entfernt wird. Nur `soft_bounce` läuft nach sieben Tagen ab. Einzel- und Bulk-Endpunkt wenden dieselbe Regel an.
...
Diese Liste ist die letzte Absicherung vor dem Versand. Die Blockliste von Owlat verwaltet ihre eigenen Spiegelkopien: Jeder dort ergänzte Bounce, jede Beschwerde und jede manuelle Sperre wird hierher gespiegelt (eine `unengaged`-Sunset-Sperre nicht — sie darf transaktionale Mail nicht blockieren). Das Entfernen einer gespiegelten Sperre in der App entfernt auch ihre MTA-Kopie. Ein täglicher Abgleich baut nach vorübergehenden Fehlern fehlende Spiegelkopien neu auf und entfernt veraltete, von Convex verwaltete Kopien. Unabhängig im MTA entstandene Sperren (einschließlich Hard Bounces persönlicher Mail) und Einträge ohne Metadaten bleiben erhalten. Bei Soft Bounces bleibt die ursprüngliche Frist von sieben Tagen bestehen; der Abgleich verlängert sie nicht.
