...
| `apps/convex-fn-proxy` | Sidecar mit minimalen Rechten vor Convex für den Code-Worker: hält den Deployment-Admin-Key und leitet NUR die freigegebenen `codeWorkTasks`/`plugins/workerTasks`-Funktionen des Workers weiter, damit die Sandbox nie einen Admin-Key besitzt |
...
| `packages/email-builder` | Vue-Komponenten des E-Mail-Builders (Notion-artiger Editor) und die Client-Vorschau unter `src/preview/` (Kompatibilitätsanalyse, Can-I-Email-Daten) |
