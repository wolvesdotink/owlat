...
### Redis-Speichergrenzen (optional)

Zwei voneinander unabhängige Obergrenzen für das Redis, das sich MTA und IMAP-Auth-Bremse teilen. Die Standardwerte passen zu einer kleinen Single-Box-Installation (4-8 GB RAM), die einige tausend Nachrichten pro Tag versendet.

Bemessen Sie `REDIS_MAXMEMORY` an Ihrem Sendevolumen und nicht an einer festen Vorstellung von „gesund“. Jeder Zustellversuch hinterlässt rund fünf Idempotenz-Belege — die Datensätze, die verhindern, dass ein wiederholter Versuch einen Bounce doppelt zählt oder eine Nachricht erneut versendet — und diese werden sieben Tage aufbewahrt, fünfunddreißig Tage beim Nenner der Kampagnen-Beschwerderate. Bei einigen hundert Byte pro Beleg sind das grob 1,5 KB je Versuch: Ein dauerhaftes Volumen in der Größenordnung von 25.000 bis 30.000 Versuchen pro Tag nähert sich allein durch die Belege den 512 MB. Darunter sind einige zehn Megabyte normal und das Erreichen der Grenze bedeutet ein Leck; darüber erhöhen Sie beide Grenzen gemeinsam.

Ein Teil des Keyspace ist von Natur aus dauerhaft und schrumpft nie von selbst: die Suppression-Liste. Ein Hard Bounce oder eine Spam-Beschwerde darf nicht ablaufen, deshalb kostet jede DAUERHAFT unterdrückte Adresse für die gesamte Lebensdauer der Installation rund 300 Byte — etwa 30 MB je 100.000 Adressen und bei einer Million mehr als die Hälfte einer 512-MB-Grenze. (Ein befristeter Eintrag kostet rund ein Drittel mehr, etwa 400 Byte, weil er zusätzlich im Fälligkeitsindex steht; befristete Einträge sind aber begrenzt, denn ein stündlicher Sweep räumt sie ab.) Fragen Sie die aktuellen Anzahlen je Grund mit `GET /suppression/stats` beim MTA ab (siehe [MTA-System](/developer/mta-system)) und rechnen Sie diesen Sockel heraus, bevor Sie auf ein Leck schließen — eine lang laufende Instanz mit großer Suppression-Liste trägt schlicht die Liste, die sie tragen soll. Macht allein die Liste einen großen Teil der Grenze aus, erhöhen Sie **beide** Grenzen gemeinsam wie oben: `REDIS_MAXMEMORY` allein in Richtung der cgroup-Obergrenze zu ziehen ist genau der Weg, auf dem der Fork beim Neuschreiben der Append-only-Datei den Container vom OOM-Killer beenden lässt — der Fall, den `REDIS_MEM_LIMIT` verhindern soll. Die Adressen liegen zusätzlich dauerhaft in der Blockliste von Owlat; diese Kopie ist also eine Absicherung und nicht der maßgebliche Datensatz.
...
| Variable | Beschreibung | Standard |
| --- | --- | --- |
| `REDIS_MAXMEMORY` | Redis' eigene `--maxmemory`-Grenze, damit Redis reagiert, bevor die cgroup des Containers es tut. Die Policy ist fest auf `noeviction` gesetzt: diese Instanz mischt entbehrliche Zähler der Rate-Limiter mit Dauerhaftigkeits-Belegen, die die MTA nicht verlieren darf (Defer-Handoffs, Annahme-Belege, Idempotenz-Belege), und jeder davon trägt eine TTL — keine Eviction-Policy kann sie also auseinanderhalten. An der Grenze **verweigert Redis Schreibzugriffe**, statt zu verdrängen: ein verweigerter Schreibzugriff wird wiederholt und ist in den MTA-Logs sichtbar (`OOM command not allowed`), ein verdrängter Handoff-Beleg ist eine still doppelt versendete E-Mail. | `512mb` |
| `REDIS_MEM_LIMIT` | Die cgroup-Obergrenze des Containers. Sie liegt deutlich über `REDIS_MAXMEMORY`, weil das Neuschreiben der Append-only-Datei forkt und dessen Copy-on-Write kurzzeitig fast das Doppelte des Datenbestands belegen kann. Sie sorgt dafür, dass ein Überschießen nur Redis beendet, statt den OOM-Killer des Kernels ein Opfer auf der ganzen Maschine aussuchen zu lassen. | `1g` |
...
# REDIS_URL=                           # von docker-compose.yml gesetzt; in Produktion erforderlich
```
...
::callout{type="warning" title="Drosselung der IMAP-Anmeldung ist in Produktion Pflicht"}
Port 993 ist aus dem Internet erreichbar, deshalb **startet der IMAP-Server in Produktion nicht** ohne `REDIS_URL` — die Variable hinterlegt den Speicher für die Brute-Force-Bremse am LOGIN (`apps/imap/src/config.ts`). Beide ausgelieferten Compose-Dateien setzen sie inzwischen für Sie und zeigen auf dasselbe Redis wie die MTA; von Hand setzen müssen Sie sie nur in einer selbst gebauten Installation. Wer den Listener ganz ohne Brute-Force-Schutz betreiben will, muss das ausdrücklich mit `IMAP_ALLOW_UNTHROTTLED_AUTH=true` erklären.

Ein Redis, das zwar konfiguriert, aber vorübergehend nicht erreichbar ist, schaltet weiterhin bewusst **fail-open**: Dieser Ausfall ist vorübergehend, und alle Nutzerinnen und Nutzer von ihrer Mail auszusperren, nur weil der Cache kurz gestolpert ist, wäre der schlimmere Fehler.
::
...
#### Google-Anmeldung für Gmail-Postfächer (optional)

Gmail- und Google-Workspace-Postfächer lassen sich statt mit einem App-Passwort per **Google-Anmeldung** verbinden (OAuth 2.0 Authorization Code + PKCE). Owlat speichert dabei nur das Refresh-Token — verschlüsselt im selben Umschlag wie bei jedem anderen externen Konto — und der Mail-Sync-Worker authentifiziert sich gegenüber `imap.gmail.com` / `smtp.gmail.com` per SASL XOAUTH2.

Das ist **optional und ergänzend** — ob Sie es einrichten, entscheiden Sie als Betreiber. Der Standardweg ist das App-Passwort und erfordert von Ihnen keinerlei Einrichtung: Die Gmail-Karte zeigt immer das Formular aus E-Mail-Adresse und App-Passwort, genau wie vor dieser Funktion. Ein konfigurierter Client *ergänzt* dieses Formular lediglich um eine Schaltfläche „Mit Google anmelden“, die Nutzerinnen und Nutzer statt eines App-Passworts verwenden können. Überspringen Sie diesen Abschnitt, geht nichts verloren.

| Variable | Beschreibung |
| --- | --- |
| `GOOGLE_OAUTH_CLIENT_ID` | Client-ID des Google-OAuth-Clients. Nicht gesetzt ⇒ die Google-Schaltfläche wird nicht gerendert; die Gmail-Karte zeigt wie gewohnt ihr App-Passwort-Formular. |
| `GOOGLE_OAUTH_CLIENT_SECRET` | Client-Secret desselben Clients. Wird ausschließlich serverseitig beim Token-Austausch und -Refresh verwendet. Nicht gesetzt ⇒ wie oben. |

**Den Google-OAuth-Client anlegen.** In der [Google Cloud Console](https://console.cloud.google.com/):

1. **APIs & Dienste → OAuth-Zustimmungsbildschirm** — **Intern** für eine Google-Workspace-Organisation (keine Verifizierung nötig) oder **Extern** für Consumer-Gmail-Konten.
2. Die Scopes `https://mail.google.com/`, `openid` und `email` hinzufügen. `https://mail.google.com/` ist der vollständige IMAP-/SMTP-Scope und der einzige, der XOAUTH2 autorisiert.
3. **Anmeldedaten → Anmeldedaten erstellen → OAuth-Client-ID → Webanwendung**, als autorisierte Weiterleitungs-URI `${SITE_URL}/oauth/google/callback` eintragen (z. B. `https://mail.example.com/oauth/google/callback`). Sie muss zeichengenau übereinstimmen; einen abschließenden Schrägstrich in `SITE_URL` entfernt Owlat selbst.
4. Client-ID und Secret als Convex-Deployment-Umgebungsvariablen setzen (das Setup-CLI und der Web-Einrichtungsassistent übernehmen das).

::callout{type="warning" title="Der Gmail-Scope ist eingeschränkt"}
`https://mail.google.com/` ist ein **eingeschränkter** (restricted) Scope. Ein **externer** Zustimmungsbildschirm im Modus *Testing* funktioniert für bis zu 100 benannte Testnutzerinnen und -nutzer, was für eine kleine selbst gehostete Installation ausreicht; für die Freigabe an alle verlangt Google eine App-Verifizierung inklusive Sicherheitsprüfung. **Interne** (Workspace-)Apps brauchen keine Verifizierung.
::
...
### Optional: Vertrauensregister für Absender (Flag `ostr`)

Das Open Sender Trust Registry liefert überprüfbare Reputationsdaten zu Domains und IP-Adressen. Setzen Sie URL und Verifikationsschlüssel gemeinsam oder lassen Sie beide ungesetzt. Die Convex-Werte dienen der Konfigurationsanzeige; die eigentliche Abfrage läuft auf der MTA, die dieselben Werte benötigt.

| Variable | Beschreibung | Standard |
| --- | --- | --- |
| `OSTR_AGGREGATOR_URL` | Basis-URL des Aggregators. Ohne URL wird kein signierter Snapshot abgefragt. | — |
| `OSTR_AGGREGATOR_PUBLIC_KEY` | Ed25519-Schlüssel zur Prüfung der signierten Antworten. Nicht prüfbare Antworten werden verworfen. | — |

Aktivieren Sie die MTA-Abfrage über `OSTR_ENABLED` mit dem Wert `true` (standardmäßig aus). `OSTR_ZONE` legt die DNS-Zone fest; alternativ oder zusätzlich wird das Aggregator-Paar verwendet. `OSTR_LOOKUP_TIMEOUT_MS` begrenzt Abfragen auf standardmäßig `2000` Millisekunden. Bei Fehlern bleibt die Zustellung unverändert.

Nur die Stufe `flagged` kann Nachrichten bei aktiviertem Flag `ostr` nach Spam verschieben; `warned` und niedrigere Stufen verändern die Zustellung nicht. Die aufgelöste Stufe wird unabhängig vom Flag gespeichert, damit sich die Wirkung vor dem Aktivieren prüfen lässt. Das Flag benötigt `postbox`.

### Optional: Vertrauensregister — Beobachtermodus

Der Beobachtermodus veröffentlicht signierte, dauerhaft protokollierte Beobachtungen über empfangene Nachrichten. Er ist standardmäßig aus und bleibt unterhalb der Mindestzahl an Postfächern gesperrt. Bei kleinen Installationen könnte bereits die Beobachteridentität verraten, wer eine Nachricht als Spam gemeldet hat. Solche Instanzen können das Register ohne eigene Veröffentlichungen nutzen.

| Variable | Beschreibung | Standard |
| --- | --- | --- |
| `OSTR_OBSERVER_ENABLED` | Aktiviert die Speicherung von DKIM-Belegen und die Veröffentlichung ausreichend aggregierter Beobachtungen. Sowohl in Convex als auch auf der MTA setzen; ausgeschaltet werden keine Belege gesammelt oder Meldungen veröffentlicht. | `false` |
| `OSTR_OBSERVER_DOMAIN` | FQDN der signierenden Instanz. Unter `_ostr.<domain>` muss der passende öffentliche Schlüssel stehen. Ohne Identität wird nichts veröffentlicht. | — |
| `OSTR_OBSERVER_PRIVATE_KEY` | Privater Ed25519-Schlüssel, 32 Bytes in Base64. Geheim halten; ohne Schlüssel wird nichts veröffentlicht. | — |
| `OSTR_LOG_URLS` | Kommagetrennte vollständige Einreichungs-URLs, etwa `https://log-a.example/v1/attestations,https://log-b.example/v1/attestations`. Mindestens zwei Logs bieten Redundanz. | — |
| `OSTR_MIN_MAILBOXES` | Darf die integrierte Mindestzahl an Postfächern nur erhöhen; niedrigere Werte werden ignoriert. | integrierte Mindestzahl |

DKIM-Belege müssen bei der Zustellung erfasst werden: Nach einem Schlüsselwechsel lassen sie sich nicht zuverlässig rekonstruieren. Bereits früher empfangene Nachrichten liefern daher keine nachträglichen Belege. Die Belege mit signierten Betreff- und Empfänger-Headern bleiben auf der Instanz und werden ungefähr 90 Tage aufbewahrt. Veröffentlicht werden ausreichend aggregierte Zählwerte und kryptografische Verpflichtungen, keine Nachrichteninhalte. Zu kleine Gruppen werden zurückgehalten.
...
| `OSTR_AGGREGATOR_URL` | Convex | Wenn `ostr` aktiv | — | Aggregator-URL; auch auf der MTA setzen |
| `OSTR_AGGREGATOR_PUBLIC_KEY` | Convex | Wenn `ostr` aktiv | — | Ed25519-Verifikationsschlüssel für signierte Antworten |
| `OSTR_OBSERVER_ENABLED` | Convex | Nein | `false` | Beobachtermodus, zusätzlich durch die Postfach-Mindestzahl begrenzt |
| `OSTR_OBSERVER_DOMAIN` | Convex | Bei Beobachtermodus | — | FQDN der signierenden Instanz mit öffentlichem Schlüssel unter `_ostr.<domain>` |
| `OSTR_OBSERVER_PRIVATE_KEY` | Convex | Bei Beobachtermodus | — | Privater Ed25519-Schlüssel in Base64; niemals veröffentlichen |
| `OSTR_LOG_URLS` | Convex | Bei Beobachtermodus | — | Vollständige Einreichungs-URLs; mindestens zwei für Redundanz |
| `OSTR_MIN_MAILBOXES` | Convex | Nein | integrierte Mindestzahl | Kann die Mindestzahl an Postfächern nur erhöhen |
...
| `TYPESAFE_API_KEY` | API-Schlüssel für den Entscheidungs-Provider TypeSafe (Jev) — die dritte KI-Ebene, die typisierte Fragen mit kalibrierten Wahrscheinlichkeiten beantwortet (`lib/decisionProvider.ts`). Das deploymentweite Gegenstück zu dem Schlüssel, den eine Administratorin auf der Seite „KI-Provider“ hinterlegen kann; ein gespeicherter Schlüssel hat Vorrang. **Vollständig optional**: Ohne Schlüssel löst die Entscheidungsebene auf den sprachmodellgestützten Adapter auf — also genau das Verhalten einer Installation, die diese Ebene nie kannte. | — |
| `DECISION_PROVIDER` | Welcher Entscheidungs-Adapter antwortet: `typesafe` oder `llm`. Wird nur herangezogen, wenn die gespeicherte Konfiguration keinen Adapter nennt; ein unbekannter Wert wird ignoriert statt abgelehnt, sodass ein Tippfehler auf die Sprachebene zurückfällt, statt den Eingangspfad scheitern zu lassen (`lib/decisionProvider.ts`). | nicht gesetzt (`llm`) |
| `DECISION_MODEL` | Überschreibt die Modell-ID der Entscheidungsebene — die hier gesetzte ID wird auch gesendet. Nicht gesetzt ⇒ die vom Adapter fest gepinnte Version. Eine andere Version zu nennen ist erlaubt und nie stillschweigend: Eine Antwort, die der Anbieter zu einer anderen als der gepinnten Version (oder einem ihrer Aliase) meldet, gilt als **unkalibriert**, sodass alle nachgelagerten Schwellenwerte ruhen, bis die Kalibrierung erneut gelaufen ist. Diese eine Regel deckt den bewussten Versionswechsel und eine anbieterseitige Umleitung gleichermaßen ab. | nicht gesetzt (die gepinnte Version des Adapters) |
| `DECISION_BASE_URL` | Überschreibt den **Origin** der Entscheidungs-API, z. B. einen selbst betriebenen Proxy vor dem Anbieter. Der Endpunktpfad wird vom Adapter angehängt; hier gehört also ein Origin (`https://api.example.com`) hin, keine vollständige Endpunkt-URL. Jeder ausdrückliche Wert kennzeichnet den Endpunkt für die Kostenzulassung als `custom`. | nicht gesetzt (der eigene Origin des Adapters) |
...
| `REDIS_URL` | Redis-URL, die die Brute-Force-Bremse am LOGIN hinterlegt. Von beiden ausgelieferten Compose-Dateien gesetzt; **in Produktion erforderlich** — ohne sie startet der Server nicht. | — |
| `IMAP_ALLOW_UNTHROTTLED_AUTH` | Genau `true`, um in Produktion ohne Brute-Force-Schutz am LOGIN zu starten. Einziger Ausweg aus der `REDIS_URL`-Pflicht. | `false` |
| `IMAP_RUNTIME_USER` | `uid:gid` auf Compose-Ebene, das den veröffentlichten TLS-Schlüssel lesen können muss. Wer das Zertifikat schreibt (`imap-cert-init`, auf dem VPS `acme-entrypoint.sh`), übereignet es diesem Wert. Nur ändern, wenn Sie das Image mit einem anderen Benutzer gebaut haben — bei Abweichung scheitert der Server mit EACCES am eigenen Schlüssel und startet endlos neu. | `1000:1000` |
...
| `GOOGLE_OAUTH_CLIENT_ID` | Convex | Nein | — | Google-OAuth-Client-ID für das Verbinden von Gmail per Google-Anmeldung; nicht gesetzt ⇒ nur App-Passwort-Formular |
| `GOOGLE_OAUTH_CLIENT_SECRET` | Convex | Nein | — | Secret desselben Clients, ausschließlich für den serverseitigen Token-Austausch/-Refresh |
...
| `TYPESAFE_API_KEY` | Convex | Nein | — | API-Schlüssel für die Entscheidungsebene TypeSafe (Jev); ein auf der Seite „KI-Provider“ hinterlegter Schlüssel hat Vorrang |
| `DECISION_PROVIDER` | Convex | Nein | `llm` | Entscheidungs-Adapter (`typesafe`, `llm`); wird nur gelesen, wenn die gespeicherte Konfiguration keinen nennt |
| `DECISION_MODEL` | Convex | Nein | gepinnte Version | Überschreibt die Modell-ID der Entscheidungsebene |
| `DECISION_BASE_URL` | Convex | Nein | — | Überschreibt den Origin der Entscheidungs-API (nur Origin — den Endpunktpfad hängt der Adapter an) |
...
| `NUXT_PUBLIC_OFFLINE_SHELL` | Nuxt-`.env` | Nein | `true` | Service Worker für die Offline-App-Shell. Auf `false` setzen (zur Buildzeit), um ihn zu deaktivieren und in Browsern, die ihn bereits haben, zu deregistrieren |
...
| `REDIS_URL` | IMAP-Env | In Produktion ja | — | Redis-URL, die die Brute-Force-Bremse am LOGIN hinterlegt. Von beiden ausgelieferten Compose-Dateien gesetzt; ohne sie startet der Server in Produktion nicht. |
| `IMAP_ALLOW_UNTHROTTLED_AUTH` | IMAP-Env | Nein | `false` | Genau `true`, um in Produktion ohne Brute-Force-Schutz am LOGIN zu starten |
| `IMAP_RUNTIME_USER` | Compose-Env | Nein | `1000:1000` | `uid:gid`, dem der veröffentlichte TLS-Schlüssel übereignet wird; weicht er vom Benutzer des Images ab, startet der Server wegen EACCES endlos neu |
