...
Ohne das aktivierte [Feature-Flag](/guide/feature-flags) `analytics.posthog` wird nichts erfasst. Das Flag ist standardmäßig aus, und ein hinterlegter Schlüssel setzt es nicht außer Kraft: Bei ausgeschaltetem Flag lädt und initialisiert der Browser PostHog gar nicht erst, und das Backend verwirft jedes Ereignis, das es sonst senden würde. Schalten Sie das Flag später wieder aus, endet die Erfassung auf beiden Seiten — der Browser meldet sich ab und verwirft die gespeicherte Identität.
...
| Variable | Beschreibung |
| --- | --- |
| `POSTHOG_API_KEY` | Projekt-API-Schlüssel von PostHog (beginnt mit `phc_`). Erforderlich für serverseitiges Tracking — zusätzlich zum Flag `analytics.posthog`. |
...
| Variable | Beschreibung |
| --- | --- |
| `NUXT_PUBLIC_POSTHOG_API_KEY` | Projekt-API-Schlüssel von PostHog. Erforderlich für clientseitiges Tracking — zusätzlich zum Flag `analytics.posthog`. |
...
- Nutzeridentifikation und Zuordnung zur Organisationsgruppe, ausschließlich über IDs — weder E-Mail-Adresse noch Anzeigename, Workspace-Name oder Slug werden übertragen. PostHog enthält Produktanalytik, keine Kopie Ihres Nutzerverzeichnisses; die menschenlesbaren Bezeichnungen verknüpfen Sie bei Bedarf aus Owlats eigener Datenbank.
...
## Authentifizierte Datei-Uploads

Browser und Desktop laden Dateien über eine einmalig verwendbare URL unter
`${SITE_URL}/api/storage/upload` hoch. Der Webserver streamt die Datei direkt zur
nativen Convex-Upload-URL und bestätigt anschließend die Benutzer- und
Organisationszuordnung. Eine Speicher-ID allein berechtigt nicht dazu, eine Datei
mit einer Ressource zu verknüpfen.

`INSTANCE_SECRET` muss im Webserver und in Convex übereinstimmen. Fehlt dieses
Geheimnis oder stimmt es nicht überein, werden Uploads abgelehnt. Während einer
Rotation akzeptiert Convex auch `INSTANCE_SECRET_PREVIOUS`. `SITE_URL` in Convex
muss die öffentlich erreichbare Webadresse sein, auch für Desktop-Clients. Der
Webserver verwendet die kleinen Steuerendpunkte `POST /storage/upload/begin`,
`/storage/upload/finish` und `/storage/upload/abort` am konfigurierten Convex-HTTP-Ursprung.
Sie erfordern das gemeinsame Geheimnis; Browser-Sitzungen oder Upload-Tokens allein
reichen nicht aus.

Nur der Web-Upload-Endpunkt benötigt Browser-CORS. Die Dateidaten durchlaufen keine
Convex-HTTP-Action, damit die bestehenden Grenzen von 50 MiB für Bibliotheksdateien
und 64 MiB für Archive erhalten bleiben. Nach einem Fehler wird eine neue URL
benötigt. Unbenutzte Upload-Berechtigungen und noch nicht zugeordnete Dateien
verfallen nach einer Stunde und werden regelmäßig entfernt.
