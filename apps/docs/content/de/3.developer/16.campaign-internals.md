...
Der Reducer lehnt jeden Übergang ab, der nicht im Graphen `CAMPAIGN_LIFECYCLE` steht. Es wird keine Exception geworfen — ein unzulässiger Aufruf gibt `{ ok: false, reason: 'illegal_edge' }` zurück, und ein Übergang aus einem terminalen Status gibt `reason: 'terminal'` zurück. Die Aufrufer übersetzen das Ergebnis in eine Antwort an die Nutzerin.
...
War die Kampagne `cancelled` oder `draft` (ungeplant) oder bereits `sent`, gibt der Orchestrator `skipped: true` mit einem Grund zurück. Bei `sending` überspringt er **nicht** — die Kampagne kommt durch den Lifecycle-Übergang bereits auf `sending` umgestellt an, und Übergänge in denselben Zustand lösen den Orchestrator-Effekt nicht erneut aus.
