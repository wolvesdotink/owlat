...
- `GET /attachment-share/:token` — lädt eine aus einer Nachricht geteilte Datei herunter. Das Token wird auf Widerruf, Ablauf, den Geltungsbereich `anyone` und darauf geprüft, ob die Bytes noch existieren. Jede Ablehnung ist dieselbe `404` im Fehler-Envelope, sodass sich über die Route nicht herausfinden lässt, welche Token echt sind. Die Antwort wird als Download erzwungen (`Content-Disposition: attachment`, `X-Content-Type-Options: nosniff`) und setzt `Cache-Control: no-store`.
