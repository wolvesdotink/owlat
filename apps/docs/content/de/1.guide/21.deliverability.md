...
3. Wählen Sie, **wer die Mail für diese Domain empfängt** (siehe unten).
4. Owlat generiert die DNS-Einträge, die Sie hinzufügen müssen.

### Wer empfängt die Mail für diese Domain?

Eine Versanddomain hinzuzufügen bedeutet nicht, Owlat Ihren Posteingang zu übergeben. Das Formular stellt genau eine Frage, und die Antwort bestimmt, welche Einträge Owlat erzeugt:

| Auswahl                                         | Bedeutung                                                                                                                                                                          |
| ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Owlat empfängt sie**                          | Owlat wird zum Mailserver für die Domain. Sie veröffentlichen einen MX-Eintrag, der auf Ihr Deployment zeigt, und eingehende Mail landet in Owlat. Das ist die Voreinstellung.     |
| **Mein bisheriger Anbieter behält den Empfang** | Owlat **versendet** nur. Google Workspace, Microsoft 365 oder wer auch immer heute Ihre Mail bearbeitet, behält jede eingehende Nachricht, und Ihre MX-Einträge bleiben unberührt. |

::callout{type="warning" title="Für eine reine Versanddomain nie einen MX-Eintrag veröffentlichen"}
Wenn Ihre Mail bei Google Workspace oder Microsoft 365 liegt, nimmt ein MX-Eintrag auf der Apex-Domain, der auf Ihr Owlat-Deployment zeigt, Ihrem Anbieter **die gesamte** eingehende Mail weg — und bis Sie es merken, hat sich der Eintrag verbreitet. Wählen Sie **Mein bisheriger Anbieter behält den Empfang**, dann verlangt Owlat für diese Domain nie einen MX-Eintrag. Das Einrichtungspanel ersetzt die MX- und Verschlüsselungsrichtlinien-Hinweise durch die Erinnerung, Ihre MX-Einträge in Ruhe zu lassen, und prüft live, wohin die Mail der Domain tatsächlich zugestellt wird — einschließlich des lauten Falls, dass der Apex-MX bereits auf Ihre Owlat-Instanz zeigt.
::

Aus der reinen Versandwahl folgen zwei Details:

- **SPF wird für Sie zusammengeführt.** Der erzeugte SPF-Eintrag auf der Apex-Domain trägt die Freigabe Ihres Anbieters _und_ die von Owlat in einem einzigen Eintrag. Veröffentlichen Sie ihn genau so; ein zweiter Eintrag, der mit `v=spf1` beginnt, ist ein dauerhafter Fehler, der SPF für alles bricht, was Sie versenden. Wählen Sie **Anderer Anbieter**, kennt Owlat keine Freigabe zum Zusammenführen — der erzeugte Eintrag autorisiert dann nur Owlat, und das Zusammenführen mit Ihrem bestehenden SPF-Eintrag übernehmen Sie.
- **Bounces laufen weiterhin über eine Subdomain.** Zustellberichte kommen über einen eigenen Return-Path-Namen zurück (zum Beispiel `bounce.example.com`), der von der Apex-Domain getrennt ist. Ihr MX-Eintrag ist daran nie beteiligt.

Sie können die Antwort später im Panel der Domain ändern. Dabei wird der SPF-Eintrag neu erzeugt und die Domain auf **ausstehend** zurückgesetzt; veröffentlichen Sie den aktualisierten Eintrag erneut und verifizieren Sie noch einmal. Owlat bearbeitet Ihr DNS nie selbst. Eine Ausnahme: Versendet die Domain über ein externes Relay wie Amazon SES, gehört ihr SPF-Eintrag diesem Relay, und Owlat schreibt ihn nicht um. Das Panel nennt dann das `include:`, das Sie selbst ergänzen, statt die Zusammenführung zu behaupten.

Um das Postfach des Anbieters in Owlat zu lesen, verbinden Sie es über IMAP unter **Postbox > Einstellungen > Externes Postfach** — die Zustellung bleibt bei Ihrem Anbieter, und Owlat synchronisiert eine Kopie. Um auch Ihre alte Mail zu übernehmen, siehe [Von Google migrieren](/guide/migrate-from-google).
