...
Der einmalige **Backfill-Job** (`apps/api/convex/agent/knowledgeBackfill.ts`) durchläuft beim erstmaligen Umschalten des Feature-Flags `ai.agent` von false auf true (`setFeatureFlag` in `apps/api/convex/workspaces/featureFlags.ts`) weiterhin die bestehenden `inboundMessages` des Deployments, damit der Entwurfsschritt von Tag eins an historischen Kontext hat. Interner Chat zwischen MEMBERn (`unifiedMessages.sendChatMessage`) löst **keine** Extraktion aus — das tun nur Nachrichten der Eingangsstrecke.
::
