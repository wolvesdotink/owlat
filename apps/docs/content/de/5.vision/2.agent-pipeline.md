...
`POST /webhooks/mta` transportiert Zustellereignisse **und** das Ereignis `inbound.received` für das Agenten-Postfach. Die separate Route `POST /webhooks/mta-mailbox` gehört zu [Postbox](/guide/postbox) (persönliche Mail pro Benutzerin und Benutzer) und läuft über `mail/webhookHttp.ts`, nicht über die Agenten-Strecke.
::
