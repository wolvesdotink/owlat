...
Die Schrittarten sind exakt: `security_scan`, `context_retrieval`, `classify`, `clarify`, `draft`, `route` (`AgentStepKind` in `agent/steps/catalog.ts`). Ein `plan`-Schritt war entworfen, wurde aber **mit ADR-0014 vor der Produktion verworfen** — es gibt keine Stufe zur Aktionsplanung. Nicht jede Nachricht durchläuft die ganze Kette: Post ohne Antwortbedarf endet nach `classify` (siehe [ADR-0061](/developer/decisions)).
...
```typescript
{
  ...
  kind: 'personal' | 'update' | 'notification' | 'receipt'
      | 'newsletter' | 'advertising',  // Art der Nachricht, unabhängig von der Kategorie
  needsResponse: boolean,  // wartet der Absender auf eine Antwort?
  language: string,        // ISO-639-1-Code der Sprache des Absenders
  importance: number,      // 0–1, wie wichtig die Nachricht für den Empfänger ist
  summary: { en: string, de: string },  // ein Satz je Oberflächensprache
}
```

Der Prompt rahmt den zusammengestellten Kontext als nicht vertrauenswürdige Daten (`SYSTEM_GUARD`); jedes Freitextfeld, das das Modell liefert, wird vor dem Speichern begrenzt und bereinigt, und `language` wird auf einen reinen Sprachcode geprüft, bevor der Entwurfsschritt ihn in seine Systemrolle übernehmen darf.

Routing: `spam` (oder eine passende `auto_archive`-Handling-Regel) → `archived`; ein **sicheres Urteil „keine Antwort nötig“** → `informational` (die Strecke endet; die Nachricht erscheint nach `importance` sortiert auf dem **Neuigkeiten**-Dashboard des Team-Postfachs, nach `kind` in die Reiter Neuigkeiten / Werbung / Benachrichtigungen aufgeteilt, wo ein Leser sie verwerfen oder doch einen Entwurf anfordern kann; ein Spam-Reiter zeigt, was als Spam archiviert wurde, und bietet das Sperren des Absenders an). Die vier Massen-Arten — notification, receipt, newsletter, advertising — sind unabhängig von ihrem Intent-Label informativ; alles Übrige → der `clarify`-Schritt, im selben Zustand. Die Leitplanken der Informations-Abzweigung sind bewusst einseitig: eine Beschwerde, alles Dringende, eine Eskalation, eine Frage oder eine Bitte nimmt immer den Antwortpfad, egal was der Boolean sagt, und einem Urteil unter 0,6 Konfidenz wird nicht getraut. Die Klassifizierung wird in `inboundMessages.classification` abgelegt.

### Schritt 3b: Rückfrage

`apps/api/convex/agent/steps/clarify/index.ts` — die Lücken-Prüfung zwischen Klassifizierung und Entwurf. Sie extrahiert die *Antwort-Slots*, die eine gute Antwort füllen muss (eine Entscheidung, ein Datum, ein Preis, ein Anhang, eine Haltung, ein Fakt), zieht einige Antwortkandidaten und behält nur die Slots, in denen sie wirklich uneins sind. Jede verbleibende Frage trägt die **Antwortvorschläge** des Modells als Auswahl-Chips und wird **in jede Oberflächensprache übersetzt** (`inbox/clarificationLocalize.ts`), sodass die Person immer in ihrer eigenen Sprache gefragt wird, während die kanonische englische Fassung gespeichert und vom Antwortgedächtnis abgeglichen wird.

Bleiben Fragen offen, parkt die Nachricht in `awaiting_clarification`, und der Lebenszyklus löst den Effekt `notify_clarification` aus: die zugewiesene Person der Nachricht bzw. der Konversation — oder ohne Zuweisung jede Leserin des gemeinsamen Postfachs — erhält einen In-App-Hinweis und eine Desktop-Benachrichtigung über denselben Kanal wie eine Zuweisung. Die Antwort setzt den Entwurf mit den Angaben als `[CONFIRMED BY OWNER]`-Fakten fort. Fehlt nichts → `drafting`.

### Schritt 4: Entwurfserzeugung

`apps/api/convex/agent/steps/draft/index.ts` — nutzt die **leistungsfähige** Modellstufe über `getLLMProvider('draft')` und `runLlmText`. Der Entwurf ist in `agentConfig.toneDescription`, `agentConfig.signatureTemplate` und dem zusammengestellten Kontext verankert und wird **in der Sprache des Absenders geschrieben**: der System-Prompt nennt die vom Klassifizierer erkannte Sprache (und verlangt immer, die eingehende Nachricht zu spiegeln), und die alternativen Prüfentwürfe folgen derselben Regel — eine Organisation mit englischer Tonvorgabe antwortet einer deutschen Kundin trotzdem auf Deutsch. Als Defense in Depth prüft der Entwurfsschritt den zusammengestellten Kontext erneut auf Injection-Muster (Thread-Verlauf, der stromaufwärts nicht einzeln geprüft wurde); ein Treffer mit hoher Konfidenz lässt den Schritt fehlschlagen. Die Ausgabe wird zusammen mit einem `confidenceScore` in `inboundMessages.draftResponse` / `draftSubject` abgelegt.
...
Eingehende Mail für das gemeinsame Agenten-Postfach kommt über eine eigene Route an, denn die Nutzlast trägt die **gesamte Nachricht**:
...
Warum eine eigenständige Route statt eines Pipeline-Adapters: `webhooks/pipeline.ts` begrenzt den Anfragetext auf `MAX_WEBHOOK_BODY_BYTES` (5 MiB) — **vor** der Authentifizierung — und antwortet darüber mit `413`. Da die Nutzlast jetzt `rawBytesBase64` enthält, würde jede Mail über rund 3,75 MiB abgewiesen; die MTA wertet ein `413` als wiederholbaren Fehler, die Nachricht würde ihre Zustellversuche aufbrauchen und in der DLQ landen. Eine eigenständige `httpAction` durchläuft die Pipeline nie, die Grenze gilt dort also nicht.

::callout{type="info" title="Drei MTA-Webhooks, drei Aufgaben"}
`POST /webhooks/mta` transportiert Zustellereignisse (Bounce, Beschwerde, IP-Reputation) und nimmt `inbound.received` weiterhin für DLQ-Wiederholungen und für ältere MTA-Binaries an — dieser Weg liefert dieselbe Mail ohne die Bytes, also ohne Anhänge und ohne Malware-Urteil. `POST /webhooks/mta-inbound` ist die oben beschriebene Route des Agenten-Postfachs. `POST /webhooks/mta-mailbox` gehört zu [Postbox](/guide/postbox) (persönliche Mail pro Benutzerin und Benutzer) und läuft über `mail/webhookHttp.ts`, nicht über die Agenten-Strecke.
::

::callout{type="warning" title="Anhänge lesen setzt den ClamAV-Sidecar voraus"}
Ungeprüfte Bytes erreichen kein Modell. Die Malware-Prüfung läuft im MTA-Container hinter `POST /scan/attachment` und ist nur aktiv, wenn die Anhangsprüfung eingeschaltet ist (`scan.attachments`, das optionale Compose-Profil `clamav`). Ist sie aus, antwortet jeder Teil mit „übersprungen“, die Nachricht speichert `virusVerdict: 'skipped'`, und die Thread-Ansicht schreibt **Nicht auf Schadsoftware geprüft — der Assistent hat diese Dateien nicht gelesen**. Mail, Metadaten und Download funktionieren weiterhin; zurückgehalten wird nur die Indizierung. Das persönliche Postfach (Postbox) ist die bewusste Ausnahme: es ist die eigene Mail der Nutzerin oder des Nutzers, also indiziert es die Anhänge weiterhin, wenn überhaupt kein Scanner geantwortet hat — alle Teile außer denen, die die Dateityp-Prüfung des Endpunkts abgelehnt hat, denn genau die sieht nie ein Scanner.
::
...
- **`/dashboard/inbox/[threadId]`** — die vollständige Konversation mit einem Antwortfeld, das mit dem Agentenentwurf vorbelegt ist (senden / bearbeiten / ablehnen) und das eine Person auch ohne Entwurf nutzen kann.
- **`/dashboard/answer?in=team`** (`answer.vue`) — die Antwortliste, gefiltert auf das Team-Postfach: ein wartender Entwurf nach dem anderen. `/dashboard/inbox/review` leitet hierher um.
- **`/dashboard/admin/delivery/quarantine`** und **`/dashboard/admin/delivery/failed`** — sicherheitsmarkierte und fehlgeschlagene Nachrichten unter Einstellungen > E-Mail-Zustellung. Die alten Routen `/dashboard/inbox/quarantine` und `/dashboard/inbox/failed` leiten dorthin um.
