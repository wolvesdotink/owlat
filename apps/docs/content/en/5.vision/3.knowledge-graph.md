...
The one-time **backfill job** (`apps/api/convex/agent/knowledgeBackfill.ts`) still walks the deployment's existing `inboundMessages` on the first `false→true` toggle of the `ai.agent` feature flag (`setFeatureFlag` in `apps/api/convex/workspaces/featureFlags.ts`) so the drafter has historical context from day one. Internal MEMBER-to-member chat (`unifiedMessages.sendChatMessage`) does **not** trigger extraction — only inbound pipeline messages do.
::
