...
Routing: `spam` (or a matching `auto_archive` handling rule) → `archived`; a **confident "no response needed" verdict** → `informational` (the pipeline stops; the message is ranked on the team inbox's **Updates** dashboard by `importance`, where a reader can dismiss it or ask for a draft after all); everything else → the `clarify` step, in-state. The guard rails on the informational fork are deliberately one-sided: a complaint, anything urgent, an escalation, a question or a request always takes the reply path whatever the boolean says, and a verdict below 0.6 confidence is not trusted. The classification is stored on `inboundMessages.classification`.
