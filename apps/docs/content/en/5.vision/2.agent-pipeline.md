...
The step kinds are exactly: `security_scan`, `context_retrieval`, `classify`, `clarify`, `draft`, `route` (`AgentStepKind` in `agent/steps/catalog.ts`). A `plan` step was designed but **dropped pre-prod with ADR-0014** — there is no action-planning stage. Not every message walks the whole chain: mail that needs no reply stops after `classify` (see [ADR-0061](/developer/decisions)).
...
```typescript
{
  ...
  kind: 'personal' | 'update' | 'notification' | 'receipt'
      | 'newsletter' | 'advertising',  // nature of the mail, orthogonal to category
  needsResponse: boolean,  // is the sender waiting for a reply?
  language: string,        // ISO 639-1 code the sender wrote in
  importance: number,      // 0–1, how much the recipient needs to know
  summary: { en: string, de: string },  // one sentence per interface locale
}
```

The prompt frames the assembled context as untrusted data (`SYSTEM_GUARD`); every free-text field the model returns is bounded and scrubbed before it is stored, and `language` is allowlisted to a plain language code before the draft step may put it in its system role.

Routing: `spam` (or a matching `auto_archive` handling rule) → `archived`; a **confident "no response needed" verdict** → `informational` (the pipeline stops; the message is ranked on the team inbox's **Updates** dashboard by `importance`, split into Updates / Promotions / Notifications tabs by `kind`, where a reader can dismiss it or ask for a draft after all; a Spam tab lists what was archived as spam and offers blocking the sender). The four bulk kinds — notification, receipt, newsletter, advertising — are informational whatever their intent label; everything else → the `clarify` step, in-state. The guard rails on the informational fork are deliberately one-sided: a complaint, anything urgent, an escalation, a question or a request always takes the reply path whatever the boolean says, and a verdict below 0.6 confidence is not trusted. The classification is stored on `inboundMessages.classification`.

### Step 3b: Clarify

`apps/api/convex/agent/steps/clarify/index.ts` — the missing-info gate between classification and drafting. It extracts the *reply slots* a good answer has to fill (a decision, a date, a price, an attachment, a stance, a fact), samples a few candidate replies and keeps only the slots they genuinely disagree on. Each surviving question carries the model's **suggested answers** as option chips and is **translated into every interface locale** (`inbox/clarificationLocalize.ts`), so the person is always asked in their own language while the canonical English copy is what gets stored and matched by answer-memory.

When questions remain the message parks in `awaiting_clarification` and the lifecycle emits a `notify_clarification` effect: the message's or thread's assignee — or, with no assignee, every shared-inbox reader — gets an in-app toast and a desktop notification through the same notice channel as a thread assignment. Answering resumes the draft with the answers folded in as `[CONFIRMED BY OWNER]` facts. Nothing missing → `drafting`.

### Step 4: Draft generation

`apps/api/convex/agent/steps/draft/index.ts` — uses the **capable** model tier via `getLLMProvider('draft')` and `runLlmText`. The draft is grounded in `agentConfig.toneDescription`, `agentConfig.signatureTemplate`, and the assembled context, and it is **written in the sender's language**: the system prompt names the language the classifier detected (and always says to match the inbound), and the alternative review drafts follow the same rule, so an organisation whose tone guidance is in English still answers a German customer in German. As defense-in-depth, the drafter re-scans the assembled context for injection patterns (thread history that wasn't individually scanned upstream); a high-confidence hit fails the step. Output is stored to `inboundMessages.draftResponse` / `draftSubject` with a `confidenceScore`.
...
Inbound mail for the shared agent inbox arrives over its own route, because the payload carries the **whole message**:

```
Inbound SMTP → owlat-mta
  → POST /webhooks/mta-inbound  (apps/api/convex/http.ts,
                                 apps/api/convex/inbox/inboundWebhookHttp.ts)
    → verifies the shared MTA HMAC + 5-minute staleness window
      (apps/api/convex/webhooks/adapters/mtaRawRoute.ts)
    → writes a bounded audit row (a digest of the body, never a copy of the mail)
    → internal.inbox.inboundIngest.ingestFromWebhook
      → seals the raw `.eml`, scans its attachment leaves for malware,
        then internal.inbox.messages.receiveMessage
```

Why a standalone route rather than a pipeline adapter: `webhooks/pipeline.ts` caps a request body at `MAX_WEBHOOK_BODY_BYTES` (5 MiB) **before** authenticating it and answers `413` above that. Since the payload now carries `rawBytesBase64`, any mail over roughly 3.75 MiB would be rejected — and the MTA reads a `413` as a retryable failure, so the message would burn its delivery attempts and land in the DLQ. A standalone `httpAction` never runs the pipeline, so the cap does not apply.

::callout{type="info" title="Three MTA webhooks, three jobs"}
`POST /webhooks/mta` carries delivery events (bounce, complaint, IP reputation) and still accepts `inbound.received` for DLQ replays and for an MTA binary older than the route split — that path delivers the same mail without the bytes, so it has no attachments and no malware verdict. `POST /webhooks/mta-inbound` is the agent-inbox route described above. `POST /webhooks/mta-mailbox` is for [Postbox](/guide/postbox) (per-user personal mail) and goes through `mail/webhookHttp.ts`, not the agent pipeline.
::

::callout{type="warning" title="Attachment reading needs the ClamAV sidecar"}
Nothing unscanned reaches a model. The malware scan runs in the MTA container behind `POST /scan/attachment`, which is only wired up when attachment scanning is enabled (`scan.attachments`, the optional `clamav` compose profile). With it off, every leaf answers "skipped", the message stores `virusVerdict: 'skipped'`, and the thread view says **Not scanned for malware — the assistant has not read these files**. The mail, its metadata and the download all still work; only the indexing is withheld. The personal mailbox (Postbox) is the deliberate exception: it is the owner's own mail, so with no scanner answering at all it keeps indexing the message's own attachment leaves — every leaf except the ones the endpoint's file-type gate refused, which are the ones nobody will ever scan.
::
...
Messages flagged by the security scan are not silently dropped. They are stored with `processingStatus: 'quarantined'` and a `securityFlags` object, and surface in a dedicated admin view (**Settings > Email delivery > Quarantine**, `apps/web/app/pages/dashboard/admin/delivery/quarantine.vue`) where a human can review and release them. Releasing a quarantined message transitions it back to `received` (the lifecycle's `release_quarantine` source), which re-runs the pipeline from `security_scan`.
...
- **`/dashboard/inbox/[threadId]`** — full conversation with a reply box pre-filled with the agent draft (send / edit / reject), which a person can also use when the agent has no draft.
- **`/dashboard/answer?in=team`** (`answer.vue`) — the Answer queue filtered to the team inbox: one waiting draft at a time. `/dashboard/inbox/review` redirects here.
- **`/dashboard/admin/delivery/quarantine`** and **`/dashboard/admin/delivery/failed`** — security-flagged and failed messages, under Settings > Email delivery. The old `/dashboard/inbox/quarantine` and `/dashboard/inbox/failed` routes redirect there.
