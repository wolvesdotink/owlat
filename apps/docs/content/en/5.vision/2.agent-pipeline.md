...
`POST /webhooks/mta` carries delivery events **and** the agent-inbox `inbound.received` event. The separate `POST /webhooks/mta-mailbox` route is for [Postbox](/guide/postbox) (per-user personal mail) and goes through `mail/webhookHttp.ts`, not the agent pipeline.
::
