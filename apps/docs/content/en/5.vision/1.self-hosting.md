...
| `EMAIL_PROVIDER` | Email provider: `mta`, `ses`, `resend`, `smtp`, `mandrill`, `emailit` | — (unset ⇒ unconfigured; sends are refused) |
...
- **Coding agent sidecar** — the `code-worker` container (run under `--profile inbox-codetasks`, also enabled by `--profile dev`) gets its own `code-workspace` volume, separate from the backend's `convex-data` volume, and runs with `no-new-privileges`. It holds **no deployment admin key**: it reaches the backend only through the `convex-fn-proxy` sidecar (which holds the admin key and forwards only the thirteen allowlisted worker functions), and all of its egress is forced through the allowlisted `code-worker-egress` forward-proxy.
...
code-worker:
  ...
  environment:
    CONVEX_URL: http://convex-fn-proxy:3220        # the allowlist proxy, not convex directly
    CODE_WORKER_CONVEX_KEY: ${CODE_WORKER_PROXY_TOKEN}  # proxy token — NOT the admin key
    HTTP_PROXY: http://code-worker-egress:8888     # allowlisted egress
...
convex-fn-proxy:                                    # holds CONVEX_ADMIN_KEY; forwards only the 13 worker functions
  build: { context: ., dockerfile: apps/convex-fn-proxy/Dockerfile }
  environment:
    CONVEX_URL: http://convex:3210
...
Enable Docker **userns-remap** on the host so the confined-root code-worker maps to an unprivileged host uid — copy `infra/templates/docker-daemon.userns.json` to `/etc/docker/daemon.json` (merging with any existing keys), create the `dockremap` subordinate-uid/gid ranges, and restart the daemon.
...
| **Code worker** | Task-specific only | `CONVEX_URL` (the `convex-fn-proxy` endpoint), `CODE_WORKER_PROXY_TOKEN` (proxy auth, **not** the admin key), `HTTP(S)_PROXY` (allowlisted egress), `LLM_*` (for direct LLM access) |
| **convex-fn-proxy** | Proxy process only | `CONVEX_ADMIN_KEY` (injected on forwarded requests), `CODE_WORKER_PROXY_TOKEN` (validated) |

Convex environment variables are never passed to agent-generated code, and the untrusted `code-worker` sidecar does not hold the deployment admin key at all. Self-hosted Convex mints only a full admin key — there is no per-function scoping — so the least-privilege boundary is built OUTSIDE Convex, in the `convex-fn-proxy` sidecar: it holds `CONVEX_ADMIN_KEY` and forwards only the thirteen `codeWorkTasks:*` / `plugins/workerTasks:*` functions the worker drives, rejecting any other function path. The worker polls an `internalQuery` and drives `internalMutation`s (unreachable by an anonymous client), authenticating to the proxy with `CODE_WORKER_PROXY_TOKEN` via `setAdminAuth` on the `ConvexHttpClient` (`apps/code-worker/src/convexClient.ts`); the proxy validates and strips that token and injects the admin key (`apps/convex-fn-proxy`). Compose passes the token into the worker and the admin key into the proxy (`docker-compose.yml`). Keep both on the internal Docker network and don't expose their endpoints. To revert to the older direct model, point `CONVEX_URL` back at `http://convex:3210` and pass `CONVEX_ADMIN_KEY` to the worker — `convexClient.ts` keeps that fallback.
