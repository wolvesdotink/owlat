...
For **Gmail**, if your operator has configured a Google OAuth client for this instance, the card offers **Continue with Google** instead: you approve Owlat in Google's own sign-in screen and no password is ever typed. Owlat stores only an encrypted refresh token and authenticates to Gmail's IMAP and SMTP servers with XOAUTH2. It is the recommended path, but not the only one — **Use an app password instead** stays available, and on an instance with no OAuth client the app-password form is what you get. See [environment variables](/developer/environment-variables#google-sign-in-for-gmail-mailboxes-optional) for the operator-side setup.
