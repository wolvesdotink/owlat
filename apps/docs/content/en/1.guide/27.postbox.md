...
For **Gmail**, if your admin enabled Google sign-in on this instance, the card offers **Sign in with Google** underneath the password fields as an alternative: you approve Owlat in Google's own sign-in screen, no password is typed, and Owlat stores only an encrypted refresh token with which it authenticates to Gmail's IMAP and SMTP servers over XOAUTH2. It is an optional convenience — enabling it means creating a Google OAuth client, so most instances will not have it, and the app-password form works the same either way. See [environment variables](/developer/environment-variables#google-sign-in-for-gmail-mailboxes-optional) for the operator-side setup.
...
The AI layer treats a connected mailbox exactly like a hosted one. In particular, [draft-on-arrival](/guide/reply-queue#draft-on-arrival-review-and-send) (`postbox.aiDraft`) works for connected mailboxes: the flag needs `ai` plus _either_ `postbox` _or_ `mail.external`, so an external-only install with no hosted Postbox — and no domain — still gets a pre-generated, human-reviewed reply draft the moment a synced message needs one. Only newly synced mail counts: an import of the mailbox's history is deliberately excluded from the Reply Queue, so it never generates drafts for mail you answered years ago.
...
The inbox opens on **Today** — a focused single column showing the day's work
and nothing else: a **For you** list of emails waiting on your reply (the same
items as the [Reply Queue](/guide/reply-queue)), today's mail (everything
received since your local midnight, plus anything still unread from
yesterday), and a centered **Show past mails (n)** affordance that expands
older mail inline. Mail the [Smart Inbox](/guide/smart-inbox) classifies as
newsletters, notifications, or receipts doesn't clutter Today — one quiet
roll-up line ("12 newsletters & receipts auto-filed · view") links to the
Categories view instead.

Opening a message in Today keeps you in the focused column: the conversation
appears in a **centered reader** over the list (the full reader — reply,
forward, snooze, AI assist all work there). **Esc** or a click outside returns
to the list exactly where you left it, **`j`**/**`k`** move to the next or
previous conversation without closing, and archiving/trashing/snoozing
advances to the next one automatically (per your auto-advance preference).
Links to a specific message open the same way while you're in Today mode.

**Browse** is the full three-pane UI (folder rail, list, reader) for folder
work and bulk triage. Switch with the **Browse** button in the Today header,
press **`b`** (or **Cmd/Ctrl+B**) to toggle, and **Esc** in Browse returns to
Today. Pressing **`/`** in Today jumps to Browse with search focused. Your
last-used mode is saved to your account. Every other folder and search use
the regular three-pane UI — every row, shortcut, and action in Today is the
same as in Browse.

### Inbox view modes

The inbox header has a labeled **Flat | Conversations | Categories** control —
exactly one mode is active at a time:

- **Flat** (default) — one row per message, with the hover and `j`/`k`
  keyboard triage.
- **Conversations** — one row per thread, with a message count and unread
  badge.
- **Categories** — the [Smart Inbox](/guide/smart-inbox) grouping into People,
  Newsletters, Notifications, and Receipts sections.

Your choice is saved to your account, so it follows you across devices and
reloads. The modes are inbox-only — every other folder stays a flat list.
...
- **Navigate**: `j`/`k` (or arrows) move, `Enter` opens, `/` focuses search,
  `b` toggles the inbox between Today and Browse (`Esc` in Browse returns to
  Today).
...
Postbox labels PGP and S/MIME messages honestly, and for **PGP-signed
(unencrypted) mail it verifies the signature on the server at ingest** — both
PGP/MIME (`multipart/signed`) and inline clear-signed messages. The badge shows
exactly what was checked:

- **Signed · verified** — the signature verified against the sender's key. The
  chip shows the key's fingerprint tail; hover for the full fingerprint and
  where the key came from (the trusted key on file, or the sender's WKD key
  directory).
- **Signed · signature invalid** — a key was found, but the signature does not
  verify. The message may have been altered in transit.
- **Signed · sender key not found** — the message is signed, but no public key
  for the sender could be discovered, so nothing could be checked.
- **Signed · sender key changed** — the sender's key differs from the key
  previously trusted for that address. Verification is refused until the key
  change is reviewed (the same trust-on-first-use rule
  [Sealed Mail](/guide/sealed-mail) applies).

Sender keys are discovered via Web Key Directory (WKD) and pinned on first
use. Verification never delays or blocks delivery — a failed check is shown
as-is, and a message whose verdict couldn't be computed falls back to the plain
structural label _Digitally signed · not verified_.

Encrypted messages Owlat cannot open are labeled _End-to-end encrypted · can't
decrypt_ and flagged rather than rendered as gibberish, with the ciphertext
available to copy or download for an external OpenPGP tool. Mail sealed between
Owlat workspaces is decrypted and verified separately — see
[Sealed Mail](/guide/sealed-mail). S/MIME messages are detected structurally
only (_Digitally signed · not verified_); their signatures are not verified.
...
Postbox can keep a small copy of recent mail on the current device so the inbox opens instantly on a cold start and stays readable without a connection — and sending while offline queues on the device instead of failing.

- **What's stored:** the newest ~200 inbox rows and the sanitized bodies of the ~50 most recently read messages, each scoped to the mailbox that cached them. Only the already-sanitized version of each message is cached — never the raw mail. Queued outgoing messages you composed offline are stored too (with their full content), until they're sent.
...
- **Offline:** a quiet banner appears ("Offline — reading from this device; sends will go out when you're back online"), the cached list and cached bodies stay readable, and other server-backed actions are paused with clear affordances.
- **Sending while offline:** hitting Send queues the message on this device and behaves exactly like an online send — including Undo, which un-queues it and reopens the composer. Queued messages go out automatically, in order, when the connection returns; the offline banner shows how many are waiting. A queued message that still can't be sent after reconnect is kept on the device with its error and surfaced with a Retry — it is never silently dropped. (Attachments must have finished uploading before the device went offline to ride along.)
- **Control it:** toggle **Store recent mail on this device** and use **Clear local cache** under **Postbox → Settings → On this device**. It's on by default in the desktop app and off by default in the browser. Leave it off on shared computers — the cache is unencrypted local storage. If the device runs out of storage, read-caching disables itself silently and says so in settings; a queued send that doesn't fit tells you immediately instead of failing quietly.

Not yet offline in this version: the folder rail (it still renders from the live query on a cold start), full-mailbox sync, and encryption at rest. Those are planned follow-ups.
...
**My external mailbox shows "Authentication failed".** Re-enter the password on **Settings → External mailbox → Update credentials**. Gmail and Outlook need an app password with 2-factor enabled — your normal account password won't work. A mailbox connected with Google sign-in shows **Reconnect with Google** there instead: that state means the authorization was revoked or expired (someone removed Owlat under Google Account → Security → Third-party access, or the account's password changed), and one round through Google's sign-in screen restores it. The `mail.external` flag (and its `external-mail` Docker profile / mail-sync worker) must also be running.

**Connecting a mailbox fails with "The mail sync service is not configured on this instance."** The flag is on, but the mail-sync worker isn't wired to the backend: the Convex deployment has no `MAIL_SYNC_API_URL` / `MAIL_SYNC_API_KEY`, so the credential check reports the same failure for both incoming and outgoing mail without ever contacting your provider. Enabling `mail.external` from **Settings → Features** only stores the flag — it starts no container and pushes no deployment env. On the host, run `owlat-setup feature mail.external on` and `owlat restart` (that activates the `external-mail` profile), then re-run setup so both variables reach the function runtime. With the flag on, `owlat-setup doctor` and `owlat-setup env --show` list them as required, and **Settings → Features** badges the flag "needs config" until they're set. Connecting anyway is not blocked, but the mailbox will never sync and its sends fail per recipient with `EXTERNAL_NOT_CONFIGURED`.

**Connecting a mailbox times out instead of failing.** The worker is wired
correctly and the connection simply never completes, which is what an outbound
port your host blocks looks like — 993 (IMAP) and 465 (implicit-TLS SMTP) are
the two this needs, and a provider that filters mail ports may drop either. **Administration → System & Updates →
Network ports** probes both from the server and says which are blocked; the fix
is a request to your hosting provider, not a setting here. Gmail and Microsoft
365 offer IMAP on 993 only, so there is no alternative port to fall back to.
