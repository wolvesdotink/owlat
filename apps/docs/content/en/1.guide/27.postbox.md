...
correctly and the connection simply never completes: most VPS providers block
outbound 993 (IMAP) and 465 (implicit-TLS SMTP) by default, so the sync worker
sits there until the socket gives up. **Administration → System & Updates →
