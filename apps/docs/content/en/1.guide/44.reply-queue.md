...
The Reply Queue ships with Postbox (`postbox` or `mail.external`). The ask summaries, urgency ranking, due hints, "Draft reply", and the "Needs your input" clarification cards require the `ai` feature flag; without it the queue is purely deterministic. **Draft-on-arrival** (the pre-generated "Draft ready" review slot) additionally requires `postbox.aiDraft` — off by default, and forced off whenever `ai` is off or neither mailbox source (`postbox` or `mail.external`) is on. It works for connected external mailboxes the same as for hosted ones — but only for mail that arrives *after* the mailbox is connected. A **full-history import** is deliberately left out of the queue, so importing years of old mail never buries you in threads you answered long ago.
::
