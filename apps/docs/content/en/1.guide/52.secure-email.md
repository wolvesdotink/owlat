...
### 5. Signature badges for signed-but-unencrypted mail

Plenty of mail arrives PGP-signed without being encrypted — the sender is
proving authorship, not hiding content. Owlat verifies these signatures on the
server when the message arrives (both PGP/MIME `multipart/signed` and inline
clear-signed messages) and records an honest verdict the reader renders as a
badge:

- **Signed · verified** — the signature verified against the sender's key. The
  badge carries the signing key's fingerprint, and its tooltip names where the
  key came from: the trusted key already on file for that sender, or a fresh
  Web Key Directory (WKD) lookup pinned on first use.
- **Signed · signature invalid** — a key was found but the signature does not
  verify; the message may have been altered since it was signed.
- **Signed · sender key not found** — the message is signed, but no public key
  could be discovered for the sender, so there was nothing to check against.
- **Signed · sender key changed** — the sender's observed key conflicts with
  the key pinned earlier. Verification is refused until the change is
  reviewed, the same trust-on-first-use rule Sealed Mail applies.

The same honesty rules hold as everywhere else in Owlat: **verified** is only
shown when the cryptography actually succeeded, verification never delays or
blocks delivery, and a message whose verdict could not be computed falls back
to the plain structural label _Digitally signed · not verified_ rather than
borrowing a claim. A signature badge also says nothing about privacy — a
verified signed message was still readable by every server that carried it.

### 6. Stored bodies and secrets are sealed too
