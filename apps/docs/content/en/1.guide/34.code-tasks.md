...
::callout{type="info" title="The worker needs the deployment admin key"}
The worker authenticates with the deployment admin key (`CONVEX_ADMIN_KEY`), exactly like the `imap` and `mail-sync` sidecars, which lets it call the internal queries and mutations it drives the task with (`getNextQueued`, `claim`, `updateBranch`, `markTesting`, `completeWithPR`, `markFailed`, `reclaimStale`). Both the dev Compose stack and the VPS template pass the var through to the `code-worker` service; the setup CLI mints the key on first boot and writes it to `.env`, where Compose interpolates it. If the worker exits at startup with `CONVEX_ADMIN_KEY environment variable is required`, the key is missing from `.env` — generate it with `docker compose exec convex ./generate_admin_key.sh` and restart the service.
::
