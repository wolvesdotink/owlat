...
The reducer rejects any transition not in the `CAMPAIGN_LIFECYCLE` graph. There is no exception thrown — an illegal call returns `{ ok: false, reason: 'illegal_edge' }`, and a transition out of a terminal status returns `reason: 'terminal'`. Callers translate the outcome into a user response.
...
If the campaign was `cancelled` or `draft` (unscheduled), or already `sent`, the orchestrator returns `skipped: true` with a reason. It does **not** skip on `sending` — the campaign arrives already flipped to `sending` by the lifecycle transition, and same-state transitions don't re-fire the orchestrator effect.
