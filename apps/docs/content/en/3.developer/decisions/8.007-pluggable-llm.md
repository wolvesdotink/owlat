...
### Three planes, one adapter rule

The abstraction now covers three AI planes. Each resolves in its own module, and each follows the same rule: a registry of one-file adapters behind a mapped-type assignment (a missing method is a compile error), with call sites importing the entry point rather than an adapter.

| Plane | Resolves in | What it does | Configured by |
| --- | --- | --- | --- |
| **Language** | `lib/llmProvider.ts` | everything that writes — drafts, replies, summaries, translation, the assistant | the `LLM_*` variables above, or the AI-provider admin page |
| **Embedding** | `lib/llmProvider.ts` (`getEmbeddingModel()`) | retrieval vectors, resolved independently of the language plane so retrieval still works under a language provider that has no embeddings API | `LLM_EMBEDDING_MODEL`, or a local embedder via `LOCAL_EMBEDDING_BASE_URL` |
| **Decision** | `lib/decisionProvider.ts` | answers typed questions — yes/no, one of these categories, a point on an ordered scale — with a probability distribution over the options that were sent | `DECISION_PROVIDER`, `TYPESAFE_API_KEY`, or the AI-provider admin page |

The decision plane is a plane rather than a fourth LLM provider because the thing it returns does not survive the AI SDK's `LanguageModel` interface: a distribution wrapped as a completion is a parsed object with the probabilities thrown away at the boundary, which is the one thing it exists to deliver.

It is **opt-in and off by default** — the `ai.decisionPlane` flag requires `ai` and is deliberately not a member of the `ai` feature pack. An install that configures no decision key resolves to the `llm` adapter, which answers the same questions through the language model already configured and marks its answers uncalibrated, so an existing deployment behaves exactly as it did before.

Its contract — the question DSL, the dispatch's deadline, retry, one-hop fallback and accounting rules, and why no decision answer may widen auto-send — is recorded in ADR-0060 (`docs/adr/0060-decision-plane.md`). The operator-facing configuration and data-flow note lives on the [Providers](/developer/providers#decision-provider-the-third-ai-plane) page. Neither is repeated here.
