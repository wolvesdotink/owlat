...
provider is set via `EMAIL_PROVIDER` (or a per-org provider route) — the
built-in MTA, Amazon SES, Resend, a generic SMTP relay, or Mailchimp
Transactional. There is no implicit default: with none of them configured, auth
mail is refused rather than sent through the MTA.
...
When the resolved provider is the built-in MTA (`EMAIL_PROVIDER=mta`),
`sendViaMta` ultimately POSTs to `${MTA_API_URL}/send` with a
`Bearer ${MTA_API_KEY}` header — the same MTA that delivers your campaigns.
Every other kind the catalog declares carries invitation/password-reset/
email-change mail just as well, selected by `EMAIL_PROVIDER` or a provider
route.
::
...
`apps/api/convex/seedAdminHttp.ts`); every subsequent user joins that singleton org
as an editor/admin/owner via the invite flow. Public signup cannot create the
first production account; that exception exists only with `OWLAT_DEV_MODE=true`
...
New signups always receive a verification email, and accepting an organization
invitation always requires a verified email address. An invitation ID alone is
not proof of inbox ownership: existing organization members can list invitations.
A working system mail provider is required for new invitees to verify their address.

`REQUIRE_EMAIL_VERIFICATION` separately controls whether existing accounts must
be verified to sign in. **New deployments default it on** through the self-host
env templates and setup CLI. The seeded owner is already verified.

**Existing installs are deliberately left unset** so legacy members without an
`emailVerified` flag can still sign in. This compatibility setting never disables
verification when accepting a new invitation. Enable sign-in verification only
after verifying or backfilling legacy users.

If a user is ever stranded (mail outage, an account that predates verification),
an owner/admin recovers them without a database edit via
`auth/emailVerificationAdmin.ts`:

- `markMemberEmailVerified` — mark the member's email verified out-of-band
  (idempotent, org-scoped, audited).
- `resendMemberVerificationEmail` — re-send the verification link through the same
  route a signup uses.
