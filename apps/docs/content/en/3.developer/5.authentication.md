...
`REQUIRE_EMAIL_VERIFICATION` gates whether a signup or invitation acceptance must
confirm a verification link before it can sign in (BetterAuth
`requireEmailVerification` + `sendOnSignUp` + the org plugin's
`requireEmailVerificationOnInvitation`). **New deployments default it on** — the
self-host env templates and the setup CLI write it for a fresh install. The web
setup wizard does not emit the flag itself, but when it is set the wizard refuses
to finish with verification on but no delivery provider, since the verification
mail routes through the system transport (which fails closed when no provider is
configured). The seeded owner is created already-verified, so
first-run admin is never locked out.

**Existing installs are deliberately left unset.** Enabling it on an install whose
users predate verification would lock out everyone who has no `emailVerified`
flag, so an upgrade never sets it — turn it on only after backfilling
`emailVerified` for legacy users.
