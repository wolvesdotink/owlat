...
The connect/test/credential modules are `external/accounts.ts` (queries + internal mutations), `external/accountsActions.ts` (`connect` / `testConnection` / credential handling), and `external/delivery.ts` (ingest).
