...
`externalMailAccounts.authMethod` is `'password'` (an app password) or `'oauth2'` (`oauthProvider: 'google'`). OAuth is **additive**: app passwords remain supported for every provider including Gmail, and an instance with no `GOOGLE_OAUTH_CLIENT_ID` / `GOOGLE_OAUTH_CLIENT_SECRET` behaves exactly as before.
...
1. `api.mail.external.googleOAuth.isConfigured` tells the connect form whether to offer the Google branch (a boolean derived from env presence — never the credentials).
