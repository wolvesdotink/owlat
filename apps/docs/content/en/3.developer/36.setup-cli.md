...
9. **Sample data** (unless `blank` or `--no-seed`) — `POST /sample-data/install`. A local `dev` deployment that already has `OWLAT_DEV_MODE=true` gets the fuller `/seed/demo` dataset instead (dummy teammate sign-ins + Postbox mailboxes); no install flow ever turns that flag on.
