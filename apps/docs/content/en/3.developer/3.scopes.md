...
| `packages/email-builder` | Vue 3 | Notion-like email editor — document canvas, block previews, settings popover, slash commands — plus the preview pane under `src/preview/` (cross-client rendering simulation, compatibility scoring, email diffs) |
...
- **web** imports `email-builder`, `email-renderer`, `ui`, `shared`, and the generated Convex client `@owlat/api` (real-time queries & mutations)
- **api** imports `email-renderer`, `email-scanner`, and `shared`
