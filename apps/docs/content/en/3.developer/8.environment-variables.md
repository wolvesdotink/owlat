...
Two independent caps on the Redis the MTA and the IMAP auth limiter share. The defaults suit a small single-box install (4-8 GB of RAM) with plenty of headroom: a healthy Owlat keyspace is tens of megabytes, so reaching 512 MB means something is leaking, not that you have outgrown the defaults. Raise both together if you genuinely run high volume.
