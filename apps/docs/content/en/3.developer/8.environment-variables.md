...
This is **optional and additive**. App passwords keep working for every provider, Gmail included: with no OAuth client configured the Gmail card offers the app-password form exactly as before, and with one configured it offers "Continue with Google" as the recommended path plus "Use an app password instead".

| Variable | Description |
| --- | --- |
| `GOOGLE_OAUTH_CLIENT_ID` | Client ID of the Google OAuth client. Unset ⇒ Google sign-in is not offered and the Gmail card falls back to the app-password form. |
