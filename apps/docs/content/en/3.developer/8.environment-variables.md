...
| `FBL_DEDUP_PROTOCOL` | Required versioned complaint reservation protocol. Set `owned-v2`; there is no automatic legacy/shadow mode. See [FBL deduplication upgrade](/developer/mta-system#fbl-deduplication-upgrade). | required |
...
### Redis memory ceilings (optional)

Two independent caps on the Redis the MTA and the IMAP auth limiter share. The defaults suit a small single-box install (4-8 GB of RAM) sending a few thousand messages a day.

Size `REDIS_MAXMEMORY` against your send volume rather than against a fixed idea of "healthy". Each delivery attempt leaves roughly five idempotency receipts behind — the records that stop a retried attempt double-counting a bounce or re-sending a message — held for seven days, thirty-five for the per-campaign complaint denominator. At a few hundred bytes each that is on the order of 1.5 KB per attempt, so sustained volume in the region of 25-30k attempts a day approaches 512 MB on receipts alone. Below that, a keyspace in the tens of megabytes is normal and reaching the cap means something is leaking; above it, raise both caps together.

One part of the keyspace is permanent by design and never shrinks on its own: the suppression list. A hard bounce or a spam complaint must not expire, so each PERMANENTLY suppressed address costs roughly 300 bytes for as long as the deployment lives — about 30 MB per 100,000 addresses, and more than half a 512 MB cap at a million. (A temporary entry costs about a third more, ~400 bytes, because it also occupies the due-date index; those are bounded, because an hourly sweep reclaims them.) Ask the MTA for the current count by reason with `GET /suppression/stats` (see [MTA system](/developer/mta-system)) and subtract that baseline before concluding you have a leak — a long-lived instance with a large suppression list is simply carrying the list it is supposed to carry. If the list alone is a large fraction of the cap, raise **both** caps together as above: lifting `REDIS_MAXMEMORY` on its own toward the cgroup ceiling is how an append-only-file rewrite's fork gets the container OOM-killed, which is the failure `REDIS_MEM_LIMIT` exists to prevent. The addresses are also held durably in Owlat's own blocklist, so this copy is a backstop rather than the record.
...
| Variable | Description | Default |
| --- | --- | --- |
| `REDIS_MAXMEMORY` | Redis's own `--maxmemory` cap, so it reacts before the container's cgroup does. The policy is pinned to `noeviction`: this instance mixes disposable rate-limiter counters with durability records the MTA must not lose (defer handoffs, intake receipts, idempotency receipts), and every one of them carries a TTL, so no eviction policy can tell them apart. At the cap Redis **refuses writes** rather than evicting — a refused write is retried and visible in the MTA logs (`OOM command not allowed`), an evicted handoff receipt is a silently double-sent email. | `512mb` |
| `REDIS_MEM_LIMIT` | The container's cgroup ceiling. Kept well above `REDIS_MAXMEMORY` because an append-only-file rewrite forks and its copy-on-write can briefly approach double the dataset. It exists so overshooting kills Redis alone instead of letting the kernel's OOM killer pick a victim across the whole box. | `1g` |
...
# REDIS_URL=                           # wired by docker-compose.yml; required in production
```
...
::callout{type="warning" title="IMAP login throttling is mandatory in production"}
Port 993 is internet-facing, so the IMAP server **refuses to start** in production without `REDIS_URL` — it backs the LOGIN brute-force limiter (`apps/imap/src/config.ts`). Both shipped compose files now set it for you, pointing at the same Redis the MTA uses; you only need to set it by hand in a bespoke deployment. To run the listener with no brute-force protection at all, you must opt out in so many words with `IMAP_ALLOW_UNTHROTTLED_AUTH=true`.

A Redis that is configured but temporarily unreachable still fails **open**, deliberately: that outage is transient, and locking every user out of their mail because the cache blinked is the worse failure.
::
...
#### Google sign-in for Gmail mailboxes (optional)

Gmail and Google Workspace mailboxes can be connected with **Google sign-in** (OAuth 2.0 authorization code + PKCE) instead of an app password. Owlat stores only the resulting refresh token — encrypted in the same envelope every external account uses — and the mail-sync worker authenticates to `imap.gmail.com` / `smtp.gmail.com` with SASL XOAUTH2.

This is **optional and additive**, and setting it up is entirely your call as the operator. The app password is the default path and needs no setup from you: the Gmail card always shows the email + app-password form, exactly as it did before this feature existed. Configuring a client only *adds* a "Sign in with Google" button beneath that form, which users can take instead of creating an app password. Skip this section and nothing is lost.

| Variable | Description |
| --- | --- |
| `GOOGLE_OAUTH_CLIENT_ID` | Client ID of the Google OAuth client. Unset ⇒ the Google button is not rendered; the Gmail card shows its app-password form as usual. |
| `GOOGLE_OAUTH_CLIENT_SECRET` | Client secret of the same client. Used only server-side, in the token exchange and refresh. Unset ⇒ same as above. |

**Creating the Google OAuth client.** In the [Google Cloud Console](https://console.cloud.google.com/):

1. **APIs & Services → OAuth consent screen** — pick **Internal** for a Google Workspace organization (no verification needed) or **External** for consumer Gmail accounts.
2. Add the scopes `https://mail.google.com/`, `openid` and `email`. `https://mail.google.com/` is the full IMAP/SMTP scope and is the only one that authorizes XOAUTH2.
3. **Credentials → Create credentials → OAuth client ID → Web application**, with the authorized redirect URI set to `${SITE_URL}/oauth/google/callback` (e.g. `https://mail.example.com/oauth/google/callback`). It must match byte for byte; a trailing slash on `SITE_URL` is stripped by Owlat.
4. Set the client ID and secret as Convex deployment env vars (the setup CLI and the web setup wizard push them for you).

::callout{type="warning" title="The Gmail scope is restricted"}
`https://mail.google.com/` is a **restricted** scope. An **External** consent screen left in *Testing* mode works for up to 100 named test users, which is enough for a small self-hosted deployment; publishing it to all users requires Google's app verification (including a security assessment). **Internal** (Workspace) apps need no verification.
::
...
- **`convex-fn-proxy`** holds the real `CONVEX_ADMIN_KEY` and forwards ONLY the thirteen Convex functions the worker legitimately calls (`codeWorkTasks:*` and `plugins/workerTasks:*`); any other function path is rejected `403`. The worker authenticates to it with `CODE_WORKER_PROXY_TOKEN`, which the proxy constant-time-compares, strips, and replaces with the admin key.
- **`code-worker-egress`** (tinyproxy) is a forward-proxy the worker's `HTTP_PROXY`/`HTTPS_PROXY` point at; it permits only the Convex proxy, the git remote and the LLM endpoint and denies all other outbound (`infra/code-worker/tinyproxy.conf` + `tinyproxy.filter`).
...
| `CONVEX_ADMIN_KEY` | Held by `convex-fn-proxy` (injected on forwarded requests). In a non-proxied deployment it is the worker's fallback credential. | (see [Required secrets](#required-secrets)) |

::callout{type="warning" title="Migration: enabling code tasks now needs CODE_WORKER_PROXY_TOKEN"}
...
### Optional: Sender trust registry (`ostr` flag)

The Open Sender Trust Registry is a public, verifiable reputation source for sending domains and IPs. These two point the Convex backend at the aggregator it reads standings from; both are consumer-side only — an instance that also *contributes* observations configures observer mode separately.

Set **both or neither**. A URL without its verifying key is a feed whose answers cannot be authenticated, and the Features page reports the `ostr` flag as needing configuration until both are present.

| Variable | Description | Default |
| --- | --- | --- |
| `OSTR_AGGREGATOR_URL` | Base URL of the aggregator that serves tier / score / explanation lookups (an `apps/ostr-registry` node — ours, your own, or a third party's). Required when the `ostr` flag is on; unset ⇒ there is no aggregator to consult and inbound mail is routed by exactly the signals it is routed by today. | — |
| `OSTR_AGGREGATOR_PUBLIC_KEY` | The aggregator's ed25519 public key. Required alongside the URL when the `ostr` flag is on. Every answer is signed at the source, so a mirror is an untrusted intermediary by design: an unverifiable answer is discarded rather than believed. Unset ⇒ answers cannot be authenticated and are treated as no answer at all. | — |

::callout{type="warning" title="These two are read by the MTA, not by Convex"}
The lookup that turns a sender into a tier runs on the **MTA** process, so both variables have to be set *there* as well: the Convex copies are what the Features page reads to decide whether the `ostr` flag is configured, and nothing in the backend queries an aggregator itself.

Turning the lookup on takes one more variable on the MTA — `OSTR_ENABLED=true`, off by default — plus something to ask: `OSTR_ZONE` for DNS queries, the aggregator pair for the signed snapshot, or both. `OSTR_LOOKUP_TIMEOUT_MS` (default `2000`) bounds each query; every failure is fail-open, so a lookup that times out leaves the message exactly as it would have arrived without OSTR. With the flag on in Convex but `OSTR_ENABLED` unset on the MTA, no tier is resolved and nothing is routed differently.
::

::callout{type="info" title="A tier is a signal, never a gate"}
Only the `flagged` tier — the one that needs strong, corroborated negative evidence from several independent observers — affects delivery, and all it does is file the message under Spam, exactly like an enforcing DMARC failure. `warned` and everything below it never move a message. The tier is recorded on every message whether or not the flag is on, so you can turn `ostr` on and see what it would have done first.
::

### Optional: Sender trust registry — observer mode

Reading the registry costs a DNS-shaped lookup. *Contributing* to it means turning your users' "Report spam" clicks into signed, permanently logged evidence about the domains that sent them mail — so observer mode is configured here, in the environment, rather than behind a feature flag. It is an operator's decision about their users' data, not a product surface anyone can toggle in the UI.

Observer mode ships **off**, and turning it on is not enough on its own: an instance below a mailbox floor is hard-disabled no matter what these variables say. For a one-mailbox instance the observer identity *is* the user identity — "`mx.example.com` received mail from X, one report" names the person who reported, and no amount of averaging fixes that. Small instances consume the registry without contributing, which the design explicitly expects.

| Variable | Description | Default |
| --- | --- | --- |
| `OSTR_OBSERVER_ENABLED` | Master switch for observer mode. Off ⇒ no DKIM evidence is retained, no report is queued, and both observer background jobs return having done nothing. On ⇒ evidence the MTA captured at verification time is stored (for ~90 days), a junk report becomes a queued commitment to it, and once an hour the instance publishes what clears the privacy floor. | `false` |
| `OSTR_OBSERVER_DOMAIN` | The FQDN this instance signs as — usually your MX host. It is the name in every attestation, and the name that must publish `_ostr.<domain>` carrying the public half of the key below. Unset ⇒ traffic is still counted and held, and nothing is published. | — |
| `OSTR_OBSERVER_PRIVATE_KEY` | The observer's raw 32-byte ed25519 private key, base64. Whoever holds it can publish reputation evidence in your name, into logs that keep it forever — treat it like `INSTANCE_SECRET`. Never fabricated and never defaulted: unset publishes nothing rather than inventing an identity. | — |
| `OSTR_LOG_URLS` | Comma-separated **full** submission endpoints of the transparency logs to cross-submit to, e.g. `https://log-a.example/v1/attestations,https://log-b.example/v1/attestations`. Two or more is the point: one log's outage — or one log's misbehaviour — must not be able to lose or bury evidence. One URL still publishes; it is simply not redundant. | — |
| `OSTR_MIN_MAILBOXES` | Raises the mailbox floor observer mode is disabled below. **Raise-only** — the packaged floor is the privacy minimum, so a lower value is ignored rather than honoured. Set it if your instance's mailboxes belong to fewer distinct people than their number suggests. | packaged floor |

::callout{type="warning" title="Evidence is captured at delivery or not at all"}
The DKIM key record and verification timestamp an evidence bundle needs cannot be reconstructed from a stored `.eml` once the sender rotates its key. Mail delivered before you enabled observer mode therefore has no evidence, and reports about it are silently not admissible. That is deliberate: the alternative is retaining signed headers speculatively on every deployment, including the ones that will never contribute anything.
::

::callout{type="info" title="What actually leaves your server"}
Windowed counts and hashes. A published `traffic-summary` carries message counts, SPF/DKIM/DMARC pass counts and log-bucketed recipient counts for one sending domain; a `spam-report-batch` carries a number of reports and a Merkle commitment. The evidence bundles themselves — which contain the signed `Subject` and `To` headers — never leave the instance, and reporters and recipients appear only as salted hashes that are counted and discarded. Anything that would describe a single user's mail, or a single user's complaint, is held back until the window is wide enough that it cannot.
::
...
| `TYPESAFE_API_KEY` | API key for the TypeSafe (Jev) **decision provider** — the third AI plane, which answers typed questions with calibrated probabilities (`lib/decisionProvider.ts`). The deployment-level equivalent of the key an admin can enter on the AI-provider page; a stored key wins when both are set. **Entirely opt-in**: with no key anywhere the decision plane resolves to the language-backed adapter, which is exactly how an install that never heard of this plane behaves. | — |
| `DECISION_PROVIDER` | Which decision adapter answers: `typesafe` or `llm`. Consulted only when the stored config names none; an unrecognised value is ignored rather than rejected, so a typo degrades to the language plane instead of failing the inbound path (`lib/decisionProvider.ts`). | unset (`llm`) |
| `DECISION_MODEL` | Decision model id override — the id set here is the id that is sent. Unset ⇒ the adapter's pinned version. Naming another one is allowed and never silent: an answer the provider reports against anything but the pinned version (or one of its aliases) comes back **uncalibrated**, so every threshold downstream goes inert until the calibration has been re-run. That one rule covers a deliberate bump and a vendor-side reroute alike. | unset (the adapter's pinned version) |
| `DECISION_BASE_URL` | Decision API **origin** override, e.g. a proxy you front the vendor with. The endpoint path is appended by the adapter, so this is an origin (`https://api.example.com`) and not a full endpoint URL. Any explicit value marks the endpoint `custom` for spend admission. | unset (the adapter's own origin) |
...
| `smtp` | SMTP relay — Mailgun, Postmark, SendGrid, Brevo or your own submission host | `SMTP_RELAY_HOST`, `SMTP_RELAY_USERNAME`, `SMTP_RELAY_PASSWORD` | `SMTP_RELAY_PORT`, `SMTP_RELAY_SECURE` | [Generic SMTP relay](#generic-smtp-relay) |
...
| `REDIS_URL` | Redis URL backing the LOGIN brute-force limiter. Set by both shipped compose files; **required in production** — the server refuses to start without it. | — |
| `IMAP_ALLOW_UNTHROTTLED_AUTH` | Set to exactly `true` to boot in production with no LOGIN brute-force protection. Only escape hatch from the `REDIS_URL` requirement. | `false` |
| `IMAP_RUNTIME_USER` | Compose-level `uid:gid` that must be able to read the published TLS key. Whatever writes the cert (`imap-cert-init`, or `acme-entrypoint.sh` on the VPS) chowns it to this. Change it only if you rebuilt the image with a different user — a mismatch makes the server EACCES on its own key and crash-loop. | `1000:1000` |
...
| `OSTR_AGGREGATOR_URL` | Convex | If `ostr` on | — | Sender trust registry aggregator consulted for inbound sender standing; set the same value on the MTA, which is where the lookup actually runs |
| `OSTR_AGGREGATOR_PUBLIC_KEY` | Convex | If `ostr` on | — | Ed25519 key that authenticates the aggregator's signed answers; an unverifiable answer counts as no answer |
| `OSTR_OBSERVER_ENABLED` | Convex | No | `false` | Observer mode: retain inbound DKIM evidence and publish signed windowed observations. Hard-disabled below the mailbox privacy floor whatever this says |
| `OSTR_OBSERVER_DOMAIN` | Convex | If observer on | — | The FQDN this instance signs attestations as; must publish `_ostr.<domain>` with the matching public key |
| `OSTR_OBSERVER_PRIVATE_KEY` | Convex | If observer on | — | Raw base64 ed25519 private key the observer signs with. Never logged, never returned to a client |
| `OSTR_LOG_URLS` | Convex | If observer on | — | Comma-separated full submission endpoints; two or more gives the cross-log redundancy the design assumes |
| `OSTR_MIN_MAILBOXES` | Convex | No | packaged floor | Raise-only override of the mailbox floor observer mode is disabled below |
...
| `GOOGLE_OAUTH_CLIENT_ID` | Convex | No | — | Google OAuth client ID for connecting Gmail with Google sign-in; unset ⇒ app-password form only |
| `GOOGLE_OAUTH_CLIENT_SECRET` | Convex | No | — | Secret of the same client, used only in the server-side token exchange/refresh |
...
| `TYPESAFE_API_KEY` | Convex | No | — | API key for the TypeSafe (Jev) decision plane; a key stored on the AI-provider page wins over it |
| `DECISION_PROVIDER` | Convex | No | `llm` | Decision adapter (`typesafe`, `llm`); read only when the stored config names none |
| `DECISION_MODEL` | Convex | No | pinned version | Decision model id override |
| `DECISION_BASE_URL` | Convex | No | — | Decision API origin override (origin only — the adapter appends the endpoint path) |
...
| `REDIS_URL` | IMAP env | Yes in production | — | Redis URL backing the LOGIN brute-force limiter. Set by both shipped compose files; the server refuses to start in production without it. |
| `IMAP_ALLOW_UNTHROTTLED_AUTH` | IMAP env | No | `false` | Set to exactly `true` to boot in production with no LOGIN brute-force protection |
| `IMAP_RUNTIME_USER` | Compose env | No | `1000:1000` | `uid:gid` the published TLS key is chowned to; a mismatch with the image's user crash-loops the server on EACCES |
