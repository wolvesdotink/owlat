...
| `REQUIRE_EMAIL_VERIFICATION` | Requires verified email for sign-in when enabled. **New deployments default it on**; existing installs remain unset so legacy members can still sign in. New signups always receive verification email and invitation acceptance always requires verified inbox ownership, regardless of this flag. A working system mail provider is required for new invitees. Enable sign-in verification on an existing install only after verifying or backfilling legacy users. Owners/admins can mark an existing member verified or resend verification (`auth/emailVerificationAdmin.ts`). | unset (off for legacy sign-in); **`true` on new installs** |
...
Nothing is captured unless the `analytics.posthog` [feature flag](/guide/feature-flags) is on. The flag ships off, and a configured key does not override it: with the flag off the browser never loads or initialises PostHog, and the backend drops every event it would otherwise send. Turn the flag back off later and capture stops on both sides — the browser opts out and forgets the identity it stored.
...
| Variable | Description |
| --- | --- |
| `POSTHOG_API_KEY` | PostHog project API key (starts with `phc_`). Required for server-side tracking, on top of the `analytics.posthog` flag. |
...
| Variable | Description |
| --- | --- |
| `NUXT_PUBLIC_POSTHOG_API_KEY` | PostHog project API key. Required for client-side tracking, on top of the `analytics.posthog` flag. |
...
- User identification and organization group association, by id only — no email address, display name, workspace name or slug is sent. PostHog holds product analytics, not a copy of your user directory; join the human labels from Owlat's own database when you need them.
...
Browser and desktop uploads use a single-use URL on `${SITE_URL}/api/storage/upload`.
The web server streams file bytes directly to Convex's native storage upload URL,
then records a receipt bound to the requesting user and organization. File binding
mutations consume that receipt once; knowing a storage ID does not grant access.

Set the same `INSTANCE_SECRET` in the web server and Convex runtime. Uploads fail
closed when this shared secret is missing or mismatched. During rotation, Convex
also accepts `INSTANCE_SECRET_PREVIOUS` until the web server has switched to the
new secret. `SITE_URL` in Convex must be the externally reachable web origin,
including for desktop clients. The web server reaches the small control endpoints
`POST /storage/upload/begin`, `/storage/upload/finish`, and `/storage/upload/abort`
on the configured Convex HTTP-actions origin; these accept the shared secret only,
never a browser session or the upload token alone.

Only the web upload endpoint needs browser CORS. Large file bodies bypass Convex
HTTP actions, whose request size limit is smaller than the supported 50 MiB library
and 64 MiB archive uploads. Failed uploads need a fresh URL. Pending capabilities
and unclaimed uploaded blobs expire after one hour and are swept periodically.
