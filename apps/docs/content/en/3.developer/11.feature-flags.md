...
The optional `ostr` flag (off by default, requires `postbox`) lets a verified registry tier of `flagged` route inbound mail to Spam and displays its tier in the reader. It requires an aggregator URL and verification key; observer publication is a separate operator opt-in. See [Trust registry](/developer/trust-registry).
