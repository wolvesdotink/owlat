...
| `requiresAny`     | Any-of dependency groups (`readonly FeatureFlagKey[][]`) — each group is satisfied when at least one member is on. Example: `postbox.aiDraft` declares `requiresAny: [['postbox', 'mail.external']]` (either mailbox source). Cascade-on never auto-enables a group member; the admin UI disables the toggle with a "needs one of: …" hint instead |
...
| Helper                                    | Use when                                                                                                                                                                  |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `resolveFlags(stored)`                    | Reading current state — applies `requires` / `requiresAny` cascades and returns the effective state (a flag is forced off when any of its any-of groups has no on member) |
...
- **Composed floors**: rather than repeating that call at the top of every handler, a gated module can bake the check into its builders — `featureGated(authedQuery, '<flag>')` for a single flag, `featureGatedAny(authedMutation, ['<a>', '<b>'])` when any one of several capabilities makes the surface legitimate. `chat/_helpers.ts` (`chatQuery` / `chatMutation`) and `mail/_helpers.ts` (`postboxQuery` / `postboxMutation`, on `['postbox','mail.external']`, mirroring the route's `requiresAnyFeature`) do this. The floor runs after the builder's auth floor and before the handler; per-record authz stays in the handler. Only queries and mutations can be gated this way — the check reads `ctx.db`, so a gated action keeps an in-handler check against a query it calls.
- **Soft gates for soft reads**: a `publicQuery` whose contract is "return the empty shape, never throw" should not gain a throwing floor, because it is often subscribed from a surface with no feature meta. Decide the flag where the read already decides access — `mail/permissions.ts` reports a `feature_off` outcome from the mailbox gate, so every mailbox-scoped read degrades to its existing empty result.
...
export const FEATURE_PACKS: Record<FeaturePackKey, FeaturePack> = {
	...
	ai: {
		flags: ['ai', 'ai.agent', 'ai.autonomy', 'ai.knowledge', 'ai.assistant', 'ai.visualizations'],
	},
};
```
...
2. **Decide dependencies**. Use `requires` for hard prerequisites and `requiresAny` when any one of several flags satisfies the need (e.g. "some mailbox source"); the resolver and the cascade-off helper handle the rest automatically — including turning the dependent off when the _last_ on member of an any-of group goes off. Cascade-on never picks an any-of member for the user.
...
6. **Gate the server**. At the top of every Convex public function behind the flag (after the auth check), call `await assertFeatureEnabled(ctx, '<key>')` from `apps/api/convex/lib/featureFlags.ts` — it reads the stored flags, resolves dependencies, and throws a `forbidden` error on its own. For a whole module, compose the check into a builder with `featureGated` / `featureGatedAny` instead of repeating it. A UI gate on its own is not enforcement: every public Convex function is reachable by any authenticated caller who knows the deployment URL.
