...
| `EHLO_HOSTNAME` | For delivery | `mail.localhost` | SMTP EHLO/HELO hostname. The placeholder lets the service boot, but its IP stays quarantined until the hostname matches a forward-confirmed PTR. |
| `EHLO_HOSTNAMES` | No | — | JSON map of sending IP to EHLO hostname for multi-IP deployments. |
| `MTA_GENERIC_PTR_SUFFIXES` | No | — | Extra comma-separated provider-default PTR suffixes to flag as reputationally poor. |
| `MTA_ALLOW_UNVERIFIED_FCRDNS` | No | `false` | Lab-only bypass for the FCrDNS quarantine. Never enable for internet delivery. |
| `MTA_IPV6_ENABLED` | No | `false` | Explicit outbound-IPv6 opt-in. IPv6 pool entries are rejected while false and remain gated by IPv4 identity, PTR/AAAA, and return-path SPF readiness when true. |
...
| Variable | Default | Description |
| --- | --- | --- |
| `OWLAT_DEPLOYMENT_MODE` | `selfhost` | Read by the web app (`config.public.deploymentMode`); compose forwards it as `NUXT_PUBLIC_DEPLOYMENT_MODE`, the only form Nitro maps at runtime. Controls the self-host onboarding banner. Leave as `selfhost` for an OSS deploy. |
...
| `CLAMAV_IMAGE` | `clamav/clamav` | ClamAV image repository. The alpine default is amd64-only; the setup wizard writes `clamav/clamav-debian` on arm64 hosts. Do not switch an existing amd64 install: the debian image runs ClamAV as a different uid and cannot read the alpine-owned signature volume. |
| `CLAMAV_VERSION` | `stable` | tag on `CLAMAV_IMAGE` |
| `TINYPROXY_VERSION` | `1.7` | `kalaksi/tinyproxy` (only used with `--profile inbox-codetasks`) |
...
| `EMAIL_PROVIDER` | Email provider: `mta`, `ses`, `resend`, `smtp`, `mandrill`, or `emailit`. No implicit default — leave it unset and sends are refused rather than routed to the built-in MTA. |
