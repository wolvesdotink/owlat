...
- `isBlocked` (session) — a point lookup. Other Convex functions read the list through `findBlockedByEmail` (`blockedEmails/lookup.ts`) with no access check; the worker's last gate before dispatching a marketing send (`delivery/marketingDispatchGate.ts`) is one of them. `isBlockedInternal` remains for one release only, for workers of the previous release still running during an upgrade.
...
- **From domain** — identical on both arms. Blocking. A per-_transport_ subdomain splits domain reputation and makes the arms incomparable; per-_stream_ subdomains (`news.`, `bounces.`) are the legitimate, separate thing and pass when both arms share them.
...
- **The own arm's SPF mechanisms come from `MTA_IP_POOLS`,** not from the domain's stored SPF value (which for an SES-registered domain _is_ the relay's include). With no pool configured the SPF check is `unknown`, never a pass on a relay-only record.
