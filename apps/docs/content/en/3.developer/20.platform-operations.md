...
This OSS repository ships **one** platform-admin-gated page: **Settings → System & Updates** (`/dashboard/admin/system`). The richer control-plane UI that consumed the queries below was extracted into a separate private repo. Treat the roster, abuse-status, and content-review functions documented here as a **backend/API surface** you call from your own tooling or scripts — not as an in-product dashboard.
::
...
Platform admins are rows in the `platformAdmins` table, keyed by BetterAuth user id. Two roles exist: `admin` and `superadmin`. An operator may create the first admin by hand with `convex run migrations/0036_seed_platform_admin:run`; it only succeeds while the table is empty and always seeds a `superadmin`. After that, `superadmin`s manage the roster.
...
| `migrations/0036_seed_platform_admin:run` | Hand-run internal mutation | Optional one-shot bootstrap of the first `superadmin` |
