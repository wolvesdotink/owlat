...
The bounce server mirrors the dispatch architecture: a typed phase pipeline (`bounce/pipeline.ts` + `bounce/phases/`) classifies each message into a `BounceAttempt`, and the pure reducer in `bounce/outcome.ts` emits a typed list of `BounceEffect`s (Convex webhooks, circuit-breaker outcomes, quota bumps). Attachment bytes are never among them: the inbound payload carries filename, content type and size only, and the raw RFC822 Convex already stores is what downstream readers re-extract MIME parts from.
