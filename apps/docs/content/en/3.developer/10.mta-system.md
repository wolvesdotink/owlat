...
| `GET` | `/queue/pending` | Master | Bounded sample of queued jobs (`?limit=`, max 200; `?domain=`) |
...
while it waits — so `delayed` is a subset of `waiting`, not a sibling of it, and
the two do not sum. Message bodies are never returned: both job endpoints report
identifying metadata and body sizes only.

`/queue/pending` returns a bounded sample rather than a page — it reports
`waiting` (the whole queue) alongside `sampled` (what the scan saw) so a partial
view is never mistaken for an empty one.

There is no bulk flush. GroupMQ groups jobs by `{ipPool}:{recipientDomain}`, so
the queue has no organization dimension to flush along; the only implementation
would read every queued payload, which is unbounded work on exactly the backlog
such a call gets reached for, and would destroy deliverable mail from one
unconfirmed request. Remove jobs individually, or see
[Maintenance & Updates](/developer/self-hosting-maintenance) for clearing the
queue wholesale.
