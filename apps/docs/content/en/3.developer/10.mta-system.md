...
This list is the last-hop backstop, not the record: Owlat's blocklist is the source of truth, and every bounce, complaint, or manual block added there is mirrored here (an `unengaged` sunset suppression is not — it must not block transactional mail). Removing a mirrored block in the app also removes its MTA copy. A daily reconciliation rebuilds missing entries after transient failures and removes Redis-only orphans, including entries left by older expiry behavior.
