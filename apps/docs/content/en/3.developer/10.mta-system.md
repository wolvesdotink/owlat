...
Retention differs by reason, deliberately. `hard_bounce` and `complaint` never expire — evidence about a mailbox does not go stale. A `manual` entry added through `POST /suppression` expires after seven days, which is how a soft-bounce escalation gets a second chance once the mailbox has had time to recover. Entries added through `POST /suppression/bulk` never expire whatever their reason, because that endpoint is how an operator carries an existing suppression list onto this MTA. Expiring entries are indexed by due date in `mta:suppressed-expiring` and reclaimed by an hourly leader-side sweep; a permanent entry is never in that index, so nothing can sweep it.
