...
| `soft_bounce` | Added after repeated temporary delivery failures |
...
Retention differs by reason, deliberately. `hard_bounce`, `complaint`, and `manual` never expire; an explicit operator block remains until an operator removes it. Only `soft_bounce` expires, after seven days. The single and bulk endpoints apply the same policy.
...
This list is the last-hop backstop. Owlat's blocklist owns the entries it mirrors here: every bounce, complaint, or manual block added there is mirrored (an `unengaged` sunset suppression is not — it must not block transactional mail). Removing a mirrored block in the app also removes its MTA copy. A daily reconciliation rebuilds missing mirrors after transient failures and removes stale Convex-owned copies. Independently created MTA blocks (including personal-mail hard bounces) and metadata-less entries are preserved. Soft-bounce repairs retain the original seven-day deadline; reconciliation does not renew it.
