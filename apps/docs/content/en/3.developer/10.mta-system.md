...
| `GET` | `/identity` | Master | Stored FCrDNS / source-address readiness per sending IP |
| `POST` | `/identity/recheck` | Master | Re-observe reverse DNS now and return the fresh verdicts (what `owlat doctor` and the installer call after a PTR change, instead of waiting for the hourly sweep) |
...
Adaptive throttling profiles can be tuned at runtime without redeploying. Profiles are seeded from `DESTINATION_PROVIDER_PROFILES` in `src/config/ispProfiles.ts` on startup (via `HSETNX`, so runtime overrides survive restarts) and managed through `src/routes/ispProfiles.ts`.
...
| Method | Path | Auth | Description |
| --- | --- | --- | --- |
| `GET` | `/queue/stats` | Master | Depth by state (waiting, active, delayed, completed, failed), group count, and the delay-set verdict `/health` reports |
| `GET` | `/queue/pending` | Master | Head of the queue (`?limit=`, max 200; `?domain=` for one recipient domain) |
| `GET` | `/queue/jobs/:jobId` | Master | One job's resolved state, attempt count and payload metadata |
| `DELETE` | `/queue/jobs/:jobId` | Master | Remove a queued job. Refuses (`409`) one a worker is already delivering |

`waiting` counts every job held in a group, and a delayed job stays in its group
while it waits — so in a healthy queue `delayed` is a subset of `waiting`, not a
sibling of it, and the two do not sum. That containment is a property of the
healthy state only: a delay-set member whose group entry is gone is counted in
`delayed` and not in `waiting`, so `delayed` can exceed `waiting`. Read that
inversion as a diagnosis rather than an error — it means the delay set is
holding entries nothing can drain, and `delayedQueue.status` names it.

`completed` and `failed` are retention-window sizes, not lifetime totals: the
queue keeps a bounded number of settled jobs (`keepCompleted` / `keepFailed`)
and discards the rest. `/delivery-logs` is bounded too (`DELIVERY_LOG_MAX_LEN`,
`DELIVERY_LOG_TTL_HOURS`). The lifetime record of what was sent lives in Convex,
written by the delivery webhooks.

Message bodies are never returned: both job endpoints report identifying
metadata and body sizes only. The sizes under `bytes` are all byte counts and
each is named for what it counts (`htmlBytes`, `textBytes`, `ampBytes`,
`sealedMimeBytes`, `attachmentCount`, `attachmentBytes`).

`/queue/pending` returns the head of the queue rather than a page — it reports
`waiting` alongside `sampled` (what the scan saw) so a partial view is never
mistaken for an empty one. With `?domain=` both numbers are scoped to that
recipient domain and both are exact: the filter is resolved against the
`{ipPool}:{recipientDomain}` groups that encode the domain, not by filtering a
sample, so a large backlog for one domain can never come back as "nothing
queued".

There is no bulk flush. GroupMQ groups jobs by `{ipPool}:{recipientDomain}`, so
the queue has no organization dimension to flush along, and no bulk remove along
the dimension a flush would need. The only implementation would read every
queued payload, which is unbounded work on exactly the backlog such a call gets
reached for, and would destroy deliverable mail from one unconfirmed request.
Remove jobs individually.

To discard the whole queue after a misconfiguration, delete the queue's own keys
and nothing else:
...
::callout{type="warning" title="Never FLUSHALL this Redis"}
The MTA's Redis is also the instance's secret store: it holds the DKIM **private
keys** (`mta:dkim:{domain}`), which exist nowhere else — Convex only ever
receives the public half — plus per-organization API credentials and warmup
state. `FLUSHALL` destroys them, and every later message is then signed with a
fresh key whose public half does not match the published `._domainkey` record.
See [Redis Maintenance](/developer/self-hosting-maintenance#redis-maintenance).
::
...
The `/send` endpoint accepts both the master `MTA_API_KEY` and per-organization API keys (`owlat_...`). All management endpoints — `/credentials`, `/dkim`, `/identity`, `/org-limits`, `/pool-rules`, `/suppression`, `/inbound/routes`, `/mailboxes`, `/isp-profiles`, `/ip-reputation`, `/delivery-logs`, `/queue`, `/dlq`, and `/scan` — require the master key (enforced inside each route group).
::
...
- **`src/queue/handler.ts`** is the only GroupMQ-coupled file. The pipeline never throws; the handler re-enqueues a `defer` outcome itself as a successor job carrying the computed delay (with ±15% jitter to avoid a thundering herd), so a deferral never consumes a delivery attempt, and translates a `drop` into a logged terminal outcome.
...
Retention differs by reason, deliberately. `hard_bounce` and `complaint` never expire — evidence about a mailbox does not go stale. A `manual` entry added through `POST /suppression` expires after seven days.

That expiry is not a second chance for the mailbox. Nothing re-tests the address, and the blocklist row that caused the mirror (below) never lapses on its own, so every send path Owlat gates goes on refusing it. What the seven days bound is the backstop COPY: the mirror is one-way and insert-only, so an address an operator later unblocks in the app would otherwise stay undeliverable through this MTA forever. The difference is visible only on the paths with no Convex-side gate — system mail including double-opt-in confirmations, Postbox outbound, operator test previews — and on those a `manual` suppression does lapse after a week.

`POST /suppression/bulk` writes every entry permanently whatever its reason. Nothing in Owlat calls that endpoint: the migration import walks each carried-over address through the same per-address mirror, which `POST`s to `/suppression`, so an imported entry that maps to `manual` expires here after seven days like any other. The import is permanent in Owlat's blocklist, which is the record; bulk permanence is a property of the endpoint, not a guarantee about imports.

Expiring entries are indexed by due date in `mta:suppressed-expiring` and reclaimed by an hourly leader-side sweep; a permanent entry is never in that index, so the sweep cannot reach one. `GET /suppression/stats` reports the current count by reason.

This list is the last-hop backstop, not the record: Owlat's own blocklist is the source of truth, and every bounce, complaint or manual block added there is mirrored here (an `unengaged` sunset suppression is not — it is a marketing-hygiene decision and must not block transactional mail). The mirror is one-way and insert-only — removing an address from the blocklist in the app does **not** remove it here. Clear the backstop copy too with `DELETE /suppression/:email`, or the address stays undeliverable through this MTA.
...
**A retry interval a remote server names is clamped at one hour.** `extractGreylistDelay` reads `try again in N seconds/minutes` out of response text, which is the only delay in the MTA whose value comes from a stranger — everything else (the provider signatures, the warming cap window, the breaker cooldown) is ours and already bounded. An unbounded parse let `try again in 999999 minutes` schedule a retry ~694 days out: the job holds its payload in Redis for the whole wait, and when it wakes its defer handoff receipt (TTL = the four-day max message age) is gone, so the message dead-letters instead of bouncing. One hour is the longest interval the MTA asks for anywhere else, and far longer than any real greylist window. When the clamp bites it says so — the deferral's delivery-log event carries an annotation naming what the receiver asked for. Independently of the parse, no deferral may schedule a retry past the message's own expiry: a wait that would outlive the message is pulled back to the deadline, where it earns a proper expired bounce.
...
1. `analytics/engagementScore.ts` computes a recency-weighted 0–100 score per contact from their activity history and stores it on `contacts.engagementScore`.
2. **Two** of the send producers put that value on the durable send **envelope** at enqueue time, and the value is normalised there (in the enqueue mutation, and at the contact read itself on the non-campaign path) so nothing degenerate can enter a durable envelope:
   - campaign sends — audience resolution projects the score from the contact row it has already loaded, and `campaigns/send.ts` carries it through to `delivery/enqueue.ts:enqueueCampaignEmails`;
   - automation and agent-reply sends — `delivery/nonCampaignIntake.ts:intake` does a single indexed point read of the contact inside the existing enqueue transaction.

   The **template-API transactional producer** (`transactional/dispatch.ts`) is deliberately **not** wired: it resolves a contact (it writes `contactId` onto the `transactionalSends` row) but does not project that contact's score onto the envelope, so a template-API receipt to a fully scored contact is dispatched **without** a score and is ordered in the default band. That is intentional — transactional receipts are latency-sensitive one-offs, not a volume stream whose ordering is worth differentiating — not an oversight to read past.

   The dispatch action never reads a contact per recipient.
3. `delivery/governedDispatch.ts` normalises the envelope value again at the read boundary and stamps it onto `MtaExtras.engagementScore`; the MTA adapter forwards it in the `POST /send` body.
...
When a dispatch phase returns a `defer` outcome, `src/queue/handler.ts` enqueues a successor job with the requested delay (plus ±15% jitter) and completes the current one, so a deferral never consumes a delivery attempt. The delay is bounded by the message's own remaining lifetime: a wait that would outlive the four-day max message age is pulled back to the deadline, because a successor that wakes after its defer handoff receipt has expired can only dead-letter.

### Delay-set integrity

GroupMQ's `:delayed` sorted set is the one structure in the retry ladder with no owner outside the promoter — a member is put there by a delayed enqueue and removed by the promoter, or by nothing at all. A member whose job hash is gone therefore accrues invisibly: it is not a job anyone is waiting on, not a Send anyone will report, and not a queue depth anything reads. Left unnamed it grows until Redis runs out of memory.

`GET /health` reports a `queue` block with the delay set's size, how many members are overdue by more than ten minutes, and how many of a fixed-size sample of those overdue members have no payload left:

| `status` | Meaning |
| --- | --- |
| `ok` | Nothing is overdue. |
| `behind` | Overdue members, all with their payloads intact — the promoter is lagging, which time alone fixes. |
| `orphaned` | At least one overdue member has no payload. Nothing can deliver it and nothing will reclaim it. |
| `unknown` | The probe could not read Redis. |

On a healthy queue this is two `O(log N)` sorted-set reads; the sample is only taken when something is overdue and never grows with the backlog it describes. `orphaned` is logged as a warning by the MTA and named by `owlat doctor`. It is deliberately **not** folded into the health endpoint's top-level `degraded` verdict: a leak in the retry ladder is not a reason to tell an operator their delivery path is down.
...
- MTA-STS enforcement: when the recipient domain publishes an enforce-mode policy, the connection requires TLS and validates the peer certificate, and MX hosts not listed in the policy are skipped (see [MTA-STS & TLS-RPT](#mta-sts-tls-rpt))
...
| TLS handshake window | 30 s (implicit-TLS listener only) |
...
| `SUBMISSION_IMPLICIT_TLS_ENABLED` | `false` | Also listen on an implicit-TLS port, encrypted from the first byte (RFC 8314) |
| `SUBMISSION_IMPLICIT_TLS_PORT` | `465` | Implicit-TLS submission port |

**Handshake window.** On the implicit-TLS listener the TLS handshake happens *before* any SMTP session exists, so the command idle timer that bounds a stalled 587 connection is not armed yet. A peer that completes the TCP handshake and then sends nothing is bounded by a **30-second** TLS handshake window instead, after which the connection is logged and dropped. The window is a fixed policy value, not an environment variable: it is wide enough for a lossy mobile link to retransmit its ClientHello several times (a TCP RTO ladder costs ~7 s for three losses and ~15 s for four) and far tighter than Node's 120-second default, which lets an unauthenticated peer pin a connection slot for two minutes. The 587 listener is unaffected — its pre-STARTTLS silence is bounded by the command idle timer.
...
The bounce server mirrors the dispatch architecture: a typed phase pipeline (`bounce/pipeline.ts` + `bounce/phases/`) classifies each message into a `BounceAttempt`, and the pure reducer in `bounce/outcome.ts` emits a typed list of `BounceEffect`s (Convex webhooks, circuit-breaker outcomes, quota bumps). Attachment bytes are never among them. On the routed-inbound (`inbound.received`) path the payload carries attachment **metadata only** — filename, content type and size — and the bytes are not available downstream at all. The personal-mailbox (`inbound.mailbox.received`) path is the one that keeps them: it ships the whole message as raw RFC822, which Convex stores and the Postbox reader re-extracts MIME parts from.
...
Round-robin IP selection within each pool. Any composed readiness quarantine
(FCrDNS, IPv4 prerequisite, SPF, or critical DNSBL) excludes the address. If no
eligible address remains, delivery stays queued; the sender never falls back to
...
The Convex backend receives MTA webhooks at the static route `POST /webhooks/mta`, served by the shared provider-feedback HTTP action (`webhooks/providerFeedbackHttp.ts`, one parameterised handler for every kind). Bounce and complaint events are parsed by `webhooks/adapters/mta.ts` — the adapter the route resolves through `webhooks/adapters/index.ts` — into typed `email.bounced` / `email.complained` `InboundEvent`s and routed through the shared inbound webhook pipeline (the same `runInboundPipeline` dispatcher SES/Resend/Mandrill use).
...
See [External reputation feedback](/developer/external-reputation-feedback) for
the required read-only scopes and provider enrollment steps.
...
| `SUBMISSION_IMPLICIT_TLS_ENABLED` | `false` | Enable the implicit-TLS submission listener (port 465); its TLS handshake window is a fixed 30 s |
| `SUBMISSION_IMPLICIT_TLS_PORT` | `465` | Implicit-TLS submission port |
...
1. Stop bounce/FBL SMTP traffic at the load balancer or firewall for **every**
   replica. Wait for every legacy handler to finish, then stop every old MTA.
2. While intake remains stopped, deploy the new binary and set these values on
   every replica:
   ...
   The acknowledgement is an operator assertion, not a distributed lock. Never
   set it while an old binary can still accept or process feedback.
3. Start the entire owned-v2 fleet, then resume SMTP traffic. Owned-v2 ignores
   residual legacy `1` values and uses only its isolated version-2 hash with
   owned `reserved`, `retryable`, and `completed` states. This is safe only
   because step 1 ensures no legacy worker can race or create another marker.
4. Do not run an old and owned-v2 binary concurrently. A rollback requires the
   same coordinated intake quiescence; old binaries cannot interpret v2 state.
