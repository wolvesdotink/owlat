...
Owlat verifies the full chain before selecting an address: the PTR must exist,
contain a valid FQDN, resolve forward to the sending IP, and match the EHLO name.
The result is stored continuously and checked hourly. A hard failure quarantines
the IP, so an install with provider-default or missing reverse DNS cannot silently
send. Provider-default PTRs that do forward-confirm remain usable but appear as a
reputation warning in **Delivery health → Outbound IPs**.

For a multi-IP pool, give every address its own identity:

```env
EHLO_HOSTNAMES={"203.0.113.10":"mail1.example.com","203.0.113.11":"mail2.example.com"}
```

**Settings → Email delivery → Delivery provider** builds this line for you from an
(IP, hostname) table, with the matching `owlat env` command.

`MTA_ALLOW_UNVERIFIED_FCRDNS=true` is an explicit lab-only escape hatch for
private/local IPv4 experiments. It never admits an unverified IPv6 address.
...
Run `owlat doctor` after changing reverse DNS. It reports the exact PTR value Owlat
expects and tailored console steps for Hetzner, DigitalOcean, and OVHcloud.

::callout{type="info" title="A PTR you just fixed shows up immediately"}
The MTA observes reverse DNS at boot and then hourly, and `/health` reports that
STORED verdict — so a PTR corrected five minutes ago would otherwise still read as
failing, and `docker compose up -d` does not restart an unchanged container to
re-observe it. `owlat doctor` and the installer's identity gate therefore force a
live re-check first (`POST /identity/recheck` on the MTA, master-key protected)
rather than trusting the stored one. If a re-check still disagrees with `dig -x
<ip>` run from your own machine, the container's DNS resolver is holding the old
answer (PTR TTLs run to a day): `docker compose restart mta`, then re-check.
::

### Optional outbound IPv6

Outbound IPv6 is off by default because it offers no automatic deliverability
advantage and receivers are unforgiving of incomplete identity. Docker bridge
IPv6 also does not provision a public address for you. Keep IPv4 working and
enable IPv6 only in this order:

1. Assign one stable, publicly routed IPv6 address to the MTA host/container and
   confirm the source-bound SMTP probe can use it.
2. Set that address's PTR to its non-generic EHLO hostname. `dig -x
   2001:db8::10` must return that exact FQDN.
3. Publish an AAAA record from the EHLO hostname back to the exact address.
4. Add `ip6:2001:db8::10` to the sending and return-path SPF records. The Owlat
   MTA-domain generator emits `ip6:` automatically from canonical IPv6 values in
   `MTA_IP_POOLS`.
5. Only then set `MTA_IPV6_ENABLED=true` and add the bare address (no brackets,
   port, CIDR, or zone id) to `IP_POOLS_*`.

Owlat checks these as live state, not a manual checkbox. Every IPv4 identity must
remain green; IPv6 PTR/AAAA and exact return-path SPF are rechecked hourly. A
confirmed regression quarantines IPv6 and invalidates its pooled sockets while
eligible IPv4 continues.

Provider paths, verified against their current documentation:

- [Hetzner Cloud rDNS](https://docs.hetzner.com/cloud/servers/cloud-server-rdns/)
  accepts the IPv6 interface identifier in Console → Server → Networking.
- [DigitalOcean IPv6](https://docs.digitalocean.com/products/networking/ipv6/how-to/enable/)
  must be enabled and routed on the Droplet; DigitalOcean documents PTR creation
  from a valid FQDN Droplet name and supports PTR only for the first assigned
  IPv6 address.
- [OVHcloud reverse DNS](https://docs.ovhcloud.com/en/guides/bare-metal-cloud/virtual-private-servers/configuring-reverse-dns)
  is under Network → Public IP Addresses → Configure reverse DNS. Confirm your
  product exposes the chosen IPv6 there before adding it to a pool; the cited
  provider guides do not document reverse-zone delegation.
...
Owlat enforces a checked-in adaptive schedule rather than generic
week-by-week estimates. It begins at 50 messages per IP on warming day 1,
reaches checkpoint caps through day 25, and removes the cap at day 30 or later
only after its graduation health gate passes. No-send days do not advance the
state; good signals can accelerate it, while bounces and deferrals slow or
pause it.

See [Sending from a VPS — What warming weeks 1–4 look like](/guide/sending-from-a-vps#what-warming-weeks-1-4-look-like)
for the exact, test-guarded checkpoints and adaptive thresholds. Focus first on
recent, explicit opt-ins who expect the message.
