...
Track-and-thumb toggle with `role="switch"` semantics — the on/off control used across settings, preferences, and config cards. Use it for every on/off setting and let the switch show the state: give it a fixed name (the setting, not "On"/"Off") via `label` or a `<label for>`. Keep checkboxes for multi-select lists, segmented controls for settings with more than two states, and a bespoke control for tri-state (on/partial/off) feature packs.
