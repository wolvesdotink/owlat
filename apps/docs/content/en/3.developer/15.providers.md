...
Three pluggable provider abstractions ship: the **LLM provider**, the **send (email) provider**, and the opt-in **decision provider** (a third AI plane that answers typed questions rather than writing text — off unless an admin configures it). The first two are fully consumed by the backend (agent steps, knowledge extraction, translation, every send path). The LLM provider reads one env var (`LLM_PROVIDER`) and caches the resolved client per-process. The **send (email) provider** is the exception: it uses a static registry keyed by provider `kind` (passed in per call) instead of reading one env var and caching the resolved instance. That difference is called out below.
...
The two always-present abstractions live under `apps/api/convex/lib/` (the decision plane's own files are listed with it below):
...
## Decision provider (the third AI plane)

**Stored config:** the AI-provider admin page. **Env fallback:** `DECISION_PROVIDER`, `TYPESAFE_API_KEY`, `DECISION_MODEL`, `DECISION_BASE_URL`.

**Supported values:** `llm` (default), `typesafe`

The LANGUAGE plane writes and the EMBEDDING plane retrieves. The **decision** plane does neither: it answers typed questions — yes/no, pick one of these categories, place this on this ordered scale — and returns a probability distribution over the options that were sent, instead of a sentence with a self-reported confidence field inside it. It is a plane of its own because those probabilities do not survive the `LanguageModel` interface the other two speak (see ADR-0060).

```
lib/
├── decisionProvider.ts         ← resolves the plane: stored config → env → default
├── decisionProviders/
│   ├── types.ts                ← DecisionProviderAdapter<K>, kinds, endpoint provenance (pure)
│   ├── index.ts                ← DECISION_PROVIDERS registry + decisionProviderFor(kind)
│   ├── typesafe.ts             ← TypeSafe (Jev) — POST <origin>/v1/systemone
│   ├── llm.ts                  ← language-backed adapter — what an install with no key runs
│   └── wire.ts                 ← request/response codec
└── decision/
    ├── questions.ts            ← noul() / choice() / score() and the answer types (pure)
    ├── catalog.ts              ← the one registry of question sets the product asks
    ├── contract.ts             ← what runDecision takes and answers in, and its ports
    ├── dispatch.ts             ← runDecision() — deadline, retry, one-hop fallback, accounting
    ├── ports.ts                ← the ctx-bound ports: breaker, usage recorder
    └── pricing.ts              ← list price + spend admission for the plane
```

Two adapters ship. `llm` renders a question set onto the language model the deployment already has, and stamps its result `calibrated: false`; it is what an install with no decision key resolves to, and it reproduces the behaviour the product had before this plane existed. `typesafe` speaks to the vendor's own endpoint and returns calibrated probabilities. Call sites import `runDecision` and their own question set — never an adapter — so switching between the two is a dropdown.

Every question set the product asks is registered in `catalog.ts`, and one test
suite runs the whole catalog through **both** adapters, asserting they answer
with the same keys, the same value domains and the same probability key spaces —
`calibrated` is the only thing allowed to differ. That is what makes the dropdown
a real choice rather than a hope.

The model version is pinned, and the pin is enforced on the **answer**: the id
you configure (settings picker or `DECISION_MODEL`) is the id that is sent, and
anything the provider reports back other than the exact pinned version
comes back uncalibrated, which puts every threshold downstream to sleep
until the calibration has been re-run. The settings page's **Test** button makes
one real request — a single yes/no question over a nine-word state — so a revoked
key, a base URL that redirects and a retired model version are all found at the
button rather than later, on the inbound path.

Nothing here is on after an upgrade. The `ai.decisionPlane` feature flag is **off** by default, requires `ai`, and is deliberately **not** a member of the `ai` feature pack: one click on a pack must not start sending message content to a provider nobody chose. With no key configured anywhere the plane resolves to `llm`.

::callout{type="warning" title="Data flow: what leaves your deployment, and how to stop it"}
**What leaves.** Only when the flag is on *and* a `typesafe` key is configured. Each call sends the **state** of one decision — message text reduced to visible text, quoted history and signatures stripped, attachments dropped, capped at a character budget — plus the text of the questions themselves. Whatever that visible text still contains travels with it, names and email addresses included. Attachments, images and stored credentials never do. What comes back is a set of numbers.

**To which host.** `POST https://api.typesafe.ai/v1/systemone`, or the origin you set in `DECISION_BASE_URL` if you front the vendor with your own proxy. HTTPS only, through the SSRF guard (`lib/ssrfGuard.ts`), which refuses redirects — a 3xx from that host surfaces as an error rather than following a hop you did not configure.

**Under whose key.** Yours. Owlat ships no key, brokers none and proxies nothing. You open an account with the vendor and either paste the key on the AI-provider page (sealed with the instance data key, decrypted only inside a Node action, never returned by a query) or set `TYPESAFE_API_KEY` on the deployment.

**How to turn it off.** Three switches, coarsest first: turn the `ai.decisionPlane` flag off — one click, no redeploy, every judgement goes back to your language model; or set the decision provider back to the language model on the AI-provider page; or clear the key. An install that never configured one is already in that state.

**Which mailboxes are covered.** Today, none: the plane ships with no caller, so the only request that leaves a deployment is the one the settings page's Test button makes. As call sites migrate, a mailbox excluded from third-party decisions is excluded before the state is ever assembled — the exclusion is a property of the mailbox, not something each caller re-decides. Until that per-mailbox control is on screen, the flag above is the exclusion, and it is deployment-wide.

**Who is responsible.** You are. In a self-hosted deployment **you** are the data controller and the decision vendor is **your** processor — not ours. We are not a party to that relationship, we cannot sign anything on your behalf, and there is no subprocessor list in this repository that your deployment appears on. Before enabling it, get the vendor's region, retention window and model-training position in writing, and hold them against what you have told your own users.
::
...
Every cell above is the value declared in `packages/shared/src/sendProviderCatalog.ts` — the table restates the catalog, it never defines it. `apps/docs/__tests__/pageInvariants.test.ts` reads both and fails if this page stops naming a kind the catalog declares, and `apps/docs/__tests__/docsVocabulary.test.ts` fails on any name here that the code no longer declares — so a cell that disagrees with the code is a test failure rather than something a reader has to catch.
...
- **Governed dispatch** (`delivery/governedDispatch.ts`) — the pre-dispatch identity binding, the message-id substitution, the `isCustodyHandoff` verdict and the replay of an ambiguous acceptance all read `acceptanceSemantics` and `messageIdSource`. That file no longer compares a provider kind to a literal at all; a test asserts it.
...
| 6 | Docs | this page — a section here, and a row you write in the [environment-variables reference](/developer/environment-variables#provider-selection); there is no generator, so both are hand-written and both are **pinned** to the entry by `apps/docs/__tests__/pageInvariants.test.ts`, which fails when a kind is missing from this page or the reference documents a Convex variable the backend never reads | always | The package `README.md`, and the package listed in `plugins.config.ts` — which is also what makes it *bundled* rather than merely written |
