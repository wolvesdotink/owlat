...
This guide covers hardening your self-hosted Owlat instance for production use.
For initial setup, see [Self-Hosting](/developer/self-hosting). Before exposing
the built-in MTA, complete the provider, outbound-port, reverse-DNS, and warm-up
preflight in [Sending from a VPS](/guide/sending-from-a-vps).
...
# Allow IMAPS — ONLY if you run the personal-mail feature, for mail clients
# (skip this line otherwise; nothing listens on 993 without it)
...
Your own firewall is only half the question. Hosting providers routinely block
outbound ports — mail ports above all — and a blocked one does not announce
itself: connecting an existing mailbox simply times out, and direct delivery
stops with the queue filling up. Ask your provider to open these for the
machine, in the same request as the reverse-DNS change.

| Port | Needed for | Required when |
| --- | --- | --- |
| 443/tcp | Image pulls, update checks, every provider and AI API | Always |
| 53/udp+tcp | MX lookups, domain verification, blocklist checks | Always |
| 25/tcp | Delivering straight to recipient mail servers | The built-in MTA is your delivery provider |
| 587/tcp | Handing mail to an external SMTP relay | Your delivery provider is a generic SMTP relay |
| 465/tcp | Sending from a connected mailbox over implicit TLS | `mail.external` — connected mailboxes |
| 993/tcp | Reading a connected mailbox over IMAP | `mail.external` — connected mailboxes |

Two of these are worth calling out because providers get them wrong together:

- **The mail ports are commonly blocked, and not as a set.** Hetzner Cloud
  blocks 25 and 465; DigitalOcean blocks 25, 465 and 587; OVHcloud blocks 25.
  See [Sending from a VPS](/guide/sending-from-a-vps) for each provider's
  policy and unblock path. Submission (587) being open implies nothing about 25,
  and an unblock request granted for one port is not granted for the others —
  name every port you need in the same ticket.
- **53 to anything but the provider's own resolver** is blocked more often than
  people expect. That alone halts sending: Spamhaus refuses queries from shared
  provider resolvers with a policy answer, Owlat cannot measure whether your IP
  is listed, and it fails closed rather than sending blind.

::callout{type="info" title="Check them from the instance"}
**Administration → System & Updates → Network ports** probes these ports from
the server itself and reports, per port, what it found and which feature depends
on it. The probes only run when you press the button: each opens a connection to
a well-known host and closes it immediately, sending nothing.

Two limits are worth knowing. The inbound rows are probed from inside the Docker
network, so they prove a service is **listening** — whether the internet reaches
it depends on the firewall in front, which nothing on the machine can test. And
the DNS row asks whether your resolver answers at all; it does not detect the
policy refusal described above, where the resolver answers everything except
blocklist queries.
::
...
Redis auth ships enabled and is mandatory. `docker-compose.yml` interpolates `${REDIS_PASSWORD:?…}` in the Redis command, its healthcheck, and the MTA's `REDIS_URL` — there is no override to write and no unauthenticated fallback. The setup wizard and `owlat quickstart` generate the password when they write `.env`.

::callout{type="warning" title="Upgrading an .env that predates this"}
An `.env` written before Redis auth became mandatory has no `REDIS_PASSWORD`, and compose interpolates the whole file before it selects services — so **every** subcommand fails, the recovery ones included: `docker compose down`, `logs` and `ps`.
::

Generate the missing key, then bring the stack up:
...
2. `git clone` the Owlat repo (same version as the backup — the MANIFEST
   records when it was taken) and `cd` into it. Use the directory name the old
   server used (`owlat` for a plain clone): Compose names the volumes after it.
   If the names differ, the restore warns and restores into the volumes this
   checkout's `docker compose up` will use.
...
4. `bash scripts/restore.sh owlat-<timestamp>.tar.gz` — this verifies the
   checksum and every volume payload, restores every volume plus
   `.env`/override/Caddyfile, and starts the stack with your feature
   profiles. You do not need to create a `.env` first: until the restored
   one is in place, the script gives Compose the `.env` from the archive. It
   stops on the first failure and never reports success unless the stack
   started (see
   [restore from backup](/developer/self-hosting-maintenance#nuclear-option-restore-from-backup)).
