...
# Allow IMAPS — only with the personal-mail feature, for mail clients
...
- **25, 465 and 587** are blocked by default on most VPS products. See
  [Sending from a VPS](/guide/sending-from-a-vps) for each major provider's
  policy and unblock path. Submission (587) being open does not imply 25 is.
...
**Administration → System & Updates → Network ports** probes every port in both
tables from the server itself and reports which are open, which are blocked, and
which feature each one carries. The probes only run when you press the button:
each opens a connection to a well-known host and closes it immediately, sending
nothing.
::
