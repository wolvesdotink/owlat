...
| `packages/channels` | Inbound mail normalization — a source-keyed registry (`mta`, `resend`, …) that turns a vendor's inbound webhook envelope into one canonical `InboundEmailMessage`. Outbound channel providers are not here: they live with their only caller in `apps/api/convex/channels/adapters/` |
