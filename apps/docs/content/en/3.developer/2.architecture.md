...
| `apps/convex-fn-proxy` | Least-privilege sidecar in front of Convex for the code worker: holds the deployment admin key and forwards ONLY the worker's allowlisted `codeWorkTasks`/`plugins/workerTasks` functions, so the sandbox never holds an admin key |
...
| `apps/ostr-registry` | OSTR registry node: append-only Merkle transparency log, reference aggregator, DNS zone generation and the HTTPS query API. Deployable standalone (`--profile ostr`); see [Trust registry](/developer/trust-registry) |
...
| `packages/mta-protocol` | The Convex↔MTA wire contract — send intake, routing decisions, the IP-reputation snapshot and the webhook events, each declared once and imported by both apps |
| `packages/email-builder` | Email builder Vue components (Notion-like editor) and the client preview under `src/preview/` (compatibility analysis, Can I Email data) |
...
| `packages/mail-message` | In-house RFC 5322 / MIME message library, two halves behind one entry: the parser (header engine, part tree, charset decoding, attachments) and the composer (pure RFC 5322 / RFC 2045 construction). Replaces mailparser inbound and nodemailer's MIME builder outbound |
| `packages/mail-auth` | In-house email authentication — SPF (RFC 7208), DMARC (RFC 7489), DKIM verification (RFC 6376), and a cached injectable DNS resolver |
| `packages/mail-canon` | The one DKIM canonicalizer (RFC 6376 §3.4). A dependency-free leaf, so the inbound verifier and the outbound signer share it without a build cycle |
| `packages/smtp-client` | In-house SMTP client — the wire role nodemailer used to hold. Reply parsing, ESMTP negotiation, TLS and the send transaction |
| `packages/smtp-listener` | In-house SMTP listener — raw-net command loop, byte budget and timeouts, behind the port-25 MX and submission paths. Replaces smtp-server |
| `packages/ostr-core` | OSTR primitives: the attestation schema, JCS canonicalization, ed25519 signing, the RFC 9162-style Merkle tree and the `POLICY_V1` scoring function. Zero dependencies, shared by every OSTR role |
| `packages/ostr-client` | OSTR consumer side: DNS tier lookup, signed snapshot and diff sync, and the blocklist-shaped compatibility views |
| `packages/ostr-observer` | OSTR observer side: DKIM evidence capture, key observations, windowed aggregation under the k-anonymity floors, and batch commitments. Pure and injected, off by default |
| `packages/provider-kit` | Runtime-neutral universal send-provider bundle, trust and host-verifier contracts shared by core and plugin providers |
| `packages/plugin-kit` | Public contracts for building Owlat plugins: the manifest, contribution buckets, capabilities and per-seam module shapes |
| `packages/plugin-codegen` | Validated build-time composition — verifies package identity, lockfile integrity and realpath containment, then emits the generated catalogs and registries |
| `packages/plugin-host` | Runtime-neutral enforcement kernel for statically composed plugins: flag, grant, environment and scope authorization before every call |
| `packages/plugin-cli` | `owlat plugins` — create/add/remove/dev/codegen over the checked-in `plugins.config.ts` |
| `packages/ui` | Nuxt layer providing shared UI components and composables |
| `packages/sdk-js` | JavaScript SDK for the Owlat API |
| `packages/sdk-java` | Java SDK for the Owlat API (not a workspace member — no `package.json`, built with Maven) |

### Examples

Workspace members too (`examples/conformance` and `examples/plugins/*`), and not
decoration: the reference plugins are one per trust tier and the conformance
gallery drives them through the **shipped** host, codegen and CLI, so the plugin
platform's extension points are proven by something that has to keep compiling.

| Example | Description |
| --- | --- |
| `examples/conformance` | The plugin-platform conformance gallery: lifecycle and full-pipeline replay suites driving the three reference plugins through the real host, codegen and CLI, plus the plugin-provider parity proof — a fixture ESP bundle driven through the shipped routing, dispatch, measurement, return-path and credential-form modules |
| `examples/plugins/escalation-guard` | Tier-1 reference plugin: an in-process escalation guard contributing an agent step, a draft strategy, an automation trigger/step/condition, a webhook event and bundled UI entries |
| `examples/plugins/slack-approvals` | Tier-2 reference connected app: a restrict-only Slack approvals hold gate over the signed synchronous-hook protocol |
| `examples/plugins/deliverability-lab` | Tier-3 protocol reference: live pre-send checks behind a restrict-only gate plus a seed-list worker request fixture; no host enqueue adapter is shipped |
...
| Email send | `providers/composition.ts` (universal bundles; routing/fallback in `lib/sendProviders/routing.ts`) | `EMAIL_PROVIDER` | `mta`, `ses`, `resend`, `smtp`, `mandrill`, `emailit` (the catalog's kinds) — no implicit default; unset resolves to unconfigured |
