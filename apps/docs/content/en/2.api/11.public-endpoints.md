...
Unauthenticated liveness probe. Answers before the deployment is configured, so
it never depends on the CORS allow-list:
...
These carry no session and no signature: the token in the URL is the whole capability, and every gate is decided per request inside the handler.

- `GET /attachment-share/:token` — downloads a file shared out of a message. The token is checked for revocation, expiry, the `anyone` scope and whether the bytes still exist. Every token refusal is the same `404` in the error envelope, so the route cannot be used to learn which tokens are real (the per-IP-and-token rate limit answers `429`). The response is forced to a download (`Content-Disposition: attachment`, `X-Content-Type-Options: nosniff`) and set `Cache-Control: no-store, private`.
- `GET /sealed-blob` — the decrypt-serving proxy for a sealed storage blob, used by the Postbox reader, the IMAP bridge and the outbound MTA. The query string carries a capability token minted only after the caller was authorized at the query site; here it is checked for signature and expiry, and a bad one is a flat `403`. Only images and PDFs render inline; everything else downloads. Unlike the token endpoints above, its CORS grant is scoped to the app origin rather than `*`.
- `POST /unsub/probe/:token` — the RFC 8058 one-click unsubscribe target carried by a deliverability seed probe. The token is signed in its own namespace, so it can never be replayed against the contact endpoint, and exercising it records the interaction on the probe's ledger row rather than unsubscribing anyone.
...
- `POST /webhooks/mta-inbound` — team-inbox (shared agent inbox) inbound delivery from the MTA, carrying the raw message. See [Webhooks](/api/webhooks).
...
- `POST /mail-sync/raw-message` — raw `.eml` upload from the mail-sync worker, authenticated with the `MAIL_SYNC_API_KEY` shared secret and fail-closed when it is unset. An HTTP action rather than a function argument because a function-call body is capped at 16 MiB, which silently dropped every message over roughly 12 MiB of source.
