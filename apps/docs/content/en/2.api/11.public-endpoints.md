...
- `GET /attachment-share/:token` — downloads a file shared out of a message. The token is checked for revocation, expiry, the `anyone` scope and whether the bytes still exist. Every refusal is the same `404` in the error envelope, so the route cannot be used to learn which tokens are real. The response is forced to a download (`Content-Disposition: attachment`, `X-Content-Type-Options: nosniff`) and set `Cache-Control: no-store`.
