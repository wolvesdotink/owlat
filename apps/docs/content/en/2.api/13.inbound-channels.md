...
These endpoints ingest **incoming** messages. Outbound SMS/WhatsApp/generic sending *is* wired end-to-end (real Twilio/Meta/webhook adapters), reached from the AI-agent reply path, the per-message **Reply** button in the Team Inbox thread view, or a manual send from a contact's Unified Timeline. See [Communication Channels](/guide/channels) for what each channel can do today. This page covers the receiving side.
::
...
| Secret | SMS channel **Auth Token**, else `TWILIO_AUTH_TOKEN` |
...
| Secret | WhatsApp channel **App Secret**, else `META_APP_SECRET` |
...
- If neither the WhatsApp channel's **Verify Token** nor `META_VERIFY_TOKEN` is set, it returns `503`.
- If `hub.mode` is `subscribe` and `hub.verify_token` matches the configured token (constant-time compare), it echoes `hub.challenge` back with `200`.
...
Set the WhatsApp channel's **Verify Token** (Settings → Channels) — or `META_VERIFY_TOKEN` — to any string you choose, and enter the same value in Meta's webhook configuration as the verify token.
...
| Secret | Generic-webhook channel **Secret Key**, else `GENERIC_WEBHOOK_SECRET` |
...
## Required secrets and fail-closed behavior

Each adapter resolves its secret from the credentials stored on the channel's own config row (**Settings → Channels**), and falls back to the deployment environment variable when the channel has no such credential. If **neither** is set, the endpoint rejects with `503` — it never accepts an unsigned request "for now."

Precedence matters when both exist: the value typed into the channel form wins, so rotating a secret there takes effect without touching the deployment.

| Endpoint | Channel credential | Env fallback | Missing both → | Bad/absent signature → |
| --- | --- | --- | --- | --- |
