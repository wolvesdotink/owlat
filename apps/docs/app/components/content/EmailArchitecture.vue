<script setup lang="ts">
import { ref, onMounted } from 'vue'

const visible = ref(false)
onMounted(() => {
  requestAnimationFrame(() => { visible.value = true })
})

const builderFeatures = [
  { name: 'Slash Commands', detail: '/text, /image, /button ...', icon: 'M7 20l4-16m2 16l4-16M6 9h14M4 15h14' },
  { name: 'Format Bar', detail: 'Floating text styling', icon: 'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z' },
  { name: 'Media Library', detail: 'Image integration', icon: 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z' },
  { name: 'Focus Mode', detail: 'Cmd+Shift+F', icon: 'M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4' },
  { name: 'Auto-save', detail: 'Every 30 seconds', icon: 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15' },
]

const blockTypes = [
  'Text', 'Image', 'Button', 'Divider', 'Spacer', 'Columns', 'Social', 'Container',
]

const renderFeatures = [
  'Responsive layouts',
  'Cross-client compatible',
  'Dark mode ready',
  'A11y audits',
]

const defaultProvider = {
  name: 'Owlat MTA',
  detail: 'Direct SMTP · Intelligence pipeline · IP warming',
  icon: 'M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2',
}

const alternativeProviders = [
  { name: 'Resend', detail: 'optional', icon: 'M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z' },
  { name: 'AWS SES', detail: 'optional', icon: 'M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.002 5.002 0 10-9.78 2.096A4.001 4.001 0 003 15z' },
]

const providerFeatures = [
  { label: 'DKIM signing', icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
  { label: 'MX delivery', icon: 'M12 19l9 2-9-18-9 18 9-2zm0 0v-8' },
  { label: 'IP warming', icon: 'M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14 5 16.09 5.777 17.656 7.343A7.975 7.975 0 0120 13a7.975 7.975 0 01-2.343 5.657z' },
  { label: 'Rate limiting', icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' },
  { label: 'Health-aware failover', icon: 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15' },
]
</script>

<template>
  <div class="email-arch" :class="{ 'is-visible': visible }">

    <!-- Email Builder -->
    <div class="ea-section" :style="{ '--s': 0 }">
      <div class="ea-header">
        <div class="ea-icon">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
        </div>
        <span class="ea-label">Email Builder</span>
      </div>

      <!-- Inline Editor features -->
      <div class="ea-sublabel">Inline Editor</div>
      <div class="ea-features">
        <div
          v-for="(feat, i) in builderFeatures"
          :key="feat.name"
          class="ea-feature"
          :style="{ '--i': i }"
        >
          <div class="ea-feature-icon">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path :d="feat.icon" /></svg>
          </div>
          <div class="ea-feature-text">
            <span class="ea-feature-name">{{ feat.name }}</span>
            <span class="ea-feature-detail">{{ feat.detail }}</span>
          </div>
        </div>
      </div>

      <!-- Block system -->
      <div class="ea-sublabel" style="margin-top: 14px;">Block System</div>
      <div class="ea-blocks">
        <span v-for="(block, i) in blockTypes" :key="block" class="ea-block" :style="{ '--i': i + 5 }">{{ block }}</span>
        <span class="ea-block ea-block--more" :style="{ '--i': 13 }">+10 more</span>
      </div>
    </div>

    <!-- Connector: Builder → Rendering -->
    <div class="ea-connector" :style="{ '--s': 1 }">
      <div class="ea-connector-line" />
      <div class="ea-connector-badge">
        <span>JSON</span>
      </div>
      <div class="ea-connector-line" />
    </div>

    <!-- Email Rendering -->
    <div class="ea-section ea-section--rendering" :style="{ '--s': 2 }">
      <div class="ea-header">
        <div class="ea-icon ea-icon--accent">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>
        </div>
        <span class="ea-label">Email Rendering</span>
      </div>
      <div class="ea-pipeline">
        <span class="ea-step">JSON Blocks</span>
        <svg class="ea-arrow" width="20" height="12" viewBox="0 0 20 12" fill="none">
          <path d="M1 6h16m0 0l-4-4m4 4l-4 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
        </svg>
        <span class="ea-step ea-step--brand">@owlat/email-renderer</span>
        <svg class="ea-arrow" width="20" height="12" viewBox="0 0 20 12" fill="none">
          <path d="M1 6h16m0 0l-4-4m4 4l-4 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
        </svg>
        <span class="ea-step ea-step--success">HTML Email</span>
      </div>
      <div class="ea-tags">
        <span v-for="feat in renderFeatures" :key="feat" class="ea-tag">{{ feat }}</span>
      </div>
    </div>

    <!-- Connector: Rendering → Providers -->
    <div class="ea-connector" :style="{ '--s': 3 }">
      <div class="ea-connector-line" />
      <div class="ea-connector-badge">
        <span>HTML</span>
      </div>
      <div class="ea-connector-line" />
    </div>

    <!-- Email Providers -->
    <div class="ea-section ea-section--providers" :style="{ '--s': 4 }">
      <div class="ea-header">
        <div class="ea-icon ea-icon--info">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>
        </div>
        <span class="ea-label">Email Delivery</span>
      </div>

      <!-- Default provider (MTA) -->
      <div class="ea-provider ea-provider--default" :style="{ '--i': 9 }">
        <div class="ea-provider-icon ea-provider-icon--brand">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path :d="defaultProvider.icon" /></svg>
        </div>
        <div class="ea-provider-text">
          <span class="ea-provider-name">{{ defaultProvider.name }}</span>
          <span class="ea-provider-detail">{{ defaultProvider.detail }}</span>
        </div>
        <span class="ea-provider-badge">Default</span>
      </div>

      <!-- Alternative providers -->
      <div class="ea-sublabel" style="margin-top: 4px;">Optional alternatives</div>
      <div class="ea-providers">
        <div
          v-for="(prov, i) in alternativeProviders"
          :key="prov.name"
          class="ea-provider"
          :style="{ '--i': i + 10 }"
        >
          <div class="ea-provider-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path :d="prov.icon" /></svg>
          </div>
          <div class="ea-provider-text">
            <span class="ea-provider-name">{{ prov.name }}</span>
            <span class="ea-provider-detail">{{ prov.detail }}</span>
          </div>
        </div>
      </div>

      <div class="ea-pf">
        <div class="ea-pf-tags">
          <div
            v-for="(feat, i) in providerFeatures"
            :key="feat.label"
            class="ea-pf-tag"
            :style="{ '--i': i + 11 }"
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path :d="feat.icon" /></svg>
            <span>{{ feat.label }}</span>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<style scoped>
.email-arch {
  margin: 2rem 0;
  display: flex;
  flex-direction: column;
  align-items: stretch;
}

/* Sections */
.ea-section {
  border: 1px solid var(--color-border-default);
  border-radius: 12px;
  padding: 16px 18px;
  background: var(--color-bg-elevated);
  opacity: 0;
  transform: translateY(14px);
  transition: opacity 0.6s var(--ease-spring), transform 0.6s var(--ease-spring), border-color var(--motion-moderate), box-shadow var(--motion-moderate);
  transition-delay: calc(var(--s) * 0.12s);
  position: relative;
  overflow: hidden;
}

.is-visible .ea-section {
  opacity: 1;
  transform: translateY(0);
}

.ea-section:hover {
  border-color: var(--color-border-strong);
  box-shadow: 0 4px 20px rgba(0, 0, 0, 0.15);
}

.ea-header {
  display: flex;
  align-items: center;
  gap: 10px;
  margin-bottom: 14px;
}

.ea-icon {
  display: flex;
  align-items: center;
  justify-content: center;
  width: 30px;
  height: 30px;
  border-radius: 8px;
  background: color-mix(in oklab, var(--color-brand) 12%, var(--color-bg-surface));
  color: var(--color-brand);
}

.ea-icon--accent {
  background: color-mix(in oklab, var(--color-accent) 12%, var(--color-bg-surface));
  color: var(--color-accent);
}

.ea-icon--info {
  background: color-mix(in oklab, var(--color-info) 12%, var(--color-bg-surface));
  color: var(--color-info);
}

.ea-label {
  font-weight: 600;
  font-size: 0.9375rem;
  color: var(--color-text-primary);
}

.ea-sublabel {
  font-size: 0.6875rem;
  font-weight: 600;
  text-transform: uppercase;
  letter-spacing: 0.05em;
  color: var(--color-text-tertiary);
  margin-bottom: 8px;
}

/* Builder features */
.ea-features {
  display: grid;
  grid-template-columns: repeat(3, 1fr);
  gap: 8px;
}

@media (max-width: 680px) {
  .ea-features {
    grid-template-columns: repeat(2, 1fr);
  }
}

@media (max-width: 480px) {
  .ea-features {
    grid-template-columns: 1fr;
  }
}

.ea-feature {
  display: flex;
  align-items: center;
  gap: 8px;
  padding: 8px 10px;
  border-radius: 8px;
  background: var(--color-bg-surface);
  border: 1px solid transparent;
  transition: border-color var(--motion-moderate), background var(--motion-moderate), transform var(--motion-moderate) var(--ease-spring);
  opacity: 0;
  transform: translateY(8px);
}

.is-visible .ea-feature {
  opacity: 1;
  transform: translateY(0);
  transition-delay: calc(0.1s + var(--i) * 0.06s);
}

.ea-feature:hover {
  border-color: var(--color-border-default);
  background: var(--color-bg-surface-hover);
  transform: translateY(-1px);
  transition-delay: 0s;
}

.ea-feature-icon {
  display: flex;
  align-items: center;
  justify-content: center;
  width: 26px;
  height: 26px;
  border-radius: 6px;
  background: var(--color-bg-elevated);
  color: var(--color-brand-muted);
  flex-shrink: 0;
}

.ea-feature-text {
  display: flex;
  flex-direction: column;
  gap: 1px;
  min-width: 0;
}

.ea-feature-name {
  font-weight: 600;
  font-size: 0.75rem;
  color: var(--color-text-primary);
  white-space: nowrap;
}

.ea-feature-detail {
  font-size: 0.625rem;
  color: var(--color-text-tertiary);
  font-family: var(--font-mono);
  white-space: nowrap;
  overflow: hidden;
  text-overflow: ellipsis;
}

/* Block types */
.ea-blocks {
  display: flex;
  flex-wrap: wrap;
  gap: 6px;
}

.ea-block {
  padding: 4px 10px;
  border-radius: 6px;
  background: var(--color-bg-surface);
  border: 1px solid var(--color-border-subtle);
  font-size: 0.6875rem;
  font-family: var(--font-mono);
  color: var(--color-text-secondary);
  transition: border-color var(--motion-moderate), color var(--motion-moderate);
  opacity: 0;
}

.is-visible .ea-block {
  opacity: 1;
  transition-delay: calc(0.1s + var(--i) * 0.04s);
}

.ea-block:hover {
  border-color: var(--color-border-default);
  color: var(--color-text-primary);
}

.ea-block--more {
  border-style: dashed;
  color: var(--color-text-tertiary);
}

/* Pipeline */
.ea-pipeline {
  display: flex;
  align-items: center;
  gap: 10px;
  flex-wrap: wrap;
  margin-bottom: 12px;
}

.ea-step {
  padding: 6px 12px;
  border-radius: 6px;
  background: var(--color-bg-surface);
  border: 1px solid var(--color-border-subtle);
  font-size: 0.75rem;
  font-family: var(--font-mono);
  color: var(--color-text-secondary);
  white-space: nowrap;
}

.ea-step--brand {
  border-color: var(--color-brand-dim);
  color: var(--color-brand);
  background: color-mix(in oklab, var(--color-brand) 6%, var(--color-bg-surface));
}

.ea-step--success {
  border-color: color-mix(in oklab, var(--color-success) 30%, var(--color-border-subtle));
  color: var(--color-success);
  background: color-mix(in oklab, var(--color-success) 6%, var(--color-bg-surface));
}

.ea-arrow {
  color: var(--color-text-disabled);
  flex-shrink: 0;
}


.is-visible .ea-arrow:nth-of-type(2) {
  animation-delay: 0.5s;
}


.ea-tags {
  display: flex;
  gap: 6px;
  flex-wrap: wrap;
}

.ea-tag {
  padding: 3px 10px;
  border-radius: 9999px;
  background: var(--color-bg-soft);
  border: 1px solid var(--color-border-subtle);
  font-size: 0.6875rem;
  color: var(--color-text-tertiary);
}

/* Connectors */
.ea-connector {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 0;
  padding: 2px 0;
  opacity: 0;
  transition: opacity var(--motion-slow) var(--ease-spring);
  transition-delay: calc(var(--s) * 0.12s + 0.1s);
}

.is-visible .ea-connector {
  opacity: 1;
}

.ea-connector-line {
  width: 2px;
  height: 10px;
  background: var(--color-border-strong);
  border-radius: 1px;
}

.ea-connector-badge {
  display: flex;
  align-items: center;
  justify-content: center;
  padding: 2px 10px;
  border-radius: 9999px;
  background: var(--color-bg-surface);
  border: 1px solid var(--color-border-default);
  animation: ea-badge-pulse 3s ease-in-out infinite;
}

.ea-connector-badge span {
  font-size: 0.625rem;
  font-family: var(--font-mono);
  font-weight: 600;
  color: var(--color-text-tertiary);
  text-transform: uppercase;
  letter-spacing: 0.05em;
}

@keyframes ea-badge-pulse {
  0%, 100% { box-shadow: 0 0 0 0 rgba(196, 120, 90, 0); }
  50% { box-shadow: 0 0 8px 2px rgba(196, 120, 90, 0.1); }
}

/* Default provider card */
.ea-provider--default {
  display: flex;
  align-items: center;
  gap: 10px;
  padding: 12px 16px;
  border-radius: 8px;
  background: color-mix(in oklab, var(--color-brand) 6%, var(--color-bg-surface));
  border: 1px solid var(--color-brand-dim);
  margin-bottom: 12px;
  opacity: 0;
  transform: translateY(8px);
  transition: border-color var(--motion-moderate), transform var(--motion-moderate) var(--ease-spring), background var(--motion-moderate), opacity var(--motion-slow) var(--ease-spring);
}

.is-visible .ea-provider--default {
  opacity: 1;
  transform: translateY(0);
  transition-delay: calc(0.15s + var(--i) * 0.07s);
}

.ea-provider--default:hover {
  border-color: var(--color-brand);
  transform: translateY(-1px);
  transition-delay: 0s;
}

.ea-provider-icon--brand {
  background: color-mix(in oklab, var(--color-brand) 12%, var(--color-bg-surface));
  color: var(--color-brand);
}

.ea-provider-badge {
  margin-left: auto;
  padding: 2px 8px;
  border-radius: 9999px;
  background: color-mix(in oklab, var(--color-brand) 15%, var(--color-bg-surface));
  color: var(--color-brand);
  font-size: 0.6875rem;
  font-weight: 600;
  flex-shrink: 0;
}

/* Providers */
.ea-providers {
  display: grid;
  grid-template-columns: repeat(2, 1fr);
  gap: 10px;
  margin-bottom: 12px;
}

@media (max-width: 580px) {
  .ea-providers {
    grid-template-columns: 1fr;
  }
}

.ea-provider {
  display: flex;
  align-items: center;
  gap: 10px;
  padding: 10px 14px;
  border-radius: 8px;
  background: var(--color-bg-surface);
  border: 1px solid transparent;
  transition: border-color var(--motion-moderate), transform var(--motion-moderate) var(--ease-spring), background var(--motion-moderate);
  opacity: 0;
  transform: translateY(8px);
}

.is-visible .ea-provider {
  opacity: 1;
  transform: translateY(0);
  transition-delay: calc(0.15s + var(--i) * 0.07s);
}

.ea-provider:hover {
  border-color: var(--color-border-default);
  background: var(--color-bg-surface-hover);
  transform: translateY(-1px);
  transition-delay: 0s;
}

.ea-provider-icon {
  display: flex;
  align-items: center;
  justify-content: center;
  width: 32px;
  height: 32px;
  border-radius: 8px;
  background: var(--color-bg-elevated);
  color: var(--color-info);
  flex-shrink: 0;
}

.ea-provider-text {
  display: flex;
  flex-direction: column;
  gap: 1px;
}

.ea-provider-name {
  font-weight: 600;
  font-size: 0.8125rem;
  color: var(--color-text-primary);
}

.ea-provider-detail {
  font-size: 0.6875rem;
  color: var(--color-text-tertiary);
  font-family: var(--font-mono);
}

/* Provider features */
.ea-pf {
  padding: 12px 14px;
  border-radius: 8px;
  background: var(--color-bg-soft);
  border: 1px solid var(--color-border-subtle);
}

.ea-pf-tags {
  display: flex;
  gap: 8px;
  flex-wrap: wrap;
}

.ea-pf-tag {
  display: flex;
  align-items: center;
  gap: 6px;
  padding: 5px 10px;
  border-radius: 6px;
  background: var(--color-bg-surface);
  border: 1px solid var(--color-border-subtle);
  font-size: 0.75rem;
  color: var(--color-text-secondary);
  transition: border-color var(--motion-moderate), transform var(--motion-moderate) var(--ease-spring);
  opacity: 0;
}

.is-visible .ea-pf-tag {
  opacity: 1;
  transition-delay: calc(0.2s + var(--i) * 0.06s);
}

.ea-pf-tag svg {
  color: var(--color-info);
  flex-shrink: 0;
}

.ea-pf-tag:hover {
  border-color: var(--color-border-default);
  transform: translateY(-1px);
  transition-delay: 0s;
}
</style>
