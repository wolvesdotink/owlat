...
| [#732](https://github.com/wolvesdotink/owlat/issues/732) | Medium: authenticated caller forges upload size metadata | Read native storage sizes before resource binding or outbound buffering; enforce actual per-file, count, and aggregate limits including inline images. [PR #730](https://github.com/wolvesdotink/owlat/pull/730). |

The worker metadata issue was found during fix review. Upload transport was revised after review identified Convex HTTP limits and Nuxt middleware buffering. Legacy deletion paths and client-supplied file sizes were revisited after the first ownership patch. These review findings are included in the corresponding fixes, rather than treated as separate completed work.
...
- Late size-policy regression checks use real stored blobs with smaller test thresholds, plus outbound preflight tests that assert no blob read occurs on rejection. Nine affected suites / 114 tests and 12 outbound tests passed; an independent reviewer also passed 37 size/share tests and recorded the review on PR #730.
...
Previously disclosed download URLs and downloaded bytes cannot be recalled by metadata classification. Legacy references without reliable ownership receipts remain readable but do not authorize destructive deletion; this can leave old orphaned bytes requiring separately reviewed cleanup. Pending native uploads minted before the rollout must be retried with a new upload URL. File-size admission uses native storage metadata; direct draft attachments, including inline images, obey the shared count and aggregate limits. Outbound buffering rechecks metadata so legacy forged sizes cannot bypass those limits.
