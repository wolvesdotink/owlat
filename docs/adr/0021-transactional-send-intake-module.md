...
| `convex/transactionalSends.ts` (the file as a whole) | Renamed → `convex/transactional/sends.ts`. Otherwise unchanged. Read queries stay (`listByTransactionalEmail`, `listAll`, `get`, `getByProviderMessageId`, `getStatsByTransactionalEmail`, `getCountByTransactionalEmail`, `getCounts`, `getByEmail`). The unreachable `deleteByTransactionalEmail` mutation was later removed under #528. |
